...
Looks like you wanted to start the **Admin** or **Website** app locally via the [`webiny watch`](/docs/{version}/core-development-concepts/basics/watch-command) command. But, upon doing that, the required environment variables that define values like the GraphQL API URL, or Amazon Cognito User Pool Client ID, could not be assigned.
