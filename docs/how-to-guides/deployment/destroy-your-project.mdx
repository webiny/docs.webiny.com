...
This command lets you destroy cloud infrastructure previously deployed by a single project application. <!-- This can be handy if you want to avoid completely destroying all cloud infrastructure that was deployed by the default project applications, which is what the shown `destroy` command does. -->

As its first argument, the `app destroy` command receives the path to the project application folder. You also need to specify the environment in which the cloud infrastructure was previously deployed, which is specified via the `--env` argument.
...
We're aware of this fact, and this is mainly because of the [Amazon ElasticSearch Service](https://aws.amazon.com/elasticsearch-service/). While other cloud infrastructure resources get destroyed reasonably fast, this service can take anywhere from 15 to 30 minutes to destroy itself.

Unfortunately, this is a well-known issue for quite some time. We've even seen cases where the service is stuck in user's account [for days](https://forums.aws.amazon.com/thread.jspa?threadID=233859). Until the present, there haven't been any positive improvements on the subject.
