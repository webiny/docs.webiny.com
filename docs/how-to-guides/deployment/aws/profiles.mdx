...
While executing deployment-related commands, for example [`deploy`](/docs/how-to-guides/deployment/deploy-your-project) or [`destroy`](/docs/how-to-guides/deployment/destroy-cloud-infrastructure), if not specified, Webiny will always use the `default` AWS profile.

A different profile can be specified via the `AWS_PROFILE` environment variable, which you can set in a couple of ways.

### Using `.env` File
The easiest way to do it would be via the `.env` file, located in your project root, for example:
