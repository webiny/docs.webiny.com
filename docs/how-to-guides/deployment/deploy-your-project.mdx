...
## List AWS resources
After deployment, you can view the list of AWS resources and services used by your project application.
As mentioned above, every Webiny project, by default, has three applications.
The **api** (`./api`), which represents your project's (GraphQL) API, **admin** (`./apps/admin`), and the **website** (`./apps/website`), which is your public website.
Each application uses several cloud infrastructure resources like Lambda functions, API Gateway, DynamoDB table, etc.

To list the resources used by a particular application for an [environment](/docs/key-topics/deployment/environments), you can use the `stack` command.
```bash
yarn webiny pulumi <application-name> --env <environment-name> stack
```

For example, to list resources used by the `api` application for `dev` environment, run the following command:

```bash
yarn webiny pulumi api --env dev stack
```
...
Similarly, you can use the following commands to list AWS resources used by the `admin` and `website` application for the `dev` environment.

```bash
yarn webiny pulumi apps/admin --env dev stack
yarn webiny pulumi apps/website --env dev stack
```
