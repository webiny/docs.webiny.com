...
Before we begin, it's worth mentioning that the deployment commands shown below also build your application code. They perform this task by just running the following [`workspaces run`](/docs/how-to-guides/webiny-cli/working-with-workspaces) command:
