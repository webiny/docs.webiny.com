...
description: Learn how to deploy your Webiny project.
---
...
Before we begin, it's worth mentioning that the deployment commands shown below will also build your application code. They will perform this task by just running the following **workspaces** command:

```
yarn webiny workspaces run build --folder {STACK_FOLDER}
```

The command first finds all packages that are located in the folder in which your project application resides. Then, it will run the `build` command that's specified in the `webiny.config.js`, which is a configuration file that every package possesses. Note that if the command exists, it will get executed. Otherwise, the command continues with the next package in line.
...
To make it a bit easier for newcoming users, Webiny CLI provides the `deploy` command, which will deploy all of the three applications that are by default included in a Webiny project, in the following order:

1. **api**
2. **apps/admin**
3. **apps/site**

So, to do a complete deploy of your Webiny project, in your terminal of choice and in your project root, execute the following command:
...
:::info
Note that the first deployment can take up to 20 minutes! So, even though it might look nothing is happening in the terminal, please be patient and let the process finish. If something went wrong, an error will be shown, and the process will exit with an appropriate error code.
:::

### Different environments
In general, when deploying projects or project applications, an environment must be specified. The `deploy` command will deploy your project into the `dev` environment by default, but you can also specify a different environment with the `--env` argument. For example, to deploy your project into the `prod` environment, you can run the following command:
...
As mentioned, the `deploy` command is made for newcoming users, making the initial deployment as easy as possible. In that sense, you need to be aware of a couple limitations of this command.
