...
The **API** (`./api`), which represents your project's (GraphQL) API, **Admin Area** (`./apps/admin`), and the **Website** (`./apps/website`), which is your public website.
Each application uses several cloud infrastructure resources like AWS Lambda functions, API Gateway, DynamoDB table, etc.
...
```bash
yarn webiny pulumi {folder} --env {env} -- stack
```
