...
This article guides you through configuring your AWS credentials for programmatic access. At the end of the tutorial, you will get your API keys needed to successfully deploy Webiny to your AWS cloud.
...
<iframe
  width="805"
  height="390"
  src="https://www.youtube.com/embed/qmtDRmplMG4"
  frameborder="0"
  allowfullscreen
></iframe>
...
If you're on UNIX, create a folder named `.aws` inside your user folder, that's `~/.aws`. Inside that folder place a file called `credentials`. So the full path will be `~/.aws/credentials`.
...
![aws-credentials](./assets/configure-aws-credentials/aws-credentials.png)

Here is a code snippet in case you want to copy paste it:
...
And that's it! You now have your AWS credentials, and you're ready to deploy Webiny.
