...
We need to add the Tailwind CSS to the `postcss-loader` plugins inside the Webpack configuration. Every package in a Webiny application has a `webiny.config.ts` file that exports development-related commands. In case of **Website** (`apps/website/code/webiny.config.ts`), it contains the following commands:
...
As we can see in the code above, we export an object with `commands.watch` and `commands.build` functions, where we use the `createWatchApp` and `createBuildApp` helper functions, imported from the `@webiny/project-utils` package.
...
It traverses all the loaders defined in `modules.rules` property of the Webpack configuration and passes each one of them to your callback. You can then apply your modifications. In our case, we add Tailwind CSS plugins to the [`postcss-loader`](https://github.com/webiny/webiny-examples/blob/master/integrate-tailwind-css/code/applyTailwindToWebpack.js#L13).
...
:::note
We've created the [`tailwind.config.js`](https://github.com/webiny/webiny-examples/blob/master/integrate-tailwind-css/tailwind.config.js) file in the root of Webiny project so that we can share it across all Webiny applications,
for example, `apps/admin` and `apps/website`. But we also need it there because of different current working directories, explained further in the article.
:::
...
**Here comes the important part**: we need to configure the `content` property to point to files that _can_ contain TailwindCSS classes, so that Tailwind can generate the minimal CSS output based on those classes only. When running the Webiny CLI, some commands are executed from the root of the project, and the current working directory (CWD) is the root of the project, and some commands will be executed from the app folders, and so the CWD will be different. Because of that, we need to define the `content` paths accordingly.

In the root `tailwind.config.js` file, define the `content` property like so:

```js title="tailwind.config.js"
module.exports = {
  content: ["./apps/website/code/public/*.html", "./apps/website/code/src/**/*.tsx"],
  theme: {
    extend: {}
  },
  plugins: []
...
Now that we have this global configuration, let's use it in the **Website** `(apps/website)` React application by creating a `tailwind.config.js` file in `apps/website/code` directory, and override the `content` property to make the paths relative to the location of that file:
...
With this, we make sure that TailwindCSS is able to figure out the files it needs to process for both `watch` and `deploy` commands, which internally use different current working directories.
...
Tailwind CSS swaps these directives out at build-time with all the styles it generates based on your configured design system.
