...
As the name suggests, the **New React Application** scaffold creates a new standalone React application, on top of which you can build [client-side rendered (CSR)](https://developers.google.com/web/updates/2019/02/rendering-on-the-web) applications. This is ideal for secured or non-public-facing applications that do not need to be [SEO](https://developers.google.com/search/docs/beginner/seo-starter-guide?hl=en%2F&visit_id=637648024549328679-3615718033&rd=1) compatible.

The created application consists of a couple of simple pages and examples which makes it possible to quickly get started with building a new frontend application.
...
Out of the box, the newly created React application contains simple home and not-found (404) pages. These will give you an idea on how to approach solving some of the common requirements:
...
Once you start building your application, you can remove them or simply move them into a separate folder, potentially for future reference.
...
#### [`code/src/`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src)

Contains the React application code. This is where you'll be creating your applications pages, shared React components, defining routes, styles, and more.

#### [`code/src/components`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src/components)

Contains React components that can be shared between multiple pages, for example, a global application layout.

#### [`code/src/images`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src/images)

Contains images that can be imported either from your React component or from SCSS files.

#### [`code/src/plugins`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src/plugins)
...
#### [`code/src/styles`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src/styles)

Contains SCSS files that define styles used by your application. And although, out of the box, this is only way to define styles, if you maybe prefer a CSS-in-JS solution, you're always free to add additional libraries.

#### [`code/package.json`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/package.json)

In case you missed it, every Webiny project is organized as a [monorepo](/docs/key-topics/project-organization/monorepo-organization), which can consist of multiple packages. This is the reason why the [`package.json`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/package.json) manifest file exists.
...
#### [`pulumi/`](https://github.com/webiny/webiny-js/tree/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/pulumi)
...
### No Initial Deployment Needed

As mentioned above, frontend application development can still be performed locally, on the developer's machine.

Which means no initial deployment is needed. Once you've completed the scaffold's wizard and the files have been generated, in order to start developing, you can simply start by running the [`webiny watch`](/docs/how-to-guides/use-watch-command) command (see below). Only when you actually want to see your application online, you can proceed as usual, by running the [`webiny deploy`](/docs/how-to-guides/deployment/deploy-your-project) command.
...
Once you've completed the scaffold's wizard and the files have been generated, the first steps would be to start creating new pages, in the similar fashion it's already done with the existing example pages. Simply create a new [`RoutePlugin`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/app/src/plugins/RoutePlugin.tsx#L8) plugins inside of the [`code/src/plugins`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src/plugins) folder, and start creating your page within it.

Every application has one or more layouts, so you can also start editing the [`code/src/components/Layout.tsx`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src/components/Layout.tsx). Feel free to remove anything that's not needed. The styles for the layout can be found in the [`code/src/styles/global.scss`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/src/styles/global.scss#L33) file.
...
But, unless the new React application was created with the [New Full-Stack Application](/docs/how-to-guides/scaffolding/new-full-stack-application) scaffold, you will have to manually link your React application with an existing GraphQL API.

For example, let's say our application needs to interact with the main GraphQL API that's, by default, set up with every Webiny project and is deployed as part of the **API** project application, located within the [`api`](https://github.com/webiny/webiny-js/tree/cli-scaffold-react-app/packages/cwp-template-aws/template/api) folder.
...
So, we need to add the following lines into the [`code/webiny.config.ts`](https://github.com/webiny/webiny-js/blob/cli-scaffold-react-app/packages/cli-plugin-scaffold-react-app/template/code/webiny.config.ts):

```ts title="webiny.config.ts" {1,3,5,6,7,9,17-21,27-31}
...
// See https://www.webiny.com/docs/how-to-guides/scaffolding/new-react-application/webiny-config.
export default {
  commands: {
...
If you need something like SSR, then you might look into other frameworks, for example [Next.js](https://nextjs.org/docs/basic-features/pages#server-side-rendering).
