...
The **Extend GraphQL API** scaffold extends your project's GraphQL HTTP API with new [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete) query and mutation operations.
...
The generated application code follows the same idea. There aren't any additional steps that need to be taken in order to enable multi-locale support.

### Testing Setup

With the application code, the scaffold also generates one unit, one integration, and one end-to-end (E2E) test, which you can use as a reference in further development. All of the tests can be found in the [`__tests__`](https://github.com/webiny/webiny-js/tree/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/__tests__) folder located within the scaffolded application code. More on this in the [Development](/docs/how-to-guides/webiny-cli/scaffolding/extend-graphql-api#development) section below.
...
The scaffold does not add any additional cloud infrastructure resources. The generated application code is simply added to the existing GraphQL API application code ([`api/code/graphql`](https://github.com/webiny/webiny-js/tree/v5.9.0/packages/cwp-template-aws/template/api/code/graphql)).

Of course, if additional cloud infrastructure resources are needed, you're free amend the [existing cloud infrastructure code](https://github.com/webiny/webiny-js/tree/v5.9.0/packages/cwp-template-aws/template/api/pulumi), and reference added and deployed resources in your application code.
...
[`resolvers/`](https://github.com/webiny/webiny-js/tree/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/resolvers)
...
[`entities/`](https://github.com/webiny/webiny-js/tree/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/entities)
...
[`__tests__/`](https://github.com/webiny/webiny-js/tree/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/__tests__)
...
And although developers often choose this approach when developing smaller features or proof of concepts, for anything larger in scope, we recommend new code is tested via one or more of the mentioned types of tests.
...
<!-- With unit tests ([`typeDefs.unit.test.ts`](https://github.com/webiny/webiny-js/blob/v5/packages/cli-plugin-scaffold-graphql-service/template/__tests__/typeDefs.unit.test.ts)), we test a single unit in our code, for example a specific business logic located within a specific function or a class. These tests are the fastest to run simply because no cloud infrastructure resources need to be deployed in order for them to be run. Otherwise we're talking about integration tests. -->

<!-- Integration tests ([`crud.integration.test.ts`](https://github.com/webiny/webiny-js/blob/v5/packages/cli-plugin-scaffold-graphql-service/template/__tests__/crud.integration.test.ts)) allow us to test our code with real cloud infrastructure resources in the mix. So, have in mind that you'll need to have your **API** project application deployed. -->

<!-- Finally, end-to-end tests ([`crud.e2e.test.ts`](https://github.com/webiny/webiny-js/blob/v5/packages/cli-plugin-scaffold-graphql-service/template/__tests__/crud.e2e.test.ts)) enable -->
...
Yes, you can. You can actually utilize the already deployed ElasticSearch cluster that's by default deployed with every Webiny project.
