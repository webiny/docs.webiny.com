...
As an example, during the scaffold's setup wizard, if we were to enter `CarManufacturer` as the initial data model name, we'd end up with the following:

1. a CRUD view which let us perform basic CRUD operations on car manufacturer entries
...
This is the entrypoint file, which exports the [`menus`](https://github.com/webiny/webiny-js/blob/next/packages/cli-plugin-scaffold-admin-app-module/template/index.ts#L1) and [`routes`](https://github.com/webiny/webiny-js/blob/next/packages/cli-plugin-scaffold-admin-app-module/template/index.ts#L2) plugins.
...
Contains all of the React components and [hooks](https://github.com/webiny/webiny-js/tree/v5.9.0/packages/cli-plugin-scaffold-admin-app-module/template/views/hooks) that form the CRUD view. The main parent component is exported in the [`views/index.tsx`](https://github.com/webiny/webiny-js/blob/v5.9.0/packages/cli-plugin-scaffold-admin-app-module/template/views/index.tsx#L10) file.
...
In order to get started, from your project root, you can run the following two commands.
