...
- replace your `.babel.node.js` with the config below:

```js title=".babel.node.js" {1,20-28}
module.exports = ({ path }) => {
  return {
    presets: [
      [
...
- replace your `.babel.react.js` with the config below:

```js title=".babel.react.js" {1,56-64}
module.exports = ({ path }) => ({
  presets: [
    [
      "@babel/preset-env",
