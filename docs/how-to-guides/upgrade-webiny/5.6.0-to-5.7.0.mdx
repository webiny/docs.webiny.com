---
id: 5.6.0-to-5.7.0
title: 5.6.0 to 5.7.0
sidebar_label: 5.6.0 to 5.7.0
...
:::info
Make sure to check out the [5.7.0 changelog](/docs/changelog/5.7.0) to get familiar with all the changes introduced in this release.
:::
...
As you can see, we now use a configuration object to explicitly pass `plugins` and other parameters required by the Lambda handler factory package. The `http: { debug }` will be passed all the way to the internal [@webiny/handler-http](https://github.com/webiny/webiny-js/blob/next/packages/handler-http/src/index.ts#L56) package which will handle the errors for us.

:::info NOT SURE WHAT TO DO?
If unsure what the final result should look like, please see [the original file](https://github.com/webiny/webiny-js/blob/next/packages/cwp-template-aws/template/api/code/graphql/src/index.ts) for a complete example.
:::
...
:::info NOT SURE WHAT TO DO?
If unsure what the final result should look like, please see [the original file](https://github.com/webiny/webiny-js/blob/next/packages/cwp-template-aws/template/api/code/headlessCMS/src/index.ts) for a complete example.
:::
