...
import firstDeployAfterUpgrade from "./5.10.0-to-5.11.0/first-deploy-after-upgrade.png";
import pulumiCliError from "./5.10.0-to-5.11.0/pulumi-cli-error.png";
...
:::danger
As stated in the [Overview](/docs/how-to-guides/upgrade-webiny/overview#pre-production-environments-first) section, to prevent any data loss or other critical problems that might occur while upgrading, we recommend that you first deploy your changes into one of your pre-production environments, like `dev` or `staging`.
:::

As mentioned in the [changelog](/docs/changelog/5.11.0), from the 5.11.0 release, all Webiny projects will use [Pulumi](/docs/key-topics/deployment/iac-with-pulumi) v3.
