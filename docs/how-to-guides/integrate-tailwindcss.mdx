...
Let's see how to do that. In this guide, we integrate `tailwindcss` to the **Website** `(apps/website)` application.
...
It traverses all the loaders defined in `modules.rules` property of the webpack configuration.
And then for every loader that includes `"postcss-loader"` we add `tailwindcss` as PostCSS plugin.

Now that we have add `tailwindcss` to webpack configuration. Let's move on to the next step.
