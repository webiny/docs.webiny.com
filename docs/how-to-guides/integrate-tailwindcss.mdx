...
It allows us to quickly build user interfaces with the help of a plethora of built-in CSS classes, without ever leaving our HTML.
...
Install `tailwindcss` and its peer-dependencies using yarn by running the following command from root of your project:
...
In this guide, we integrate Tailwindcss in the **Website** project application's React application `(apps/website)`.
...
Every Webiny application has a `webiny.config.ts` file that exports the often used development-related commands.
...
We use `options.webpack` to add Tailwindcss in the Webpack configuration in both `startApp` and `buildApp` commands.

:::info
We made this change in both `startApp` and `buildApp` commands so that we can use Tailwindcss in both local development and deployed application.
:::
...
You can then apply your modifications. In our case, we add Tailwindcss plugins to the [`postcss-loader`](https://github.com/webiny/webiny-examples/blob/master/integrate-tailwind-css/code/applyTailwindToWebpack.js#L13).
...
Now it's time to include the Tailwindcss directives in your styles.
As most of the styles for Webiny applications are present in `apps/theme` package, we also include tailwind styles there.
...
Tailwindcss swaps these directives out at build-time with all of the styles it generates based on your configured design system.
...
```
Tailwindcss is ready to use in your Webiny project.

### Use Tailwind CSS in Website

Now, you're all set to use the Tailwind CSS in your **Website** `(apps/website)` React application,
let's try adding an example section made with Tailwindcss as shown below:
...
#### Can I include the Tailwindcss directives in Website `(app/website)` React application directly?

Yes, you can include the directives directly in the style file of in React application.
We choose the `apps/theme` package in the guide because it's being used in all React applications of a Webiny project, as a result, we don't have to include these directives in every React application.
