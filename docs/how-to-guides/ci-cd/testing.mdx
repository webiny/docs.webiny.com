...
While designing our CI/CD workflow, one of thing we need to pay attention to is how fast it is being executed or in other words, how fast it can provide valuable feedback to our organization.
...
:::info
Merging strategies are covered in the upcoming [Workflows](/docs/how-to-guides/ci-cd/workflows) guide.
:::

#### Local Development

It's useful to mention that developers should also able to run tests without the need of an external CI/CD workflow. This is important when making code changes locally, where we want to have a fast feedback loop and quickly ensure we aren't breaking any existing functionality. We certainly don't want to end up waiting for the CI/CD workflow to tell us that.

### Deploying Short-Lived (Ephemeral) Environments For Testing Purposes

As mentioned in the previous [Points of Execution](/docs/how-to-guides/ci-cd/testing#points-of-execution) section, tests can be run upon submitting PRs. And since integration and end-to-end (E2E) tests require cloud infrastructure resources to be deployed, we end up with a need to deploy our project into a short-lived, or sometimes also called an ephemeral, environment.
...
In some cases, doing this can [incur additional cost](/docs/how-to-guides/ci-cd/environments#multiple-environments-and-extra-costs).
