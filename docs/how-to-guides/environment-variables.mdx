...
Webiny is a full-stack solution, meaning, out of the box, developers are able to create both backend and frontend applications. But, before we continue, have in mind that these are two different types of applications, for which different development practices and technologies apply.
...
Note that, upon executing any `webiny` command, the [Webiny CLI](/docs/key-topics/webiny-cli) automatically injects the following two environment variables into the running process:

- `WEBINY_ENV` - https://create-react-app.dev/docs/adding-custom-environment-variables/ironment that was passed via the `--env` argument
- `WEBINY_PROJECT_NAME` - the project name, set in your `webiny.root.js` file (`projectRoot`)
...
When you set up a brand new Webiny project, the contents of this file might look similar to the following:
...
From there, we can easily add additional environment variables, like we did with the `MY_CUSTOM_ENV_VARIABLE`. Once set, we can pass them to cloud infrastructure resources that we might have, for example, a Lambda function. If we wanted to add an environment variable to the existing `graphql` Lambda function, deployed within the default **API** project application, we could do that by setting it directly via its `environment.variables` property:
    ...
    api: new aws.lambda.Function("graphql", {
        ...
        environment: {
            variables: {
                ...
                MY_PROCESS_ENV_VARIABLE: process.env.MY_PROCESS_ENV_VARIABLE
            }
        },
        vpcConfig: {
...
While in the context of backend applications, the environment variables represent values that actually exist in the running Node.js process, for frontend applications, the values are embedded in the application code during the build time.

#### Local Development - No Deployment Needed
Developing serverless backend applications usually entails redeploying your application code, in order to see that changes that were made. With frontend development, that is not the case, and adding an environment variable on our machine will usually result in our web development server refreshing the application.
...
#### Can you provide any details on how the `.env` file is loaded?

Behind the scenes, the Webiny CLI uses the [`dotenv`](https://www.npmjs.com/package/dotenv) library in order to load the shown `.env` file. Note that the values that are defined in the `.env` file will not get assigned as environment variables if they were already assigned in the running process (e.g. via terminal, or as a secret in your CI/CD workflow). This is the default behaviour of the `dotenv` library.

#### Are there any other commands that load the `.env` file?

The following commands load the `.env` file: [`deploy`](/docs/how-to-guides/deployment/deploy-your-project#the-deploy-command), [`destroy`](/docs/how-to-guides/deployment/destroy-cloud-infrastructure#the-destroy-command), and `pulumi`.
