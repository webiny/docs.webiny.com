...
There are few plugins types that you need to have in mind when creating a new field. For the UI part of the application, there are `CmsEditorFieldTypePlugin` and `CmsEditorFieldRendererPlugin`. For the API part of the application, there are `CmsModelFieldToGraphQLPlugin`, `CmsModelFieldToStoragePlugin` `CmsModelFieldToElasticSearchPlugin` of which only `CmsModelFieldToGraphQLPlugin` is required to make the field work.

### Creating a plugin of type `CmsEditorFieldTypePlugin`
...
export default (): CmsEditorFieldTypePlugin => ({
  ...
  name: "cms-editor-field-type-target",
  field: {
    type: "target",
    label: `Target`,
    description: `A description the of field`,
    icon: <Icon />,
    allowMultipleValues: true,
    allowPredefinedValues: false,
    multipleValuesLabel: `A description used when there are multiple values enabled.`,
    createField() {
      return {
        type: "target",
        settings: {},
...
This example adds a `target` type field to the UI which can be set to accept multiple values.
...
### Creating a plugin of type `CmsEditorFieldRendererPlugin`

Plugins of this type are used to render a field in the create or update entry form in the UI side of the application.
A simple example of the renderer meant to render a `target` type field when set to allow multiple values.

```tsx
export default (): CmsEditorFieldRendererPlugin => {
    type: "cms-editor-field-renderer",
    name: "cms-editor-field-renderer-target-multiple",
    renderer: {
        rendererName: "targetsRenderer",
        name: `Targets`,
        description: `Renders targets.`,
        canUse({ field }) {
            return field.type === "target" && field.multipleValues;
        },
        render(props) {
            return (
                <TargetListComponent />
...
You also must create a renderer for a field type `target` when not set to allow multiple values.

### Create a plugin of type `CmsModelFieldToGraphQLPlugin`

Plugins of this type define GraphQL part for a given field type in the API side of the application.
A simple example of the plugin:

```tsx
export default (): CmsModelFieldToGraphQLPlugin => ({
  type: "cms-model-field-to-graphql",
  name: "cms-model-field-to-graphql-target",
  fieldType: "target",
  isSortable: false,
  isSearchable: false,
  manage: {
...
Note that there can be differences in management and read definitions. That is because of a case that administration is working with data directly and the user on the read side expects a resolved target.

When you created `CmsEditorFieldTypePlugin`, `CmsEditorFieldRendererPlugin` and `CmsModelFieldToGraphQLPlugin` type plugins, your field is ready to be used. Just import them in the application.

### Create a plugin of type `CmsModelFieldToElasticSearchPlugin`
...
export default (): CmsModelFieldToElasticSearchPlugin => ({
  ...
  name: "cms-model-field-to-elastic-search-target",
  fieldType: "target",
...
### Create a plugin of type `CmsModelFieldToStoragePlugin`
...
export default (): CmsModelFieldToStoragePlugin => ({
  ...
  name: "cms-model-field-to-storage-target",
  fieldType: "target",
