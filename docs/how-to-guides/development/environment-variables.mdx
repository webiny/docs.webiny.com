...
As mentioned, the reason for the obscurity is primarily security, as we want to avoid potential leak of sensitive information to the public.
...
But, for development purposes, setting the `DEBUG` environment variable to `true`, will force the backend to return something like the following:
