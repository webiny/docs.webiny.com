...
```js title="apps/website/code/webiny.config.ts" {11,15,26,30}
(...)
...
If set to `true`, all errors detected in the application runtime will be presented in a clear error report, with all the relevant information like error message, and a complete stack trace:
...
#### Do I have to always `REACT_APP_` prefix when defining frontend environment variables?

Yes, you must create custom environment variables beginning with `REACT_APP_` to avoid accidentally exposing a private key on the machine that could have the same name.