...
For frontend applications, environment variables need to be handled in their respective `start` and `build` commands. In a Webiny project, these can be found in the `webiny.config.js` file, which is the essential configuration file of every package that a project application may hold.

:::info Project Applications
Learn more about project applications and project organization in general, in the [Project Applications and Packages](/docs/key-topics/project-organization/project-applications-and-packages) key topic.
:::

The following code shows the `webiny.config.js` file of the `website` package, which is the one and only package of the **Website** project application:

```js title="apps/website/code/webiny.config.js" {11,15,26,30}
(...)
...
module.exports = {
    commands: {
        async start(options, context) {
            ...
            process.env.MY_CUSTOM_ENV_VARIABLE = 'some-random-value';
...
To improve the debugging experience, we have the `DEBUG` environment variable in place.
It enables two features, which we'll cover in detail shortly:
1. Detail error response in case of server error
2. Visual feedback in case of a network request fails

### Detail Error Response
By default, in case of server error, the API responds with an _Internal server error_ with status code `500`, for example:
...
It is to ensure that unnecessary internal details don't pass to the end-user.

Although in development, you might want to know the details about the error for a better debugging experience. For example, the error message, code, and even the stack trace.

Setting `DEBUG=true` helps you achieve exactly that, for example:
...
:::note
You must redeploy the API after updating the environment variables. Otherwise, the change will not reflect.
:::

### Visual Feedback
With the above change in place, you have the error with details that we can inspect in the network tab of the browser's dev tools.
That's nice, but, to further improve the development experience, you can set the `REACT_APP_DEBUG=true`.

It will render an overlay in case of any network error so that you have all the details about the error without inspecting the network tab:
...
We set it by default when you [start](https://github.com/webiny/webiny-js/blob/next/packages/project-utils/bundling/app/index.js#L33) the Admin app locally.
But, you can always do the same for other environments like `dev` for example.
...
The following commands load the `.env` file: [`deploy`](/docs/how-to-guides/deployment/deploy-your-project#the-deploy-command), [`destroy`](/docs/how-to-guides/deployment/destroy-cloud-infrastructure#the-destroy-command), [`watch`](/docs/how-to-guides/webiny-cli/use-watch-command), and [`pulumi`](/docs/how-to-guides/deployment/execute-pulumi-commands).
