...
title: Use Watch Command
sidebar_label: Use Watch Command
...
If the above option is not enough, you can also pick the exact packages you want to watch, using the `--scope` argument. The following command will watch for code changes in the `api-graphql` and our custom `api-car-manufacturers` packages:
...
yarn webiny watch --env dev --scope api-graphql --scope api-car-manufacturers
```

The `--scope` argument can also take a [`minimatch`](https://www.npmjs.com/package/minimatch) pattern, allowing you to specify multiple packages to watch, with a single `--scope` argument. For example, the following command will watch all packages whose name starts with `api-`:
...
yarn webiny watch --env dev --scope "api-*"
```
...
yarn webiny watch --env dev --scope api-headless-cms --scope api-cms-custom-field
```
...
That's why, by appending the `--logs` argument, you can have all of your `console.log` calls stream logged information directly to your terminal. For example:
...
To start watching one or more packages, simply omit the project application related arguments, and provide package names, via the already seen `--scope` argument. For example:

```bash
# Starts watching three packages.
yarn webiny watch --scope "api-car-manufacturers" --scope "api-headless-cms" --scope "api-cms-custom-field"
```
