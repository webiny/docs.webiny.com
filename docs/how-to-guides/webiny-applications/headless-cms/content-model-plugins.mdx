...
In this example, we show how we can define a simple **Product** content model, that belongs to the **E-Commerce** content model group. Both plugins can be define in a single file, in our case, [`api/code/headlessCMS/src/plugins/models.ts`](#):
...
In this example, we show how we can content models and content model groups only for a specific locale, in our case, the `en-US`.
...
There is a difference in how these two IDs are used internally within Headless CMS' application code, but this is more important when a content model is defined regularly, via the [Content Model Editor](http://localhost:3000/docs/how-to-guides/webiny-applications/headless-cms/content-model-plugins#via-the-admin-area). In case of content models defined via plugins, we can simply use the same value for both fields.
