...
The easiest way is to use the built-in `FileManagerFileTypePlugin` class. Create the plugin inside of the
/apps/admin/code/src/plugins/ folder, and register it in the apps/admin/code/src/plugins/base.ts entrypoint file. Let's configure a plugin that will render files of type `video/mp4`:
