...
We performed 4 variations of this test. The reason is that Webiny uses Elasticsearch, which is the only non-serverless infrastructure piece that's part of the architecture. Being non-serverless means we need to manually scale it. So we decided to run 4 different test variations. In each of variations we changed the load amount and the instance type.
