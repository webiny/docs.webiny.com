...
1. Call the Headless CMS manage API and insert new data ([view results](./pb-deliver-a-page))
2. Call the Headless CMS preview API and retrieve data ([view results](./))
3. Deliver a page created by the Page Builder ([view results](./))

## Execution

We deployed Webiny into an AWS region, in our case `eu-west-2` (London). Then we deployed an EC2 machine in the same region to minimize the network latency. The machine instance type we used was `c5.2xlarge`. We used a fairly powerful instance as we needed to generate a lot of load for some of the tests and didn't want to be CPU or memory bound.
