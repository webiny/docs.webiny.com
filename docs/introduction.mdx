...
Welcome! 👋 **Webiny** is a JavaScript serverless application framework with a built-in CMS. We created Webiny specifically to help developers architect, build and deploy solutions on top of the serverless infrastructure, like AWS Lambda, DynamoDB, S3, and others. (multi-cloud support coming)

We believe that serverless is the future of development, but having tried ourselves building a solution by using only serverless services we saw this is a challenging task. There are many tools that help you deploy and monitor an app on top of the serverless infrastructure. When it comes to building one, the existing tools are very limited and don't handle many use-cases.
...
Webiny is build using a modern development toolchain that makes the experience for developers much more enjoyable. Technologies Webiny uses:
...
At the moment Webiny only runs on the AWS cloud. It you wish to help us add support for another cloud provider, please see [this issue](https://github.com/webiny/webiny-js/issues/1150).
:::
