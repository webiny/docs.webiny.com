...
**Webiny** is a JavaScript serverless application framework with a built-in CMS. We created Webiny specifically to help developers architect, build and deploy solutions on top of serverless infrastructure like AWS Lambda, DynamoDB, S3, and others. Support for other cloud vendors besides AWS is coming.

We believe that serverless is the future of development. There are many tools that help you deploy and monitor an app on top of serverless infrastructure. Those tools do that well. However, when it comes to architecting and coding an application, your set of options is very limited. This is where Webiny comes in.
...
This is the core of Webiny. It's a framework created specifically for building serverless solutions. The framework handles the deployment workflow, code bundling, project organization, and provides a ready-made architecture. It also includes an access control list (ACL) module, GraphQL API, and an Admin Panel single-page application (SPA).
...
Webiny at its core doesn't do much. Over 90% of Webiny's code is actually inside plugins. From how the Webiny CLI deploys your build, to how the API checks the user permissions, to the Webiny logo inside the Admin panel, those are all plugins. This approach allowed us to build a platform that is flexible in every meaning of the word. You can completely replace existing plugins, turn them on or off or expand on them, customizing Webiny to fully fit your needs. An added benefit of this approach is that your plugins live outside the Webiny's core codebase, making upgrades much easier.

Another great thing about the plugin system is that it is unified across the board. It doesn't matter if you are changing the CLI functionality, adding new GraphQL resolvers, or a new button in the UI, the approach is always the same. This makes it easy for anyone to pickup and understand how the whole system works.

Webiny believes in having a great developer experience. From installing your project to deploying it and seeing it <i>Live</i>, all that's needed are 2 commands. Webiny handles all the heavy lifting under the hood, but in a transparent way, not hiding anything, or making it hard to understand. Less abstraction means better understanding of the system.
...
Our project and documentation is constantly improving and changing. If there is anything important, we post it to our [blog](https://www.webiny.com/blog) and our [newsletter](https://landing.mailerlite.com/webforms/landing/g9f1i1).
