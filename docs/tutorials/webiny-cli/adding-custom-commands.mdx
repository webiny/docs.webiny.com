...
sidebar_label: Adding Custom Commands
...
For this tutorial, we're going to create a new plugin directly in the `webiny.root.js` file, located in your project root folder.
...
// webiny.root.js
module.exports = {
  template: "@webiny/cwp-template-aws@5.0.0-beta.2",
  projectName: "homed",
  cli: {
    plugins: [
...
Inserting new plugins directly into the `webiny.root.js` works, but overtime, may become crowded. Feel free to move this code into a separate file, for example `scripts/drinkCommand.js`, and use the `webiny.root.js` just to call the necessary `require` statements.
:::
