...
Because all page elements need to share code (a plugin) between the Admin Area application `(apps/admin)` and the Website application `(apps/website)`, it becomes necessary to put that code in some shared folder.
...
In this tutorial, we will create a folder named `pb-element-iframe` in `apps/extensions` which, will contain all the code for plugins.
:::note
Please feel free to create a folder named `extensions` inside `apps` if it doesn't exist already.
:::
...
1. Create a folder named `pb-element-iframe` inside `apps/extensions`.
2. Create a `package.json` file inside `apps/extensions/pb-element-iframe` with the following contents:
...
4. Create a `tsconfig.json` file with the following contents:
```json
{
  "extends": "../../../tsconfig",
  "include": ["."],
...
5. Create `admin` and `render` folders inside `apps/extensions/pb-element-iframe`.

6. Link workspaces by running `yarn install` command from project root.

:::info
Look in `node_modules` there should be a [symbolic link](https://en.wikipedia.org/wiki/Symbolic_link) of `@extensions/pb-element-iframe`.
:::

7. Add `@extensions/pb-element-iframe` package as a dependency to both the Admin Area application and Website application,
...
Now that we've created plugins for `iframe` element. We need to register them in the Admin Area application to work.
...
In order to render the page element on the Website application, we will use the [`pb-render-page-element`](/docs/references/page-builder/plugins#pb-render-page-element) plugin again.
...
Make the following changes in [`apps/website/code/src/plugins/pageBuilder.ts`](https://github.com/webiny/webiny-examples/tree/master/iframe-page-element/apps/website/code/src/plugins/pageBuilder.ts) file:
