...
We can simply copy that code over to the app we need it in but that will violate DRY rule.
So, for a clean solution we'll going to create a package named `iFrameElement` inside `packages` folder.
And simply import and reuse the code(plugin) across apps.

:::info
Learn more about creating and organizing a package in monorepo here.
:::
...
The settings UI will be shown automatically in RHS sidebar when the user drags and drops the page element on the page.
