...
All Page Builder elements are plugins which means we can easily modify, override or even add new ones.
<!-- TODO: Add an info section about What are plugins? And how we use them in Webiny? -->
Let's see how we can add two predefined settings which are `padding` and `width` to the `button` element.

## Plugin Type

There's only one plugin you need to modify to customise an element: [`PbEditorPageElementPlugin`](/docs/references/page-builder/plugins#pb-editor-page-element).
...
All element plugins are registered as factory so you can customize them by passing the following options:
...
Head over to `apps/admin/code/src/plugins/pageBuilder.ts` file where all the plugins related to the Page Builder app are located.
...
Now if you drop a new `button` element to the editor it will look much better as show below:
