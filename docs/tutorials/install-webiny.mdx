...
As noted, this option should be used if you intend to have a small to medium size project: up to a few hundred records, per application, in the database.
:::note
Because the DynamoDB does not provide advanced filtering or sorting we had to implement our own.
:::

Basically, we load the smallest amount of records possible from the DynamoDB and then perform filtering and sorting in the code.
...
Also note that Headless CMS is quite specific application where entries are split through the models. So if you have 5 models with 50 entries each we will, at most, load 50 entries.


### DynamoDB + Elasticsearch (for Larger Projects)
...
DynamoDB + Elasticsearch project has exactly the same functionalities (filtering and sorting) as the DynamoDB one.
When having a lot of records, it will be faster to search and sort in project deployed with Elasticsearch.
