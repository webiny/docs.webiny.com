...
Every identity (for example, a logged-in administrator) belongs to **exactly one** security group. But, note that a single security group can have multiple security permissions objects assigned to it, allowing or preventing access across multiple Webiny and your custom applications.
...
By adding a new permissions section in the security group form that we've seen above, we can allow logged-in administrators to manually toggle access for our own custom features, for all identities that are linked with the particular security group.

Continuing with the car manufacturers example that we've used in previous tutorials, we could add a new Car Manufacturers section, and offer a couple of different options to the logged-in administrator:
...
- the code uses a custom [`directions_car-24px.svg`](https://github.com/webiny/webiny-examples/tree/master/extend-admin-area/securing-your-application/code/packages/car-manufacturers/admin-app/src/assets) SVG icon which you'll also want to copy into your project (otherwise the build will fail)
...
By completing this step, we've added the new Car Manufacturers permissions section in the security group form. Logged-in administrators can now create new (or extend existing) security groups, and give access to our new Car Manufacturers module. For example, one security group can allow only reading car manufacturers data, while the other one may allow all three primary actions - read, write, and delete, and even allow access to the imaginary special feature.
