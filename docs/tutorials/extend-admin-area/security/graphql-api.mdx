...
In order to secure the `carManufacturers` GraphQL API queries and mutations, we have to revisit the GraphQL resolver functions that were generated for us via the [Extend Admin Area](/docs/how-to-guides/webiny-cli/scaffolding/extend-admin-area) scaffold.
