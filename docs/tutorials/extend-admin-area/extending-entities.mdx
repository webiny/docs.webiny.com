...
Initially, the `CarManufacturer` entity that was created in the scaffolding process only contains two attributes: `title` and `description`. Of course, these are just starter attributes and we are certainly encouraged to extend the entity with additional ones.

For example, let's see how we can add the new `isPopular` boolean attribute to the initial `CarManufacturer` entity, and then, enable logged-in administrators to modify its value, by adding a simple [`Switch`](https://storybook.webiny.com/?path=/story/components-switch--usage) form element in the generated form.
...
Finally, let's update our GraphQL `CarManufacturer`-related GraphQL types in our schema, located in the [`typeDefs.ts`](https://github.com/webiny/webiny-examples/blob/master/blog/webiny-v5.9.0-improved-scaffolds-and-development/api/code/graphql/src/plugins/scaffolds/graphql/carManufacturers/typeDefs.ts) file:
...
:::tip
If you've run the two [`webiny watch`](https://www.webiny.com/docs/how-to-guides/webiny-cli/use-watch-command/) commands mentioned in the [previous section](/docs/tutorials/extend-admin-area/scaffolding#using-the-watch-command), the performed application code changes should be automatically rebuilt and redeployed into the cloud.
:::
...
Once we've updated the `CarManufacturersForm` React component, the form itself should look like the following:
...
Adding the `isPopular` field can be easily done by updating the [`CAR_MANUFACTURER_FIELDS_FRAGMENT`](#) fragment, located in the `graphql.ts` file:
