...
Let's see how we can add the new `isPopular` boolean attribute to the [`CarManufacturer`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/api/code/graphql/src/plugins/scaffolds/carManufacturers/entities/CarManufacturer.ts) entity, and then, via the new Admin Area application module, enable logged-in administrators to modify its value, by adding a simple [`Switch`](https://storybook.webiny.com/?path=/story/components-switch--usage) form field to the Car Manufacturers form.
...
Once we've done all of the above shown changes, we can switch to our Admin Area application, and continue by updating the form.

## Admin Area Application

In the [`CarManufacturersForm`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/apps/admin/code/src/plugins/scaffolds/carManufacturers/views/CarManufacturersForm.tsx) form, let's add a new [`Switch`](https://storybook.webiny.com/?path=/story/components-switch--usage) form element, which will enable logged-in administrators to mark a particular car manufacturer as popular or not popular (parts of code removed for brevity):
