...
- in the Admin Area, how to extend the initially created form with new fields
...
For example, let's see how we can add the new `isPopular` boolean attribute to the initial [`CarManufacturer`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/api/code/graphql/src/plugins/scaffolds/carManufacturers/entities/CarManufacturer.ts) entity, and then, enable logged-in administrators to modify its value, by adding a simple [`Switch`](https://storybook.webiny.com/?path=/story/components-switch--usage) form element into the generated form.
...
```ts title="api/code/graphql/src/plugins/scaffolds/carManufacturers/entities/CarManufacturer.ts" {14}
(...)
...
:::tip
If you've run the two [`webiny watch`](/docs/how-to-guides/webiny-cli/use-watch-command/) commands, mentioned in the [previous section](/docs/tutorials/extend-admin-area/scaffolding#using-the-watch-command), the performed application code changes should be automatically rebuilt and redeployed into the cloud.
:::
...
In the [`CarManufacturersForm`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/apps/admin/code/src/plugins/scaffolds/carManufacturers/views/CarManufacturersForm.tsx) form, let's add a new [`Switch`](https://storybook.webiny.com/?path=/story/components-switch--usage) form element, which will enable logged-in users to mark a particular car manufacturer as popular or not popular (parts of code removed for brevity):
...
Finally, we need to add the `isPopular` field to all relevant GraphQL query and mutation operations. This ensures that we're both sending and receiving the `isPopulate` value while interacting with the GraphQL API.

Adding the `isPopular` field can be easily done by updating the [`CAR_MANUFACTURER_FIELDS_FRAGMENT`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/apps/admin/code/src/plugins/scaffolds/carManufacturers/views/hooks/graphql.ts#L3) fragment, located in the [`graphql.ts`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/apps/admin/code/src/plugins/scaffolds/carManufacturers/views/hooks/graphql.ts#L3) file:
...
Because this is not something we can effectively predict, the generated application code does not include any authentication and authorization logic. But luckily, with a couple of [built-in utilities](/docs/tutorials/create-an-application/security/introduction), this is not too hard to implement.
