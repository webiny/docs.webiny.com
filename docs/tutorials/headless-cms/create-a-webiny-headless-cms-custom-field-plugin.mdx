...
`CmsModelFieldToStoragePlugin` is used to manipulate the data before passing it to the storage layer, and also to modify data while retrieving it. In our case, we will _encrypt_ the data before storing it, and _decrypt_ it after retrieval.
...
As you create the entry, you will see your data is encrypted, because, as per our current code, we encrypt our data before storing it, but we're not decrypting it back after retrieving from the database.
...
With this change, after retrieving the data, it will be decrypted.
