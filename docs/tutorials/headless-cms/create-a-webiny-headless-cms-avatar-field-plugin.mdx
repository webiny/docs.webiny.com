...
### Overview
A detailed and more in-depth explanation and how-to for a Webiny Headless CMS field plugins. The tutorial example is the idea you need to search for some address and get it's coordinates.
In this tutorial we assume that you know how Webiny plugins work.

For quick info on required plugins check our [How-to Guide: Create a Webiny Headless CMS field plugin](https://www.notion.so/webiny/How-to-Guide-Create-a-Webiny-Headless-CMS-field-plugin-cbe2bc97f19c4909a32d5032e0a9c143).

### Narrative
You need a field that gives you a functionality to search for an address via Google Maps API. When you select the address information (for example: coordinates) are set also set in the data. Also, you do not want to index that field and you want to encrypt it before saving into the storage.
...
First we need to create a field definition plugin. We can start with the UI one. The base for the plugin is:
...
```
Note that renderer name is left blank so code automatically determines which renderer to use. You can put the name of the renderer, but for our tutorial we leave it blank to use that for later.
...
render({ field, getBind }) {
    const Bind = getBind();
    const onSelect = ({coordinates, ...address}) => {
        bind.onChange({
...
```typescript title="[API]/addressFieldPlugin.ts"
read: {
    createSchema() {
        return {
            typeDefs: `
...
```typescript title="[API]/addressFieldIndexPlugin.ts"
...
```typescript title="[API]/addressFieldStoragePlugin.ts"
