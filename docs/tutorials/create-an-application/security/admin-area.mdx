...
:::info
The code that we will cover in this tutorial can also be found in our [GitHub examples repository](https://github.com/webiny/webiny-examples/tree/master/extend-admin-area/security/packages/car-manufacturers/admin-app).
:::
...
The code for these menu items is located in the scaffolded [`packages/car-manufacturers/admin-app/src/menus.tsx`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/security/packages/car-manufacturers/admin-app/src/menus.tsx) file. So, in there, we can add the following code:

```tsx title="packages/car-manufacturers/admin-app/src/menus.tsx" {4,6,7,9,10,16-41}
...
:::tip
If you're curious about the `CarManufacturersPermission` type, check its definition in [packages/car-manufacturers/admin-app/src/types.ts:24](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/security/packages/car-manufacturers/admin-app/src/types.ts#L24).
:::
...
To prevent unauthorized users from accessing the `/car-manufacturers` URL path (route), let's jump to the [`packages/car-manufacturers/admin-app/src/routes.tsx`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/security/packages/car-manufacturers/admin-app/src/routes.tsx) file, in which we're going to wrap our all of our child React components with the `SecureRoute` component:
...
Let's see how we can prevent rendering of React components, in case the logged in user is not authorized to see them. For example, let's hide the New Car Manufacturer button, for users that aren't allowed to create or update existing car manufacturers (in our `car-manufacturers` permission object, this is determined via the `rwd` property).

<CenteredImage alt='Hiding the "New Car Manufacturer Button"' src={hidingNewButton} />

To do that, let's jump to the `CarManufacturersDataList` React component ([`packages/car-manufacturers/admin-app/src/views/CarManufacturersDataList.tsx`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/security/packages/car-manufacturers/admin-app/src/views/CarManufacturersDataList.tsx)), in which we're going to wrap the `ButtonSecondary` component with the `SecureView` component. The following code shows how we can do that (for brevity, some of the code was excluded):
...
Finally, note that there are still a couple of places in which we would want to perform authorization checks. Also, in the same way as in the previous [GraphQL API section](/docs/tutorials/create-an-application/security/graphql-api), in case you start seeing yourself copying some of the authorization related code, it's certainly recommended that you extract it into one or more separate utility functions. This way we're not repeating our selves (DRY and our code will be more maintainable and less error-prone.
