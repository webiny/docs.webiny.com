...
All of the GraphQL resolver functions are located in the [`packages/car-manufacturers/api/src/resolvers`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/security/packages/car-manufacturers/api/src/resolvers) folder:
...
For purposes of this tutorial, we can open the simplest resolver, which is the [`getCarManufacturer.ts`](https://github.com/webiny/webiny-examples/blob/master/extend-admin-area/security/packages/car-manufacturers/api/src/resolvers/getCarManufacturer.ts), and add the following lines of code:
...
const getCarManufacturer = async (
    ...
    // within the detected locale. An error is thrown if access is not allowed.
...
:::info
Learn more about the API Playground GraphQL client in the [API Playground](/docs/how-to-guides/admin-area/api-playground) guide.
:::
...
Before we wrap this up, note that this is just a single resolver we secured, and that you should implement the same logic into [others](https://github.com/webiny/webiny-examples/tree/master/extend-admin-area/security/packages/car-manufacturers/api/src/resolvers) as well.
