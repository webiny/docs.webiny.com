...
- [`package.json`](https://github.com/webiny/webiny-examples/blob/master/iframe-page-element/packages/iFrameElement/package.json): This file is used to give information to [npm](http://npmjs.com/) that allows it to identify the package as well as handle the package's dependencies.
- [`.babelrc.js`](https://github.com/webiny/webiny-examples/blob/master/iframe-page-element/packages/iFrameElement/.babelrc.js): A configuration file for babel.
- [`tsconfig.json`](https://github.com/webiny/webiny-examples/blob/master/iframe-page-element/packages/iFrameElement/tsconfig.json):  A configuration file of the TypeScript compiler (tsc).
- [`tsconfig.build.json`](https://github.com/webiny/webiny-examples/blob/master/iframe-page-element/packages/iFrameElement/tsconfig.build.json)  A configuration file of the TypeScript compiler (tsc).
...
it removes the content of `dist` folder and compile the source code via `babel` and run the `postbuild` command.
- [`watch`](https://github.com/webiny/webiny-examples/blob/master/iframe-page-element/packages/iFrameElement/package.json#L38):
it run the `babel` compiler in `watch` mode, which means the latest changes will compile automatically as source file content changes.
...
as name suggest, it runs after the completion of the `build` command.
We use it to copy the meta files like `package.json`, `README.md` into `dist` folder and compile typescript code.
...
We can simply `cd` into the package folder that is `packages/greeting` and run:
