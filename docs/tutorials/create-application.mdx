...
Default value is `packages/car-manufacturers/api`. Notice that data model name `CarManufacturer` is in plural and kebab-cased.
...
Default value is `@car-manufacturers/api`.
...
Now that you have functioning API, let's move onto the UI side.

## Admin area package scaffold
### Running the scaffold
Run the scaffold command and go follow the questions. The questions are as the ones in the API package scaffold, functionality behind them as well.

### The code
Generated code contains:
1. `src/views/graphql.ts` with GraphQL queries and mutations for the data model.
2. `src/views/CarManufacturers.tsx` with a React component that renders both list view and the form.
...
```ts title="apps/admin/code/src/plugins/index.ts"
// at the top of the file
...
And now you have the functioning UI in the admin area to create, update, delete and list the CarManufacturers.
