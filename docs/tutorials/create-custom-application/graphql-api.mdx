...
import extendGqlApiScaffold from "./extending-graphql-api/extend-gql-api-scaffold.png";
import extendGqlApiScaffoldOutput from "./extending-graphql-api/extend-gql-api-scaffold-output.png";
import createPinMutation from "./extending-graphql-api/create-pin-mutation-test.png";
import coverImagePreview from "./extending-graphql-api/cover-image-preview.png";
...
For this tutorial, we'll go with the second approach. So, in your terminal of choice, run the `webiny scaffold` and pick the **Extend GraphQL API** from the list of all available scaffolds.
...
As the answer to the first question, we can simply use the [`pinterest-clone/api/code/graphql/src/plugins`](https://github.com/webiny/webiny-examples/tree/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins) path, which should be automatically prefilled for us. As for the name of the initial entity, we'll enter **Pin**.
...
- mutations:
  - [`createPin(data: PinCreateInput!): Pin!`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts#L53)
  - [`updatePin(id: ID!, data: PinUpdateInput!): Pin!`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts#L56)
  - [`deletePin(id: ID!): Pin!`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts#L59)
- queries:
  - [`listPins(limit: Int, before: String, after: String, sort: PinsListSort): PinsList!`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts#L48)
  - [`getPin(id: ID!): Pin`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts#L47)

Notice how all of these ultimately return the [`Pin`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts#L2) type, which is defined like the following:
...
This is great because, as we can see, the type already contains two out of three fields we wanted our central **Pin** entity to have. The only field that's missing here is the `coverImage`. Don't worry, we'll see how we can add this field soon.
...
Let's say we wanted to quickly test all of the newly created GraphQL query and mutation operations. The easiest way to do that would be to use a GraphQL client, like for example the [GraphQL Playground](https://www.electronjs.org/apps/graphql-playground). With it, we should be able to run the following `CreatePin` mutation:
...
Running the `yarn webiny output pinterest-clone/api --env dev` command from your project root will display all of the values that were exported from your cloud infrastructure code. That includes GraphQL API URL, Amazon DynamoDB table name, AWS region, and more.
:::
...
1. add the `coverImage` attribute to the `Pin` entity
2. add the `coverImage` field to the `Pin` GraphQL type

So, for starters, let's open our [`Pin`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/entities/Pin.ts#L10) entity and simply define the new attribute on it (parts of code removed for brevity):

```ts title="pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/entities/Pin.ts" {13}
(...)
...
Finally, let's update our `Pin` GraphQL types in our schema, located in the [`typeDefs.ts`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/03-extending-graphql-api/pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts) file:

```ts title="pinterest-clone/api/code/graphql/src/plugins/scaffolds/pins/typeDefs.ts" {6,15,21}
...
Now that we've defined our fundamental `Pin` GraphQL type and the related set of CRUD GraphQL queries and mutations, we can switch to frontend, and start building our React application.
