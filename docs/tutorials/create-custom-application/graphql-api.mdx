...
:::tip
In order to access our GraphQL API, we need to have its public URL. This is where the [`webiny output`](/docs/key-topics/webiny-cli#yarn-webiny-output---env-env) command can help.

Running the `yarn webiny output pinterest-clone/api --env dev` command from your project root will display all of the values that were exported from your cloud infrastructure code (also known as **stack output**). This includes information like GraphQL API URL, Amazon DynamoDB table name, AWS region, and more.
:::
