...
:::tip
Want to learn more about serverless? Check out our [Serverless Guide](https://www.webiny.com/serverless-knowledge-base/serverless)!
:::
...
These are some of the reasons why Webiny advocates developing in the cloud and not locally, within the developer's machine. As we'll see, as soon as our application starts relying on additional cloud infrastructure resources, we'll need to deploy them.

The same goes when it comes to writing integration and end-to-end (E2E) tests, which are being executed against real services deployed into our cloud (AWS) account. No local emulations or running services are being used.
...
In this tutorial, we'll get to chance to work with both types of code, so you'll be able to get a better feel of how it all works together.
...
With that in mind, the point we're trying to highlight here is: as a developer, you don't have to be _scared_ or try to avoid understanding how the application works on the cloud infrastructure level. Application code and cloud infrastructure are equally important in delivering the application you're building.

Sooner or later, you'll be faced with a task that will require you to introduce additional cloud infrastructure resource(s) into the mix. Sure, it's definitely not required of developers to know everything about every available cloud infrastructure resource out there, but, having at least the basic knowledge of how our application works on the cloud infrastructure level definitely can't hurt.
...
Furthermore, on top of the consumption-based pricing, today, different cloud infrastructure resources often include a solid free-tier. In other words, completely free consumption until a certain threshold has been reached. This makes the whole [developing in the cloud](/docs/tutorials/create-custom-application/serverless-application-development#developing-in-the-cloud) proposition an even more attractive option.
