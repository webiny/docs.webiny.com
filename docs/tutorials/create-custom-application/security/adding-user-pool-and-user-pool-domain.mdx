...
```ts title="pinterest-clone/api/pulumi/dev/cognito.ts" {12,39}
...
<CenteredImage alt="Automatic Re-deployment of Cloud Infrastructure" style={{ maxWidth: 750 }} src={cloudInfraAutoDeploy} />

If not, you can either start a new watch session, or simply deploy the changes via the [`webiny deploy`](#) command. Both approaches will ensure the new cloud infrastructure resources are deployed and are ready to be used.
