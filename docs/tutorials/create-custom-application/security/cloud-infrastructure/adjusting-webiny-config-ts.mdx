...
:::note
Changes made to the [`webiny.config.ts`](#) file aren't automatically picked up by already active watch sessions, initialized with the [`webiny watch`](/docs/how-to-guides/use-watch-command) command. Before you start making any changes, make sure to terminate any active watch sessions you might have and run them again once the [`webiny.config.ts`](#) file is in the right place.
:::
...
Essentially, the [`webiny deploy`](/docs/how-to-guides/deployment/deploy-your-project) command performs two steps:
...
This means that upon deploying our React application for the first time, the [`getStackOutput`](#) function call, that we've added in the [`build`](#) command in our [`webiny.config.js`](#) file, won't return any results. No deployed cloud infrastructure means no stack output to read from.

So, when doing the first deployment, we recommend you first deploy your cloud infrastructure resources only, and then proceed by doing another deployment, this time with our application code. We can achieve this with the following command:
