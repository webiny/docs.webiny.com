...
By itself, the above library doesn't do much. It's simply a framework, which is always paired with an authentication provider. In our case, we want to pair it up with the Amazon Cognito provider, which is what this library does.
...
Optionally, the `getIdentity` callback function enables us to return a custom [`SecurityIdentity`](https://github.com/webiny/webiny-js/blob/next/packages/api-security/src/SecurityIdentity.ts#L8) instance once the identity has been successfully authenticated. This is helpful when we want to define additional properties on it, or maybe decide to assign a different value to the required ones - `id`, `type`, and `displayName`.

In our case, the reason we went with a custom `getIdentity` callback function is because, by default, the `id` property of the [`SecurityIdentity`](https://github.com/webiny/webiny-js/blob/next/packages/api-security/src/SecurityIdentity.ts#L8) instance will be mapped to signed in user's e-mail address, which is not what we want. Be relying on the `cognito:username` property of the received and decoded ID (JWT) token (notice the `token` named parameter), we're relying on an ID that was generated on the Amazon Cognito side.

:::info
The [`SecurityIdentity`](https://github.com/webiny/webiny-js/blob/next/packages/api-security/src/SecurityIdentity.ts#L8) instance that we return from the `setIdentity` callback function is what we'll be returned to us while performing authentication down the road, for example within our GraphQL resolver functions. More on this in the following section of this tutorial.
:::
