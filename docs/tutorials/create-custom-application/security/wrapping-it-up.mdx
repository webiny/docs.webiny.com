...
With the all of the GraphQL API changes in place, let's wrap up the security section of this tutorial by returning back to our React application and making one small addition to the **Pin Details** page, which currently looks like this:
...
```tsx title="pinterest-clone/app/code/src/plugins/routes/pinDetails.tsx" {19-21,51-52}
...
As we can see, we didn't do anything too complex in terms of the actual visual layout. But, nevertheless, we're now finally able to see who created the particular pin we're looking at.
