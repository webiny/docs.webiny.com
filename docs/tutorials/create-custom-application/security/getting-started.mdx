...
As mentioned in the previous section, in order to perform authentication in our application, we first need to have an **authentication flow**. In other words, we need to provide a way for users to sign up and sing in to our application.
...
With this approach, the authentication flow is completely offloaded to Amazon Cognito. Once a user wishes to sign up or sing in to our application, she/he usually clicks on a button or a link in it, which causes a redirect to the standalone hosted authentication flow. So, as mentioned, the authentication flow is happening completely outside of our application.
...
It's useful to know that every package in a Webiny project possesses a `webiny.config.ts` configuration file. For example, if we were to open our GraphQL API's [`webiny.config.ts`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/05-security/02-getting-started/pinterest-clone/api/code/graphql/webiny.config.ts) file, located in the [`pinterest-clone/api/code/graphql`](pinterest-clone/api/code/graphql) folder, we'd see the following:

```ts title="pinterest-clone/api/code/graphql/webiny.config.ts" {6,7}
...
```ts title="pinterest-clone/app/code/webiny.config.ts" {22,36}
