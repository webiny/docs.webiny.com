...
- create the [`Authentication`](#) React component, which will be responsible for performing the initial authentication process (every time a user starts using our React application)
- modify our [`App`](#) entrypoint React component, by rendering the [`SecurityProvider`](#) and the just created [`Authentication`](#) component
