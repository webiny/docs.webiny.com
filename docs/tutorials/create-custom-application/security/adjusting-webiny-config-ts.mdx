...
export default {
    commands: {
        async watch(options, context) {
            ...
            Object.assign(process.env, getStackOutput("pinterest-clone/api", options.env, MAP));

            // Let's also the user pool client ID and pass it via environment variables too.
            Object.assign(process.env, getStackOutput("pinterest-clone/app", options.env, APP_MAP));
...
First, we've updated the existing `MAP` object, which is used upon retrieving our GraphQL API's stack output:

```ts
getStackOutput("pinterest-clone/api", options.env, MAP);
```
...
So, we've added three new properties to the `MAP` object: `REACT_APP_USER_POOL_REGION`, `REACT_APP_USER_POOL_ID`, and `REACT_APP_USER_POOL_DOMAIN`, which will represent the `region`, `cognitoUserPoolId`, and `cognitoUserPoolDomain` properties included in the retrieved GraphQL API's stack output.
...
```ts
getStackOutput("pinterest-clone/app", options.env, APP_MAP);
```

## RESTART SESSION TODO
