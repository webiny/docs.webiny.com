...
import importTMApi from "./multi-tenancy/import-tenant-manager-api.png";
import importTMReact from "./multi-tenancy/import-tenant-manager-react.png";
import lambdaDelete from "./multi-tenancy/lambda-delete-error.png";
import newTenantForm from "./multi-tenancy/new-tenant.png";
import rootTenantLabel from "./multi-tenancy/root-tenant-settings.png";
import rootTenantDialog from "./multi-tenancy/tenant-settings-dialog.png";
...
CloudFront will be used for TLS termination, and ACM for certificate management. Customers may use other 3rd party content delivery networks (CDNs) for those steps. If that’s the case, please get in touch so we can support you through the setup.
...
You can't have the same fully qualified domain name (FQDN) configured in more than 1 CF distribution under the same account. If you do, then you will see the following error.
