...
import cloudfront from "./setup/cloudfront-distribution.png";
...
If you're upgrading your existing `5.18.x` project, we need to upgrade the `npm` packages to the experimental release, which contains the multi-tenancy functionality. To do that, execute the following command in your terminal:

```shell
yarn up "@webiny/*@0.0.0-mt-3"
```

Alternatively, you can create a new project using the experimental release, by running:

```shell
npx create-webiny-project@0.0.0-mt-3 my-project --tag=0.0.0-mt-3
```
...
```shell
yarn workspace api-graphql add @webiny/api-tenant-manager@0.0.0-mt-3
```

Add Tenant Manager module dependency to the Admin app dependencies:

```shell
yarn workspace admin add @webiny/app-tenant-manager@0.0.0-mt-3
```

Add a package with custom AWS components for Pulumi to the root package.json:

```shell
yarn add @webiny/pulumi-aws@0.0.0-mt-3
```
...
Once the domains in the certificate are verified, we can proceed with Pulumi configuration.
...
This one is usually resolved by refreshing the Pulumi state:

```shell
 yarn webiny pulumi apps/website -- refresh --yes
````
