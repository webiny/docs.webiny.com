...
To learn more about creating content models via code, check out the [Define Content Models via Code](/docs/{version}/headless-cms/extending/content-models-via-code) guide.
...
From time to time, users may need to re-render a particular page they've previously created and published. This can now be done easily via the built-in [API Playground](/docs/{version}/admin-area/basics/api-playground), by executing a simple `rerenderPage` GraphQL mutation. For example:
...
For more details on all of the available events, please see the dedicated [Lifecycle Events](/docs/{version}/page-builder/references/lifecycle-events) guide.
...
In a couple of scenarios, upon running the [`webiny watch`](/docs/{version}/core-development-concepts/basics/watch-command) command, adding the `--remoteRuntimeLogs` (`-r`) argument would not properly return runtime logs in user's terminal. This has now been addressed.
