...
Because of what we stored into the database prior to *5.19.0* searching for all `Articles` with any `Category` version of *My Category* is not possible with previous query.

There are workarounds for that, and you can read about them [here](/docs/release-notes/5.19.0/ref-field-workarounds).
