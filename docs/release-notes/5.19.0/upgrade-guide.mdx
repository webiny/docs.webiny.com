...
From the 5.19.0 version of Webiny, Admin Area React code is no longer deployed via Pulumi code, but with a custom `after-deploy` plugin, defined within the also newly created [`apps/admin/cli/deploy.ts`](https://github.com/webiny/webiny-js/blob/v5/packages/cwp-template-aws/template/common/apps/admin/cli/deploy.ts) file.
