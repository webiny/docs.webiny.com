...
For example, if using `my-123-prefix-` as the custom Pulumi resource name prefix, then the resource should be set to `arn:aws:states:*:*:stateMachine:wby-123-prefix-*`. So, instead of ending with `wby-*`, the resource now ends with `wby-123-prefix-*`.
