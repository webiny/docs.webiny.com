...
Prior to this release, in order to fully destroy a deployed Webiny project, users would need to destroy all four project applications (**Website**, **Admin**, **API**, and **Core**), by sequentially running four separate [`webiny destroy`](/docs/{version}/infrastructure/basics/destroy-cloud-infrastructure#the-destroy-command) commands.
