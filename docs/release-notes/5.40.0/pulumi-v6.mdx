...
code framework. Find out more in the following [IaC with Pulumi](/docs/{version}/infrastructure/pulumi-iac/iac-with-pulumi)
article.
...
the logs, you can append the `--deployment-logs` flag upon running the [`webiny deploy`](/docs/{version}/core-development-concepts/basics/project-deployment) command.
