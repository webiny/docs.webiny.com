...
// Traverse an entry, and create a flat object with value path as object key.
traverser.traverse(entry, ({ field, value, path }) => {
  /**
   * Most of the time you won't care about complex fields like "object" and "dynamicZone", but only their child fields. The traverser will still go into the child fields, but this way you can conditionally apply your logic.
   */
...
The code to achieve this is as simple as this:

```diff-tsx
import React from "react";
+ import { Admin, AddMenu } from "@webiny/app-serverless-cms";
