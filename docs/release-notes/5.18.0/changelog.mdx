...
Previously, when invoking the Pulumi CLI via the [`webiny pulumi`](/docs/{version}/core-development-concepts/basics/webiny-cli#yarn-webiny-pulumi-folder---env-env----command) command and passing the `--json` flag to it, the final output would still include informative messages:
...
Learn more about remote runtime logs (logs forwarding) in the [Use the Watch Command](/docs/{version}/core-development-concepts/basics/watch-command#enabling-logs-forwarding) how-to guide.
...
To learn more more about it, please read our [Lifecycle Events](/docs/{version}/headless-cms/references/lifecycle-events) key topic.
