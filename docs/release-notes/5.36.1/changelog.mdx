...
Prior to this release, the [`webiny info`](/docs/{version}/core-development-concepts/basics/webiny-cli#common-commands) command would not work if an environment wasn't passed (in which case the command should internally retrieve a list of all environments).
