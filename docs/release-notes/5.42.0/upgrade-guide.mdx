...
##### API
```bash
yarn webiny build api --env=YOUR_ENVIRONMENT
```
##### Admin UI
```bash
yarn webiny build admin --env=YOUR_ENVIRONMENT
```
##### Website
```bash
yarn webiny build website --env=YOUR_ENVIRONMENT
```

##### All at Once

```bash
yarn webiny build api --env=YOUR_ENVIRONMENT && yarn webiny build admin --env=YOUR_ENVIRONMENT && yarn webiny build website --env=YOUR_ENVIRONMENT
```
