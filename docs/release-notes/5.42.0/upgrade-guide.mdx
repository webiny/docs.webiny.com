...
##### API
```bash
yarn webiny build apps/api --env=YOUR_ENVIRONMENT
```
##### Admin UI
```bash
yarn webiny build apps/admin --env=YOUR_ENVIRONMENT
```
##### Website
```bash
yarn webiny build apps/website --env=YOUR_ENVIRONMENT
```

##### All at Once

```bash
yarn webiny build apps/api --env=YOUR_ENVIRONMENT && yarn webiny build apps/admin --env=YOUR_ENVIRONMENT && yarn webiny build apps/website --env=YOUR_ENVIRONMENT
```
