...
Check out the [upgrade guide](docs/release-notes/5.42.0/upgrade-guide) to find out more.
...
### Default Throttling Limits for Api Gateway Deployments ([#4496](https://github.com/webiny/webiny-js/pull/4496))
...
Feel free to modify these limits in your `webiny.application.ts` configuration files.

We decided not to add these limits to previous versions of Webiny, because it would change infrastructure deployment - which is something we are not keen on doing in patch releases.
