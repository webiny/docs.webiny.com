...
The AI-powered image tagging feature was broken because the enrichment task read an `apiKey` field that is never stored — only the encrypted form (`apiKeyEncrypted`) is persisted. The task now correctly decrypts the key at runtime using the `Encryption` service. Response parsing has also been improved to use structured output with a Zod schema, eliminating fragile JSON parsing that broke when the model returned markdown-fenced responses.
