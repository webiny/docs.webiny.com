...
Webiny now includes AI-powered content generation capabilities through the new AI PowerUps feature. You can configure AI providers (OpenAI, Anthropic) in the Admin settings, then use AI to generate page content directly within the Website Builder.
...
A new `DatePicker` component is now available, supporting multiple picker types via a discriminated union on the `type` prop: `date`, `time`, `dateTimeLocal`, `dateTimeTz`, `week`, `month`, `year`, `dateRange`, `multipleDates`, `multipleMonths`, and `multipleYears`.
...
<DatePicker type="dateRange" label="Booking Period" value={{ from: startDate, to: endDate }} onChange={setDateRange} />
```

## Development

### Typescript Upgraded to 6.0.3 ([#5043](https://github.com/webiny/webiny-js/pull/5043))
...
### Fixed Tailwind CSS Build Leaking Filesystem Paths into Production CSS ([#5156](https://github.com/webiny/webiny-js/pull/5156))
...
### Export Missing Dynamic Zone Field Renderer Components ([#5153](https://github.com/webiny/webiny-js/pull/5153))
...
```typescript
import { DynamicZoneRenderer } from "webiny/admin/cms/field-renderers/dynamic-zone";
```

### New `webiny-mailer-smtp` Skill ([#5146](https://github.com/webiny/webiny-js/pull/5146))

A new AI skill has been added to help configure SMTP mailer settings. The `webiny-mailer-smtp` skill can write SMTP configuration directly into your `webiny.config.tsx` file when prompted.

### Improved Content Models and Website Builder Skills ([#5107](https://github.com/webiny/webiny-js/pull/5107))

The `content-models` skill now includes corrected renderer names (matching actual framework implementations like those in `DataFieldBuilder.ts`) and better documentation for list fields and layout nesting with `object` and `dynamicZone` fields.

The `website-builder` skill now documents the TypeScript prop types that input components receive at runtime. For example, file and image inputs receive an object with `{ id, name, src, width, height, mimeType, size }` rather than a plain string.

### Project Hygiene ([#5154](https://github.com/webiny/webiny-js/pull/5154), [#5142](https://github.com/webiny/webiny-js/pull/5142), [#5128](https://github.com/webiny/webiny-js/pull/5128))

Yarn has been upgraded to 4.14.1. Third-party dependencies have been updated to their latest stable versions, reducing exposure to known vulnerabilities and picking up recent upstream improvements. Several packages had missing `exports` fields in their `package.json` files — these have been corrected, improving compatibility with modern bundler toolchains.

### Upgrade Command Improvements ([#5161](https://github.com/webiny/webiny-js/pull/5161))

The `webiny upgrade` command now supports a `--force` flag, allowing you to bypass certain checks when running upgrades. Additionally, debug output is now enabled by default during upgrade operations, making it easier to diagnose issues without additional configuration.

### Storybook Upgraded to Version 10 ([#5105](https://github.com/webiny/webiny-js/pull/5105))

The internal Storybook setup has been upgraded to the latest version (v10). This brings improved performance, better TypeScript support, and access to the latest Storybook features for component development and documentation.
...
- [Form Model Reference](/reference/admin/form-model) — API reference for the declarative Form Model system

**Website Builder**
- [Page Settings](/website-builder/page-settings) — customise and extend Page Settings with the FormModel architecture
...
Large CMS models with many fields can hit storage limits or slow down model loading. You can now enable field compression to reduce the stored size of model definitions.
...
This is primarily useful for testing scenarios or projects with unusually complex models.

### Hidden Built-in Models ([#5099](https://github.com/webiny/webiny-js/pull/5099))

Built-in system models (Languages and Tenants) are now hidden from the Admin UI by default. These models are tagged with `$hidden:true` and no longer appear in the content model list, reducing clutter for content editors.

### Refactored Entry Filtering System ([#5086](https://github.com/webiny/webiny-js/pull/5086), [#5089](https://github.com/webiny/webiny-js/pull/5089))

The internal filtering system for CMS entries has been refactored for better maintainability and extensibility. The previous plugin-based approach (`CmsEntryFilterPlugin` and `ValueFilterPlugin`) has been replaced with a cleaner abstraction pattern using a filter registry. This change is internal and does not affect the public API, but it lays the groundwork for more flexible filtering capabilities in future releases.
