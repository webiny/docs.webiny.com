...
### Introducing Single Entry Models ([#4228](https://github.com/webiny/webiny-js/pull/4275) [#4298](https://github.com/webiny/webiny-js/pull/4298) [#4296](https://github.com/webiny/webiny-js/pull/4296))
...
Once created, we proceed with defining the fields for the model, just like we would for any other model. The only difference is that, with single entry models, there can only be one entry of a given model, which is actually created automatically when the model is created.
