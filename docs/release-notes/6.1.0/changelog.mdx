...
Component manifests now support element placement validation through `constraints`, `descendantConstraints`, and `canDelete` functions. Additionally, `onChange` and `onDescendantChange` lifecycle hooks let components react to create, update, and delete events, which is useful for implementing cross-element side effects.
