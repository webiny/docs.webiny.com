...
In 5.38.0 a bug was introduced that prevented users from removing the last item in a multiple reference field - item was removed from the UI, but it was not sent to the API, and it was not updated.

### Fix Multiple Model Multi Reference Field Output Order on Read API ([#3761](https://github.com/webiny/webiny-js/pull/3761))
