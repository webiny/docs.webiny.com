...
- **Add a Bulk Action Task**: Implement background task support. Detailed guidance is available in [the documentation](/docs/{version}/headless-cms/extending/customize-entry-list-bulk-actions#bulk-action-via-background-task).
...
the [Prerendering Pages](https://www.webiny.com/docs/{version}/architecture/website/prerendering-pages) article.
...
With all of these steps completed, you should be able to see the new watch command in action. To learn more, we've also created a new section in our existing [`watch command`](/docs/{version}/core-development-concepts/basics/watch-command) article, which explains how to use this new feature.
...
Credentials](/docs/{version}/infrastructure/aws/configure-aws-credentials#deploy-webiny-project-aws-cloud-formation-template)
article.
...
Pulumi](/docs/{version}/infrastructure/pulumi-iac/iac-with-pulumi) topic to learn more.
...
To find out more about shared indexes, please read [this](/docs/{version}/core-development-concepts/extending-and-customizing/how-to-customize-elasticsearch-index) article.
