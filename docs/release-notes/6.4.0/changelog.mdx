...
### Webhooks: Deliveries View Is Now a Standalone Page ([#5244](https://github.com/webiny/webiny-js/pull/5244))

The webhook deliveries view has been refactored from an embedded component to a dedicated standalone page. This improves navigation clarity and makes delivery logs easier to access and manage independently from the webhook configuration screen.
...
### Desktop Notifications on Build Completion ([#5251](https://github.com/webiny/webiny-js/pull/5251))

The build script now sends native OS notifications when a build completes or fails, so you don't have to watch the terminal. Works on macOS and Linux.

### Webiny Telemetry System v3 ([#5197](https://github.com/webiny/webiny-js/pull/5197))

The Webiny Telemetry System (WTS) has been completely re-implemented. The new system connects user sessions across the web, CLI, and admin app, providing a unified view of telemetry data. The implementation continues to respect user telemetry preferences — if telemetry is disabled in your project, no data is collected.
