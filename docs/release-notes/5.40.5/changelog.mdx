...
By [updating Pulumi package versions](#updated-pulumi-package-versions-4216), we've also fixed an issue with the [`webiny about`](/docs/{version}/core-development-concepts/basics/webiny-cli#yarn-webiny-about) command, which would incorrectly report the `@pulumi/pulumi` package version.
