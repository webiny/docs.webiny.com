...
### External Identity Providers - Assigning Groups Now Works As Expected ([#4349](https://github.com/webiny/webiny-js/pull/4349)]

We've addressed a couple of issues related to assigning roles when using external identity providers like [Auth0](/docs/enterprise/auth0-integration) or [Okta](/docs/enterprise/okta-integration). 

#### 1. `groups` Property Now Works As Expected

Using Auth0 and assigning roles via the newly introduced `groups` property would not work:
...
Additionally, we've also addressed an issue where even the `group`  
...
When using external identity providers, trying to assign a role to a user that was programmatically created would not work. This has been addressed, and now users can assign roles to programmatically created users when using external identity providers.
...
Since the introduction of the Teams feature, users were not able to assign teams to users when using external identity providers. We've addressed this issue, and now users can assign teams to users when using external identity providers.
...
As already mentioned above, in the same fashion, the extension can be accesssed via the `yarn webiny scaffold`, or the new `yarn webiny extension` command.
