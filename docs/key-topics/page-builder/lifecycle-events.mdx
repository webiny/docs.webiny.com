...
new ContextPlugin<PbContext>(async context => {
    context.pageBuilder.onBeforePageUnpublish.subscribe(async({latestPage, page}) => {
         ...
         */
        if (latestPage.version === page.version) {
        	throw new Error(`Page you are trying to unpublish is the latest revision of the page and it is not allowed to unpublish it.`);
        }
    });
})
```
...
:::danger
Please, be aware that you can change what ever you want on the page object before it is stored into the database, so be careful with changing the data.
:::
