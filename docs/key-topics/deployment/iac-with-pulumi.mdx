...
<!-- vale off -->
In the following folder structure, we are looking at three applications that are set up by default in a new Webiny project. These are the **api** (`./api`), **admin** (`./apps/admin`), and **website** (`./apps/website`), which are shown in the following directory structure (`code` folder omitted for brevity):
<!-- vale on -->
...
These files describe at what state your cloud infrastructure is currently at. For example, it contains a list of all deployed resources, various metadata, configuration, and so on. State files are stored per Pulumi program stack, and by default, they are stored in your project application folder, inside the `.pulumi` folder, like so:
