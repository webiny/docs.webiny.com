...
:::note
This article uses `@webiny/app-admin` as a source of all UI Composer classes.
In your non-Webiny projects, you can use `@webiny/ui-composer` package directly.
:::

## Defining a Basic View

For those of you who have experience working with other programming languages (like C#, Java, Python, Swift), this way of building UIs will feel very familiar. For JS-only folk, this will most likely seem weird in the beginning.
...
Now we need to render it. You can render this new view anywhere in your existing React app, by using the provided `ViewComponent` React component:

```tsx title="Rendering in React"
<ViewComponent view={new MyView()} />
```
...
```ts title="Adding a Hook to Existing View and Attach Rendering Logic"
new UIViewPlugin<MyView>(MyView, view => {
  ...
  firstButton.addShouldRender(() => {
    // Get view hook to access state
    const { buttonClicked } = view.getHook<UseMyView>("myView");
    return !buttonClicked;
  });

  // Call a hook function on button click
  secondButton.setOnClick(() => {
    const { setButtonClicked } = view.getHook<UseMyView>("myView");
