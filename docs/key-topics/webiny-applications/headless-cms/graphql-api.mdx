...
The Headless CMS GraphQL API is accessed via multiple URL endpoints, each including the GraphQL API type and locale. This is because different combinations of the two yield different GraphQL schemas.
...
<CenteredImage alt="API Playground" src={apiPlaygroundSwitchLocale} />
...
You can only access content models, groups, and entries to which you have access, which is based on the security group your account belongs to.
