...
keywords: ["webiny", "headless", "cms", "graphql", "api", "locales", "types"]
...
import securityImage from "./graphql-api/security.png";
import apiPlaygroundMainMenu from "./graphql-api/api_playground_from_main_menu.png";
...
:::info
If you want to learn how to use the Headless CMS GraphQL API via the built-in API Playground or programmatically, via a GraphQL client, make sure to check out the [Using the GraphQL API](/docs/how-to-guides/headless-cms/using-graphql-api) guide.
:::
...
Except for the fact that this GraphQL API type allows you to create new, and edit and publish existing content entries, another specific aspect of it is that it returns both published and unpublished content entries. This is intentional, because, as a content manager, you need to be able to access all of the data, not just published.
...
The Headless CMS GraphQL API sits behind a security layer that provides fine-grain access control, both for logged-in, and anonymous users. The following are some of the available options:
...
<CenteredImage alt="Fine Grain Access Controls" src={securityImage} />
...
For example, the **read** GraphQL API only allows users to perform queries on different content entries. On the other hand, not only the **manage** type allows querying content entries, but also content models and groups, and of course, provides the necessary management-related mutation operations. Finally, different locales can have different content models, which, again, means a different GraphQL schema needs to be in place.
...
Note that if you want to access content in different locales, you can switch to a different one via the locale selector, located in the top right corner of the screen.
...
#### When using the API Playground, is my access limit in any way?

You can only access content models, groups, and entries to which you have access, which is based on the security group your account belongs to. To learn more, visit the [Using the GraphQL API](/docs/how-to-guides/headless-cms/using-graphql-api) guide.
