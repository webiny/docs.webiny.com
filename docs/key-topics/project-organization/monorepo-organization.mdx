...
When talking about the monorepo organization and yarn 2 and its workspaces feature, than the term **workspace** makes more sense to use. On the other hand, when talking about project applications and the actual code, the term **package** is used. We always say "import this package", never "import this workspace".
...
:::info
For convenience's sake, different scaffolding commands update the workspaces list automatically for you.
:::

:::info
To make it a bit easier to work with multiple packages in your Webiny project, Webiny CLI offers the `workspaces` (or `ws` for short) command. Check out the [Working With Workspaces](/docs/how-to-guides/webiny-cli/working-with-workspaces) guide to learn more.
:::
