...
As mentioned above, when a PR is submitted, we want to run different types of tests against a project that was dynamically deployed into a [short-lived (ephemeral)](/docs/how-to-guides/ci-cd/testing#deploying-short-lived-ephemeral-environments-for-testing-purposes) test environment. Once all tests are passing, we can then approve the PR and merge its code changes into the `dev` branch.
...
Because of the following, our recommendation is to use the first [Choosing `staging` As The Target Branch](/docs/how-to-guides/ci-cd/workflows#choosing-staging-as-the-target-branch) strategy:
