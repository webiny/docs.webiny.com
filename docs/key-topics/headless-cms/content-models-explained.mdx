...
## Lifecycle hooks
All lifecycle calls can trigger custom plugin hooks via [CmsContentModelHookPlugin](../../references/headless-cms/api/cmscontentmodelhookplugin) plugin type.
...
For the next topic we would suggest reading about [content model lifecycle hooks](./lifecycle-hooks/content-model.mdx) and [content entry lifecycle hooks](./lifecycle-hooks/content-entry.mdx).