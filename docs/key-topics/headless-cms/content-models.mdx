...
## Overview
We will go through all files that are connected with `./packages/api-headless-cms/src/content/plugins/crud/contentModel.crud.ts`, including that file, to explain what do we do with the input on the manage API.
...
At the start of each method we check for user permissions (read, write and delete), depending on what the method is. We do not check, directly, combinations of the permissions like `write+read` or `read+delete`.

### Creating the content model
Input data for creating the content model is [CmsContentModelCreateInput](../../references/headless-cms/api/cmscontentmodelcreateinput). At this point there are no fields defined yet.
...
```
With all these steps successfully ran, you have a content model into which you can add fields.

### Updating the content model
Input data for updating the content model is [CmsContentModelUpdateInput](../../references/headless-cms/api/cmscontentmodelupdateinput). This is where you define the fields you want in your content model.
...
```
13. run the after save hook
```ts
await afterSaveHook({ context, model: fullModel });
```

This completes the content model definition.
Without update part of the flow, your content model does not contain any of the fields and so it is not usable in creating the content entries.


### Content model fields[](#content-model-fields)

The content model field definition, that we store in the database, is [CmsContentModelField](../../references/headless-cms/api/cmscontentmodelfield).
Input for the field is [CmsContentModelFieldInput](../../references/headless-cms/api/cmscontentmodelfieldinput) and, as you can see, we do not actually transform the input to storage type.
...
The layout is saved as an array of strings array. The type Representation is `string[][]`. This is basically the list of content model field IDs. It is used to display fields in some order when creating or updating the content entry.