...
For the article we will assume that we have some outside service, named `ModelService`, that also stores models. Also, we will split hook plugins to simplify the examples.
...
What can you do in this hook? You want to check to make sure that `modelId` is unique in the `ModelService`:
...
What can you do in this hook? You can now add the model to the outside service:
