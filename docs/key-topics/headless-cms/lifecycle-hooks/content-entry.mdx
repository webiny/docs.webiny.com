...
For the article we will assume that we have some remote service, named `EntryService`, which stores the entries and a webhook call that can run some background process.
...
This hook is triggered before we request review for the entry revision.
...
This hook is triggered after we request review for the entry revision.
...
const afterRequestReview = (): CmsContentEntryHookPlugin => ({
	...
	afterRequestReview: async({model, entry}): Promise<void> => {
		const result = await ReviewService.request(entry);
		if (result){
		    return;
		}
...
## Conclusion
There are a lot of hooks, yes, but probably will not use most of them. Also, feel free to define hooks in a single plugin, eg. Memcache hooks should be in a single plugin.