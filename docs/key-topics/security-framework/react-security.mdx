...
For these purposes, WSF provides a `SecurityProvider` component which holds your identity information and allows other parts of the application to use it using a dedicated `useSecurity()` hook. Here's a simplified React application to visualize the component hierarchy:
...
The only requirement from the Webiny API is that you provide the necessary plugins to authenticate your IdPs JWT token and load identity's permissions.

TODO: Add links to tutorials on securing your app.
