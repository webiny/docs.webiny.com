...
The diagram gives an overview of the complete cloud infrastructure needed by the Webiny applications that come set up by default with every new Webiny project, which together form the **API** project application.

:::info
All of the shown cloud infrastructure resources are deployed into a single AWS region, which is defined by you.
:::
