...
import cliReporting from "./5.3.0/cli-reporting.png";
import searchBarCms from "./5.3.0/search-bar-cms.png";
...
- no infinite redirects when switching from one content entry to another (via main-menu)
...
### `create-webiny-project` - Added `--no-cleanup` flag [(#1526)](https://github.com/webiny/webiny-js/pull/1526)
...
With the newly added `--no-cleanup` flag, this can now be prevented, allowing the user to check the contents of generate files, and maybe provide more info while debugging these errors.

You can use the `--no-cleanup` flag like so:
