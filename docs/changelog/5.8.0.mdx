...
import createdBy from "./5.8.0/createdBy.png";
import entryId from "./5.8.0/entryId.png";
import retrieveById from "./5.8.0/retrieveById.png";
...
:::info How To Upgrade?
Please check the [Webiny 5.8.0 migration guide](/docs/how-to-guides/upgrade-webiny/5.7.0-to-5.8.0) for the upgrade steps.
:::
...
:::info
With Webiny Headless CMS, users first define both content models, for example **Blog Post**, **Book**, **Author**, and so on. Once those are in place, they continue by creating content entries, which consist of the information that was specified in their respective content model. Every content entry can have multiple revisions, of which only one can be marked as the published one.
:::
