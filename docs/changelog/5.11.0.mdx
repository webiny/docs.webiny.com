...
import objectField from "./5.11.0/objectField.gif";
import patternValidators from "./5.11.0/patternValidators.png";
...
### Fine-grain AWS policies ([#1741](https://github.com/webiny/webiny-js/pull/1741))

From now on, all new Webiny projects will benefit from improved security, due to fine-grain policies used with AWS Lambda roles (instead of using the default `AdministratorAccess` policy, which was previously the case).

In addition to that, you can also use a fine-grain policy for the [CLI user](/docs/how-to-guides/deployment/aws/configure-aws-credentials/#attaching-a-fine-grain-policy), which is important for making secure deploys of your Webiny project.
