...
When files are uploaded via File Manager, ultimately, they must always be served to users over CDN ([Amazon Cloudfront](https://aws.amazon.com/cloudfront/)). This was true for images, but in every other case, files would be directly served from S3, making the actual file download less optimal.
