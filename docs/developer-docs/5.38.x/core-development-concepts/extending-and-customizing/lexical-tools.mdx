...
To convert raw Lexical state to HTML, Lexical editor requires DOM to be present on the `global` object.
...
Then use it to setup the environment:

```ts Setup the necessary DOM objects
