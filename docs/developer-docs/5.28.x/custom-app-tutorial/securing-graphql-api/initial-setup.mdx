...
In order for the new environment variables to be available in our GraphQL API handler function, we'll need to redeploy it. If you had a watch session already up and running, this should've happened automatically. Otherwise, you'll need to either start a new session via the [`webiny watch`](/docs/{version}/core-development-concepts/basics/watch-command), or simply deploy the change via the [`webiny deploy`](/docs/{version}/core-development-concepts/basics/project-deployment) command.
