...
```ts api/code/graphql/src/plugins/scaffolds/carManufacturers/entities/CarManufacturer.ts
(...)
...
```ts api/code/graphql/src/plugins/scaffolds/carManufacturers/types.ts
...
```ts api/code/graphql/src/plugins/scaffolds/carManufacturers/typeDefs.ts
...
If you've run the two [`webiny watch`](../../core-development-concepts/basics/watch-command) commands, mentioned in the [previous section](./get-started#using-the-watch-command), the performed application code changes should be automatically rebuilt and redeployed into the cloud.
...
```tsx apps/admin/code/src/plugins/scaffolds/carManufacturers/views/CarManufacturersForm.tsx
(...)
...
```ts apps/admin/code/src/plugins/scaffolds/carManufacturers/views/hooks/graphql.ts
...
Because this is not something we can effectively predict, the generated application code does not include any authentication and authorization logic. But luckily, with a couple of [built-in utilities](../../overview/features/security), this is not too hard to implement.
