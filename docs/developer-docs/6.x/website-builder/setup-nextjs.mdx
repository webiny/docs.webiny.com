...
<Alert type="info" title="Peer Dependency Warnings">

Yarn prints peer dependency warnings for `next`, `react`, and `mobx` from `@webiny/*` packages. They're expected, and the kit builds and runs normally. If you use npm instead, it stops with an `ERESOLVE` error for the same reason. Run the install with `--legacy-peer-deps` in that case.
