...
import pageSettingsDrawer from "./assets/wb-page-settings-drawer.png";
...
<Image src={pageSettingsDrawer} title="Page Settings Drawer"/>
...
<Alert type="warning" title="IMPORTANT">
  ...
  Store all custom data in `doc.extensions` — never write to `doc.properties`, which is reserved for built-in system
  properties (title, path, snippet, image, tags, seo, social). Writing custom fields into `doc.properties` risks naming
  collisions with future Webiny updates.
