...
Most day-to-day work uses [`webiny deploy`](/cli/deploy) and [`webiny destroy`](/cli/destroy). Use `webiny pulumi` when you need lower-level access that those commands do not expose.
