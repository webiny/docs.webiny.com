...
When you run [`webiny deploy`](/{version}/cli/deploy) or [`webiny watch`](/{version}/cli/watch), Webiny executes a build step followed by a deploy step for each project application (Core, API, Admin). You can hook into any of these steps using `Infra.*` lifecycle extensions registered in `webiny.config.tsx`.
