...
Use `env.isProduction` to check whether the current environment is a production environment. This is calculated based on the list of production environments defined via [`Infra.ProductionEnvironments`](/{version}/infrastructure/extensions/production-environments):
...
After modifying a Pulumi extension, redeploy the affected stack by running the [`webiny deploy`](/{version}/cli/deploy) command:
