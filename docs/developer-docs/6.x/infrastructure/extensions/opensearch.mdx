...
The shared OpenSearch cluster must be deployed and managed outside of Webiny — it is not provisioned by Webiny itself. You can deploy it using the AWS console, Terraform, Pulumi, or any other tool of your choice.

Once the cluster is running, configure Webiny to connect to it:
...
If your shared OpenSearch instance is running on a custom domain (e.g. `opensearch.mysite.com`), use the `endpoint` prop to point Webiny at it directly:
...
<Infra.OpenSearch
  ...
  domainName={"my-shared-opensearch-domain"}
  indexPrefix={"dev_"}
  endpoint={"https://opensearch.mysite.com"}
/>
```

If the instance requires authentication, add the `username` and `password` props:
...
`domainName` must always be specified when connecting to a shared instance. Webiny uses it to locate the domain in AWS even when `endpoint` is provided.
