...
For brevity, diagrams don't include network-level resources like VPC, availability zones, and subnets. See [Deployment Modes](/{version}/infrastructure/deployment-modes) for network architecture details.
...
Learn more about DynamoDB-only vs DynamoDB + OpenSearch setups in [Database Setups](/{version}/infrastructure/introduction).
