...
| `isPrivate?: boolean`                                                               | —                                                        |
| `run(params: ITaskRunParams<I, O>): Promise<ITaskResult<I, O>>`                     | Core run method - receives ONLY input params             |
| All runtime dependencies (logging, state management, etc.) come from TaskController |
| `onBeforeTrigger?(params: ITaskBeforeTriggerParams<I>): Promise<void>`              | Optional lifecycle hooks - receive task data, no context |
...
| `createInputValidation?(
    ...
    ): GenericRecord<keyof I, zod.Schema> \| zod.Schema` | Create a validation schema for the task input.
