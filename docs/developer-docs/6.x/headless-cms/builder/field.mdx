...
**`renderer(name: string, settings?: Record<string, any>)`** - Sets the field renderer. See [Field Renderers](/headless-cms/ui/field-renderer) for available renderers.
...
address: registry
  ...
  .layout([["street"], ["city", "zipCode"], ["country"]]);
```
...
```typescript
title: registry.text().label("Title").renderer("text-input");
```

For a complete list of available renderers and which fields they support, see the [Field Renderers](/headless-cms/ui/field-renderer) documentation.
