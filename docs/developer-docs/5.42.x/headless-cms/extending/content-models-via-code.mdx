...
To create a content model group and a content model, we use `createGroupPlugin` and `createModelPlugin` plugin factories, as shown in the following code snippet.
...
As we can see, first we define a new content model group via the `createGroupPlugin` plugin factory. The following properties are required:
...
import { createGroupPlugin, createModelPlugin } from "@webiny/api-serverless-cms";
