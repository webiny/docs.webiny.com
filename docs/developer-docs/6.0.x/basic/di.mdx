...
Use `createImplementation()` from the abstraction namespace to bind your class to it. This function requires three properties:
...
const InMemoryBookRepositoryImpl = createImplementation({
    abstraction: BookRepository,
...
const CreateBookUseCaseImplementation = createImplementation({
    abstraction: CreateBookUseCase,
...
Decorators extend behavior without modifying the original implementation. Use `createDecorator()` to wrap existing functionality:
...
const LoggingCreateBookDec = createDecorator({
    abstraction: CreateBookUseCase,
...
class MyClass implements SomeAbstraction.Interface {
    public constructor(
        private firstDep: IFirstDep,    // Position 0
        private secondDep: ISecondDep   // Position 1
    ) {}
}

const MyClassImpl = createImplementation({
    abstraction: SomeAbstraction,
...
```typescript
const MyImplementation = createImplementation({
