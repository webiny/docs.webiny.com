...
## How Pulumi Is Installed

Webiny does not install the Pulumi CLI globally. It downloads the Pulumi binary into `node_modules/@webiny/pulumi-sdk/pulumi` when you first set up the project. The `webiny pulumi` command invokes that local binary, so the version is always pinned to what the project expects.
