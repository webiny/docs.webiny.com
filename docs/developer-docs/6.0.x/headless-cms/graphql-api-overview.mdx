...
description: Understand the Read and Manage GraphQL APIs and how to access them
---
...
- What are the Read and Manage APIs?
...
Webiny Headless CMS exposes two separate GraphQL APIs: the **Read API** for fetching published content and the **Manage API** for full CRUD operations. Both APIs are available via HTTP endpoints and require authentication via API keys.
...
Both APIs require authentication via API keys passed in the `Authorization` header:
...
| Preview unpublished content                | Manage API |

## GraphQL Playground

Both APIs provide a GraphQL playground for exploring the schema and testing queries:
