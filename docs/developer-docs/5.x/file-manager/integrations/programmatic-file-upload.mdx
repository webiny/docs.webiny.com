...
Upload of files into the File Manager is a 3-step process, which is described, from the architectural point of view, in the [File Upload](/{version}/architecture/api/file-upload) article. Very often, files can be several gigabytes (or even terabytes) large, which makes it impossible to use the GraphQL API, or even a REST API, to upload binary content using the Lambda functions, which power our API, due to its physical limitations.
