...
In this example, we add two fields: `carMake` and `year`. You can add as many fields as you like. The field definition is identical to that of the Headless CMS model field, and all the same rules and options apply. To learn more about programmatic model and field definition, visit [Define Content Models via Code](/{version}/headless-cms/extending/content-models-via-code).
...
<Image
  src={fmMp4CustomRenderer}
  title="Custom Renderer Is Applied to 'carMake' Field on All Mp4 Files"
/>
