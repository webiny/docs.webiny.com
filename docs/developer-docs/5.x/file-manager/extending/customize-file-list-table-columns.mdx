...
The following code examples are built using the [custom file field](/{version}/file-manager/extending/customize-file-fields) `copyright`.
...
```tsx
<FileManagerViewConfig>
  <Browser.Table.Column name={"extensions.copyright"} header={"Copyright"} />
</FileManagerViewConfig>
```
...
```tsx
<FileManagerViewConfig>
  <Browser.Table.Column name={"extensions.copyright"} header={"Copyright"} before={"createdBy"} />
</FileManagerViewConfig>
```
