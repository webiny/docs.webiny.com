...
1. A user triggers the task either via [GraphQL API mutation](/{version}/core-development-concepts/background-tasks/background-task-management#trigger-a-background-task) or via the [code](/{version}/core-development-concepts/background-tasks/background-task-management#trigger-a-background-task).
