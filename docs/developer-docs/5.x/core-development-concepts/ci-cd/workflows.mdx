...
In this guide, we combine all of the concepts we covered in previous [Environments](/{version}/core-development-concepts/ci-cd/environments), [Version Control](/{version}/core-development-concepts/ci-cd/version-control), [Managing Cloud Infrastructure Files](/{version}/core-development-concepts/ci-cd/cloud-infrastructure-state-files), and [Testing](/{version}/core-development-concepts/ci-cd/testing) articles, in order to establish a complete CI/CD workflow. In case you missed those, we encourage you to check them out first.
...
If you'd like to learn more about the CI/CD practice in general and all of the different phases it consists of, check out the [CI/CD](/{version}/core-development-concepts/ci-cd/introduction) key topic.
...
The `dev` branch is usually what we select as the [default branch](/{version}/core-development-concepts/ci-cd/version-control#shared-branches) of our project's Git repository.
...
As mentioned above, when a PR is submitted, we want to run different types of tests against a project that was dynamically deployed into a [short-lived (ephemeral)](/{version}/core-development-concepts/ci-cd/testing#deploying-short-lived-ephemeral-environments-for-testing-purposes) test environment. Once all tests are passing, we can then approve the PR and merge its code changes into the `dev` branch.

![Webiny Cloud Infrastructure - Environments](./assets/workflows/merging_1.png)

### 2. Merge Changes To `staging` Branch

Once all of the pending PRs have been tested successfully and merged into the `dev` branch, we then continue by merging the `dev` branch into `staging`. This can be done either via a new PR or a direct merge, but we recommend the former. This is simply because creating a PR first allows us to maybe [preview the deployment](/{version}/core-development-concepts/ci-cd/testing#testing-cloud-infrastructure-changes) that will happen once the code is merged, and maybe perform some other tests.
...
Because of the following, our recommendation is to use the first [Choosing "staging" As The Target Branch](/{version}/core-development-concepts/ci-cd/workflows#choosing-staging-as-the-target-branch) strategy:
