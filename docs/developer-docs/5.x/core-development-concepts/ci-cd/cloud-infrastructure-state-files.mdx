...
Read more about Pulumi, environments, and other deployment-related concepts in our [Deployment](/{version}/infrastructure/basics/introduction) key topics section.
...
Fortunately, out of the box, Pulumi already offers a solution that solves the state management overhead for us. Using different [backends](/{version}/infrastructure/pulumi-iac/iac-with-pulumi#different-backends), we are able to store our state files in different places, in a secure and reliable way.
...
Learn more about project applications and project organization in general, in the [Project Applications and Packages](/{version}/core-development-concepts/project-organization/project-applications-and-packages) key topic.
...
Learn more about different types of environments in the [Environments](/{version}/core-development-concepts/ci-cd/environments) guide.
...
Check out the [Environment Variables](/{version}/core-development-concepts/basics/environment-variables) how-to guide to learn more.
...
Finally, copy all of the folders and files located in your project root `.pulumi` folder into the newly created bucket. Once copied, in your Amazon S3 bucket, you should end up with the following paths:
...
In order to confirm everything works as expected, from your project root, simply run the [`webiny info`](/{version}/core-development-concepts/basics/webiny-cli#yarn-webiny-info---env-env) command. If everything was done correctly, it should display the correct information (GraphQL API URL, Headless CMS GraphQL API URLs, ...).

### Can state files contain secrets or passwords? How are these stored?

If you're passing secrets or passwords to cloud infrastructure resources in your cloud infrastructure ([Pulumi](/{version}/infrastructure/pulumi-iac/iac-with-pulumi)) code, for example an API token from an external service, then it's recommended that you mark them as secret.
