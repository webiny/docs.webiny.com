...
All of the mentioned tools (and some others) are already included and configured for you in every Webiny project.
</Alert>
...
Merging strategies are covered in the upcoming [Workflows](/{version}/core-development-concepts/ci-cd/workflows) guide.
...
As mentioned in the previous [Points of Execution](/{version}/core-development-concepts/ci-cd/testing#points-of-execution) section, tests can be run upon submitting PRs. And since integration and end-to-end (E2E) tests require cloud infrastructure resources to be deployed, we end up with a need to deploy our project into a short-lived, or sometimes also called an ephemeral, environment.
...
In some cases, doing this can [incur additional cost](/{version}/core-development-concepts/ci-cd/environments#multiple-environments-and-extra-costs).
...
Read more about the preview command in the [Preview Deployments](/{version}/infrastructure/basics/preview-deployments) guide.

</Alert>

#### Mark Mission Critical Cloud Infrastructure Resources As Protected

By default, Webiny uses [Pulumi](/{version}/infrastructure/pulumi-iac/iac-with-pulumi) as the default infrastructure as code (IaC) framework, which allows us to mark one or more cloud infrastructure resources as [protected](https://www.pulumi.com/docs/intro/concepts/resources/#protect):
...
Within a Webiny project, note that the `pulumi up` and `pulumi destroy` commands are run via the [`webiny deploy`](/{version}/core-development-concepts/basics/project-deployment) and [`webiny destroy`](/{version}/infrastructure/basics/destroy-cloud-infrastructure) commands, respectively.
...
To learn more about the necessary cloud infrastructure resources Webiny is relying on, check out the [Cloud Infrastructure](/{version}/architecture/introduction) section.
...
Learn more about the **API** project application in the [Cloud Infrastructure](/{version}/architecture/api/overview) key topics section.
