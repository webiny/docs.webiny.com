...
<Alert type="warning">
  This feature will change in the future. We cannot guarantee the data integrity after the system
  upgrade (in minor version, eg. 5.42.x to 5.43.0)
</Alert>
...
<Alert type="info">
  For detailed information on how to use the GraphQL API, check out the API Playground in your
  Webiny Admin UI.
</Alert>
...
query ListLogs {
  logs {
    listLogs(
      where: { tenant: "root", source: "myCustomSource", type: error }
      sort: DESC
      limit: 100
