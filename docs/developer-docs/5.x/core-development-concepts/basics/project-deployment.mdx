...
Webiny makes it easy to deploy your project, using the [Webiny CLI](/{version}/core-development-concepts/basics/webiny-cli) and [Pulumi](/{version}/infrastructure/pulumi-iac/iac-with-pulumi) as the default infrastructure as code and deployment solution. You can also easily deploy your project into multiple [environments](/{version}/infrastructure/basics/environments), which is covered in this guide as well.
...
To learn more about configuring your AWS credentials, please check out the [Configure AWS Credentials](/{version}/infrastructure/aws/configure-aws-credentials) guide. If you're looking for information on how to use different AWS profiles, feel free to skip to the [Use AWS Profiles](/{version}/infrastructure/aws/use-aws-profiles) guide.
...
As explained in the [introduction to deployment](/{version}/infrastructure/basics/introduction) key topic, doing a complete project deployment entails deploying all of its [project applications](/{version}/core-development-concepts/project-organization/project-applications-and-packages).
...
For more information on the `webiny pulumi` command, please visit the [Execute Pulumi Commands](/{version}/infrastructure/pulumi-iac/execute-pulumi-commands) guide.
...
Read more about project organization in the [Project Organization](/{version}/core-development-concepts/project-organization/project-applications-and-packages) key topics section.
...
To learn more, please check out the [Modify Cloud Infrastructure](/{version}/infrastructure/basics/modify-cloud-infrastructure) article.
...
We're aware of this fact, and this is mainly because of the [Amazon OpenSearch Service](https://aws.amazon.com/opensearch-service/). While other cloud infrastructure resources get deployed reasonably fast, this service can take anywhere from 15 to 30 minutes to become ready.
...
This error can occur if one of the previous deployments of your [project application](/{version}/core-development-concepts/project-organization/project-applications-and-packages) has been [interrupted](/{version}/core-development-concepts/basics/project-deployment#one-of-the-previous-deployments-has-been-interrupted), or another deployment is [currently in progress](/{version}/core-development-concepts/basics/project-deployment#another-deployment-currently-in-progress).
...
If you didn't install the Pulumi CLI manually by yourself, which is usually the case for most Webiny users, you can execute the above commands via the [`pulumi`](/{version}/infrastructure/pulumi-iac/execute-pulumi-commands) command, for example:
...
Learn more about cloud infrastructure state files in the [Cloud Infrastructure State Files](/{version}/core-development-concepts/ci-cd/cloud-infrastructure-state-files) development workflows guide.
...
When deploying your project applications with the [`webiny deploy`](/{version}/core-development-concepts/basics/project-deployment) command, on a couple of occasions, we've seen users receive the following error:
...
You can find more details about the development and deployment principles in this [article](/{version}/core-development-concepts/development/local-development).
