...
To connect to the Webiny MCP server in Cursor, add the following to your `.cursor/mcp.json` file or Cursor settings, as explained in the [Cursor documentation](https://docs.cursor.com/context/model-context-protocol):
...
<Alert type="info" title="Additional One-Time Setup in VS Code">
  Sometimes, in VS Code, you may need to perform a one-time setup. When you ask a question and
  include a phrase like “use Webiny Docs”, then VS Code will prompt you to enable the Webiny MCP
  server for all sessions. Allow it, and the setup will be complete. This step is required only
  once. Please refer to the image below for reference.
</Alert>
...
```json
{
  "mcpServers": {
    "webiny": {
      "command": "npx",
      "args": ["@modelcontextprotocol/server-fetch", "https://mcp.docs.webiny.com/mcp"]
    }
  }
}
```
