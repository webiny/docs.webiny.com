...
With the [`webiny watch`](/{version}/core-development-concepts/basics/watch-command) command running, any changes to the application code will be automatically rebuilt and redeployed to the cloud.
...
**Please note that the `slug` is important and should match the one we defined in the [Cognito Authenticator for Notes App Users](/{version}/headless-cms/notes-app/webiny-infrastructure-setup#add-cognito-authenticator-for-notes-app-users). For our use case, the `slug` should be `notes-app-users`.**
...
As the final step on the Webiny side, we'll create a Note Model to store user notes. If you're new to creating models in Webiny, refer to the [Create Content Model](/{version}/user-guides/headless-cms/essentials/create-content-model) user guide.
...
Great, we're all set on the Webiny side. Now, let's move on to building the [Notes App in React](/{version}/headless-cms/notes-app/react-notes-app).
