...
Great, we're all set on the [Webiny side](/{version}/headless-cms/notes-app/webiny-infrastructure-setup). Now, let's move on to building the Notes App in React using the <ExternalLink href={"https://ui.docs.amplify.aws/react/connected-components/authenticator"}>AWS Amplify UI Authenticator</ExternalLink>.
...
You can find these config values from [this step](/{version}/headless-cms/notes-app/webiny-infrastructure-setup#deploy-the-core-and-api-applications).
...
The `uri` mentioned below is the Manage API URL for the Headless CMS. For more details on how to obtain the Manage API URL, refer to [this guide](/{version}/headless-cms/basics/graphql-api#where-to-find-the-url-of-the-graph-ql-api).
...
export const deleteNote = gql`
  mutation DeleteNote($revision: ID!, $options: CmsDeleteEntryOptions) {
...
import "./App.css";
import "@aws-amplify/ui-react/styles.css";
import { Button, Flex, Heading, Text, TextField, View, Authenticator } from "@aws-amplify/ui-react";
import { signUp } from "aws-amplify/auth";
