...
- **[Webiny Infrastructure Setup](/{version}/headless-cms/notes-app/webiny-infrastructure-setup)** - The first part focuses on creating the necessary infrastructure on the Webiny side and limiting users' access to only the content they've created.
- **[Building a React Notes Application with the Amplify UI Library](/{version}/headless-cms/notes-app/react-notes-app)** - The second part covers creating a notes React application using the Amplify UI library.
