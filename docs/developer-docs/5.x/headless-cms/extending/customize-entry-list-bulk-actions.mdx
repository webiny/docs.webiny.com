...
export const Extension = () => {
  return (
    <>
      <ContentEntryListConfig>
        <BulkAction name={"copy-json"} element={<ActionCopyJson />} />
      </ContentEntryListConfig>
    </>
  );
};
    ...
    showConfirmationDialog({
      ...
      execute: async () => {
        await worker.processInSeries(async ({ item, report }) => {
          try {
            const response = await fetch("https://any.url.com", {
              method: "POST",
              headers: {
...
export const Extension = () => {
  return (
    <>
      <ContentEntryListConfig>
        <BulkAction name={"sync"} element={<ActionSync />} />
      </ContentEntryListConfig>
    </>
  );
};
...
export const createBulkActionUpdateYear = () => {
  return [
    createBulkAction({
      ...
      dataProcessor: (context: HcmsBulkActionsContext) => new PrependCurrentYearToTitle(context)
    })
  ];
};
```
...
const ActionUpdateYear = () => {
  ...
  const openUpdateYearDialog = () =>
    showConfirmationDialog({
      title: "Update Year",
      message:
        "You are about to update the year for the selected entries. Are you sure you want to continue?",
      loadingLabel: "Processing",
      execute: async () => {
