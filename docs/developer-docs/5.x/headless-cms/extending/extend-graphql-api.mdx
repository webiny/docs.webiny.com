...
Use the [`webiny watch`](/{version}/core-development-concepts/basics/watch-command) command to continuously deploy application code changes into the cloud and instantly see them in action. For quick (manual) testing, you can use the built-in [API Playground](/{version}/admin-area/basics/api-playground).
    ...
    resolvers: {
      Query: {
        listMyPosts: async (_, args: { id: string }, context) => {
          ...
          const [items, meta] = await cms.listLatestEntries(model, {
            where: {
