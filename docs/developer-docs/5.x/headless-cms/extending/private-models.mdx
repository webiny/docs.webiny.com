...
Content models are a main building block in Headless CMS. The first step to storing information in a Headless CMS is to create a [content model](/{version}/user-guides/headless-cms/essentials/glossary#content-model).
...
Alternatively, you can also build public models through code using a [plugin](/{version}/headless-cms/extending/content-models-via-code-storage-id).
...
Use the [`webiny watch`](/{version}/core-development-concepts/basics/watch-command) command to continuously deploy application code changes into the cloud and instantly see them in action.
