...
Run the [`webiny watch`](/{version}/core-development-concepts/basics/watch-command) command to start a new watch session on `apps/admin` application code.
...
3. As the [`webiny watch`](/{version}/core-development-concepts/basics/watch-command) command is already running on `apps/admin`, we should see these changes immediately.
...
```ts apps/api/graphql/src/fields/secretText/secretTextFieldPlugin.ts
...
3. Import this new plugin in `apps/api/graphql/src/index.ts`.
...
4. Deploy the API changes, run the [`webiny watch`](/{version}/core-development-concepts/basics/watch-command) command to start a new watch session on `apps/api/graphql` application code.
...
Notice how we had to run the [`yarn workspace`](https://classic.yarnpkg.com/en/docs/cli/workspace/) command and specify the workspace name (`api-headless-cms`) in order to add the [`cryptr`](https://www.npmjs.com/package/cryptr) NPM package. This is because every Webiny project is organized as a monorepo and can consist of multiple workspaces. To learn more, check out the [Monorepo Organization](/{version}/core-development-concepts/project-organization/monorepo-organization) key topic.
...
Create a file [`secretTextFieldStoragePlugin.ts`](https://github.com/webiny/webiny-examples/blob/master/headless-cms/custom-field-plugin/api/secretTextFieldStoragePlugin.ts) in `apps/api/graphql/src/fields/secretText` directory.

```ts apps/api/graphql/src/fields/secretText/secretTextFieldStoragePlugin.ts
...
```ts apps/api/graphql/src/fields/secretText/secretTextFieldStoragePlugin.ts {5}
const plugin = new StorageTransformPlugin({
  (...)
...
To create a read-only text input field in Webiny's Headless CMS, you need to create a [custom field type](/{version}/headless-cms/extending/custom-field-type).
