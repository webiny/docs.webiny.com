...
Design your data models to support multiple languages if your content will be available in different languages. Webiny offers content localization that allows you to adapt your content to different languages by creating multiple locales. You can learn more about content localization in Webiny [here](/{version}/user-guides/headless-cms/advanced/content-localization).
...
Using Reference Fields, you can only search using the ID or entry ID. For example, let’s say you have two models, Article and Category. Article model has a reference field Category, which refers to the Category model.
