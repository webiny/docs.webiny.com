...
In the 5.x version of Webiny we introduced the `AND` and `OR` conditionals to help users filter the CMS entries.
...
In this example, we've used the `createdOn` entry meta field. To learn more about date/time and identity meta fields, please refer to the [Date/Time and Identity (On/By) Meta Fields](/{version}/headless-cms/references/date-time-and-identity-meta-fields) reference article.
