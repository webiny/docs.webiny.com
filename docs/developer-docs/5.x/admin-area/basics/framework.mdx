...
The `AddRoute` component is described in the dedicated [Custom Routes](/{version}/admin-area/extending/custom-routes) article.

The `AddMenu` component is described in the dedicated [Customize Navigation](/{version}/admin-area/extending/customize-navigation) article.
...
<Alert type={"info"} title={"Extensions"}>
  ...
  We highly recommend using our [Extensions](/{version}/core-development-concepts/basics/extensions) to organize your code in a scalable and portable manner.
