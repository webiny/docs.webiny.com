...
1. Call the Headless CMS Manage API and insert a new record ([view results](/{version}/performance-and-load-benchmark/headless-cms/write-benchmark))
2. Call the Headless CMS Preview API and retrieve a record ([view results](/{version}/performance-and-load-benchmark/headless-cms/read-benchmark))
3. Deliver a page created by the Page Builder ([view results](/{version}/performance-and-load-benchmark/page-builder/deliver-a-page))
...
We deployed Webiny into an AWS region, in our case `eu-west-2` (London). This is the basic vanilla Webiny setup you would get if you follow our [install Webiny tutorial](/{version}/get-started/install-webiny). We ran all 3 tests using this setup. Afterwards we modified one of the infrastructure resources and repeated tests 1 and 2. All the details can be found in the test results.
