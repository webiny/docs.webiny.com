...
This command lets you destroy cloud infrastructure previously deployed within a [project application](/{version}/core-development-concepts/project-organization/project-applications-and-packages). <!-- This can be handy if you want to avoid completely destroying all cloud infrastructure that was deployed by the default project applications, which is what the shown `destroy` command does. -->
...
When deploying into the `prod` environment, some of the cloud infrastructure resources that Webiny deploys for you as part of the [Core project application](/{version}/architecture/core/overview) are marked as [protected](https://www.pulumi.com/docs/intro/concepts/resources/#protect).
...
Once that's in place, run the [`webiny deploy`](/{version}/core-development-concepts/basics/project-deployment) command to apply changes, and finally, run the [`webiny destroy`](/{version}/infrastructure/basics/destroy-cloud-infrastructure) to destroy everything:
