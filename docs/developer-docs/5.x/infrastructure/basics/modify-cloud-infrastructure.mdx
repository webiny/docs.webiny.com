...
Users use the [`webiny deploy`](/{version}/core-development-concepts/basics/project-deployment) command to deploy their Webiny project, into an [environment](/{version}/core-development-concepts/basics/project-deployment#environments) of their choice. During the deployment, apart from building the actual project applications, the command also ensures that a set of required cloud infrastructure resources is deployed.

<Alert type="info">

To deploy necessary cloud infrastructure resources, by default, Webiny relies on Pulumi, a modern infrastructure as code framework. Find out more in the following [IaC with Pulumi](/{version}/infrastructure/pulumi-iac/iac-with-pulumi) key topic.

</Alert>

<Alert type="info">

Read more about the cloud infrastructure resources that get deployed into your AWS account in our [Cloud Infrastructure](/{version}/architecture/introduction) key topics section.
...
Upon running the [`webiny deploy`](/{version}/core-development-concepts/basics/project-deployment) command, when `prod` is passed as the environment name, a Webiny project is deployed in the so-called [production deployment mode](https://www.webiny.com/docs/{version}/architecture/deployment-modes/production).
...
To learn more about the default GraphQL API and differences between it and the Headless CMS GraphQL API, please check out the [Introduction](/{version}/core-development-concepts/extending-and-customizing/extend-graphql-api#introduction) section of the [Extend GraphQL API](/{version}/core-development-concepts/extending-and-customizing/extend-graphql-api) article.

</Alert>

<Alert type="info">

If you want to learn more about the main GraphQL API and how it works on the cloud infrastructure level, check out the [GraphQL Requests](/{version}/architecture/api/graphql-requests) page of the [Cloud Infrastructure - API](/{version}/architecture/api/overview) key topics section.
...
[Amazon OpenSearch Service](https://aws.amazon.com/opensearch-service/) was introduced with Webiny version 5.39.0. If you're using an older version of Webiny, you're using Amazon ElasticSearch Service.
...
The following example shows how we can adjust the configuration of the deployed domain when using Amazon ElasticSearch Service (for projects created before the 5.39.0).
...
Note that the configuration must be applied within both `webiny.application.ts` files. Once applied, in order for the changes to take effect, redeploying the **Core** and **API** applications is required. As usual, this can be done via the [`webiny deploy`](/{version}/core-development-concepts/basics/project-deployment) command:
...
A single Webiny project deploys four Amazon CloudFront distributions: one for [API](/{version}/architecture/api/overview), one for [Admin](/{version}/architecture/admin/overview), and two for [Website](/{version}/architecture/website/overview) project application.
