...
While executing deployment-related commands via the [Webiny CLI](/{version}/core-development-concepts/basics/webiny-cli), for example [`deploy`](/{version}/core-development-concepts/basics/project-deployment) or [`destroy`](/{version}/infrastructure/basics/destroy-cloud-infrastructure), if not specified, Webiny will always use the `default` AWS profile.
