...
To upgrade to **Business** tier, users [link their project](/{version}/wcp/link-a-project) with [Webiny Control Panel (WCP)](/{version}/wcp/overview), from where they can activate the Advanced Access Control Layer (AACL) for their project. By doing this, users will be able to define fine-grained permissions for individual Webiny apps.
...
<Alert>
  ...
  To learn more about all of the enterprise features, check out the [Webiny Enterprise](/{version}/enterprise/multi-tenancy) section.
