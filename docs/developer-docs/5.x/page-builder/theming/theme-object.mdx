...
To learn more about custom page elements, check out the existing [Create a Custom Page Element](/{version}/page-builder/extending/create-a-page-element)
article.
...
While styling [page layouts](/{version}/page-builder/theming/page-layouts) or [custom page elements](/{version}/page-builder/extending/create-a-page-element), the theme object can be accessed in order to use the styling rules defined in it. To access the theme object, we rely on Emotion's suggested approach, which is through the React context.
...
More information on page layouts can be found in the following [Page Layouts](/{version}/page-builder/theming/page-layouts) article.
...
If needed, theme object can also be accessed when creating [custom page elements](/{version}/page-builder/extending/create-a-page-element), via the `useRenderer` React hook. For example:
...
Out of the box, every Webiny project starts with a single theme (the one located in the [`extensions/theme`](https://github.com/webiny/webiny-js/tree/v5.40.0/packages/cwp-template-aws/template/common/extensions/theme) folder. For a multi-theme setup, please take a look at the [Setup Theme Manager](/{version}/enterprise/theme-manager) article.
