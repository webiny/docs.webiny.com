...
With this feature, users can define access permissions on a folder level. More specifically, users that are designated as **folder owners** can now define which users (and also teams with the [Teams](/{version}/enterprise/aacl/teams) feature enabled) can access which folders, and what actions they can perform on those folders:
...
For Webiny Enterprise users, apart from [linking their Webiny project](/{version}/wcp/link-a-project) with Webiny Control Panel (WCP), there are no additional steps required to enable Folder Level Permissions.
...
Folder Level Permissions feature is an extension of the existing [security layer](/{version}/enterprise/aacl/introduction#the-three-tiers-of-webiny-s-security-layer). It does not replace it.
