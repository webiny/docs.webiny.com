...
To upgrade to **Business** tier, users [link their project](/{version}/wcplink-a-project) with [Webiny Control Panel (WCP)](/{version}/wcpoverview), from where they can activate the Advanced Access Control Layer (AACL) for their project. By doing this, users will be able to define fine-grained permissions for individual Webiny apps.
...
For Webiny Enterprise users, apart from [linking their Webiny project](/{version}/wcp/link-a-project) with Webiny Control Panel (WCP), there are no additional steps required to enable Teams.
