...
When a Webiny project is deployed in [production mode](/{version}/architecture/deployment-modes/introduction), it is deployed into a new [Amazon Virtual Private Cloud (Amazon VPC)](https://aws.amazon.com/vpc/), with already configured public and private subnets, security groups, and so on.
...
The following examples were created using a brand new Webiny project, initialized with the [Amazon DynamoDB + Amazon OpenSearch database setup](/{version}/architecture/introduction#different-database-setups). That's why we'll encounter the `openSearch` flag (set to `true`) in Core and API applications' `webiny.application.ts` configuration files.
...
Once the above changes have been applied, we’re ready to do a test deployment, which can be done via the [`webiny deploy`](/{version}/core-development-concepts/basics/project-deployment) command:
