...
<CanIUseThis enterprise={true} since={"5.x"} />
...
Your project needs to be at version `5.x`, or greater, to use this feature.
Please follow the [upgrade guide](/release-notes/upgrade-webiny) to upgrade your project to the appropriate version.

Alternatively, you can create a new `>=5.x` project, by running:
...
If you need to use multi-tenancy, follow [this guide](/{version}/enterprise/multi-tenancy) before continuing with the Auth0 setup.
