...
Check out the [Deploy Your Project](../../core-development-concepts/basics/project-deployment) guide to learn more about the [`deploy`](../../core-development-concepts/basics/project-deployment#the-deploy-command) command and how to deploy your entire Webiny project. Also, if you'd like to learn more about the deployment process in general, you can also visit our [Deployment](../../infrastructure/basics/introduction) key topics section.

</Alert>

### Webiny Environment Variables

Note that, upon executing deployment-related commands, the [Webiny CLI](../../core-development-concepts/basics/webiny-cli) automatically injects the following two environment variables into the running process:
...
+ WEBINY_MY_CUSTOM_ENV_VARIABLE=some-random-value
```

From there, we can easily add additional environment variables, like we did with the `WEBINY_MY_CUSTOM_ENV_VARIABLE`. Note that all variables with the `WEBINY_` prefix will be automatically added to all deployed AWS Lambda functions.
...
```diff-js apps/website/webiny.config.ts
(...)
...
The `DEBUG` environment variable can be set via the application's `webiny.config.ts` config file, for example, [`apps/admin/webiny.config.ts`](https://github.com/webiny/webiny-js/blob/next/apps/admin/webiny.config.ts) or [`apps/website/webiny.config.ts`](https://github.com/webiny/webiny-js/blob/next/apps/website/webiny.config.ts):
...
Behind the scenes, [Webiny CLI](../../core-development-concepts/basics/webiny-cli) uses the [`dotenv`](https://www.npmjs.com/package/dotenv) library in order to load the shown `.env` file. Note that the values that are defined in the `.env` file will not get assigned as environment variables if they were already assigned in the running process (e.g. via terminal, or as a secret in your CI/CD workflow). This is the default behaviour of the `dotenv` library.

### What are the commands that will load the root `.env` file?

The following commands load the `.env` file: [`deploy`](../../core-development-concepts/basics/project-deployment#the-deploy-command), [`destroy`](../../infrastructure/basics/destroy-cloud-infrastructure#the-destroy-command), [`watch`](../../core-development-concepts/basics/watch-command), and [`pulumi`](../../infrastructure/pulumi-iac/execute-pulumi-commands).
