...
- `OptionMenuLink`: takes the user to a new location, such as a new web page
...
To position your custom folder action before or after an existing action, you can use the `before` and `after` props on the `<Browser.EntryAction>` element:
