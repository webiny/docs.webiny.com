...
Also, the `webiny.application.ts` configuration file, and, more specifically, via the `indexPrefix` property in the `elasticSearch` configuration section, it's also possible to prefix all index names with a custom prefix. To learn more, check out the [Using a Shared Amazon OpenSearch Service Domain](/docs/{version}/infrastructure/basics/modify-cloud-infrastructure#using-a-shared-amazon-open-search-service-domain) section.
