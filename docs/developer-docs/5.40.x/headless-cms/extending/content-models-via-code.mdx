...
<ExtensionsGettingStarted
  ...
  scaffoldCommandExtraInfo={<>Since we'll be using a couple of utilities from the <code>@webiny/api-headless-cms</code> package, we've immediately included it in the list of dependencies.</>}
/>

## Basic Example

In this example, we show how to define a simple **Product** content model, that belongs to the **E-Commerce** content model group.

### Create Content Model Group and Content Model Plugins

To create the content model group and content model, we use `createCmsGroupPlugin` and `createCmsModelPlugin` plugin factories, as shown in the following code snippet.
...
As we can see, upon calling the `createCmsGroupPlugin` plugin factory, we get to define a content model group, which consists of the following properties:
