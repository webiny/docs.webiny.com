...
<ExtensionsGettingStarted type={"api"} name={"models"} dependencies={"@webiny/api-headless-cms"} />
...
To create the content model group and content model, we use `createCmsGroupPlugin` and `createCmsModelPlugin` plugin factories.
...
With this code in place, we should be able to see the following two items in our Admin Area main menu:

![Product and E-Commerce Items in the Admin Area Main Menu](./assets/content-models-plugins/basics-plugins-work.png)

As shown in the example, the `createCmsModelPlugin` lets us define all of the content model's properties, like its name, ID (`modelId`), a content model group to which it belongs to, and most importantly, all of the fields that it consists of.

All of the fields of a single content model are defined via the `fields` property, which is an array of [`CmsModelField`](https://github.com/webiny/webiny-js/blob/c3d4955bf74e7ffdb9628867e3b23cdfe64ea8dc/packages/api-headless-cms/src/types.ts#L109) objects, which we create via the `createModelField` factory function.
...
```ts
const myModel = createCmsModelPlugin({
  modelId: "myModel",
  fields: [
    {
      id: "id",
