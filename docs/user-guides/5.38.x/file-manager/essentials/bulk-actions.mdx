...
3. In the root folder, over the **Men's Red Tshirt** image we uploaded in the [File Upload](../file-manager/upload-file) tutorial, click the **SELECT** icon (☑️).

    ✔️ A file action bar appears below the search bar.

4. Similarly, select the **Men's Black Tshirt** image we uploaded in the [Tagging Files](../file-manager/tagging-files) tutorial.
