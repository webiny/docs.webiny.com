...
Add a React component file under `src/editorComponents` (or a subfolder). In this tutorial we will create `MyBlock` component.
...
```diff-tsx src/editorComponents/MyBlock.tsx
...
- Pick an existing group (`custom`, `basic`, `sample`) or add a new one in `initializeContentSdk.ts`. 
