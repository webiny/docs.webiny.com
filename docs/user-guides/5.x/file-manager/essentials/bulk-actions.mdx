...
2. In the root folder, create a folder titled **Tshirts**.
   ...
   **Note**: If you are not familiar with how to create a folder, please follow the [Organizing Files](/{version}/user-guides/file-manager/essentials/organizing-files) tutorial.

3. In the root folder, over the **Men's Red Tshirt** image we uploaded in the [File Upload](/{version}/user-guides/file-manager/essentials/upload-file) tutorial, click the **SELECT** icon (☑️).

   ✔️ A file action bar appears below the search bar.

4. Similarly, select the **Men's Black Tshirt** image we uploaded in the [Tagging Files](/{version}/user-guides/file-manager/essentials/tagging-files) tutorial.

![Bulk Actions](./assets/bulk-actions/selected-files.png)

5. In the file action bar, click the **MOVE _N_ FILES** icon (📁).
