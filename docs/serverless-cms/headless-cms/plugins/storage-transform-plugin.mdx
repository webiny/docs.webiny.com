...
If you have a `extremelyLargeText` field type, which can get really large (couple of megabytes). You might want that it is not stored into the database (DynamoDB for example, as it has 400 kilobytes record limit).
...
Be aware that in case of `extremelyLargeText` you cannot search the text but only the identifier created when copying data to another storage system.
