...
export class Navigation {
    ...
    private async applyPageModifiers(
        groups: NavigationTree["items"]
    ): Promise<NavigationOutputData> {
        const promises: Promise<void>[] = [];
        const map = new Map<string, NavigationOutputPage>();

        const tempOutput = this.traversePages<NavigationPage, WithId<NavigationPage>>(
            groups,
            page => {
                const id = mdbid();
                promises.push(
