...
![The New React Application](./assets/react-application/preview.png)
...
![Cloud Infrastructure Resources](./assets/react-application/new_react_app_scaffold.png)
...
But, once it's time to deploy it, you can do that as usual, with the [`webiny deploy`](../../core-development-concepts/basics/project-deployment) command.
...
```bash New React Application Folder (Some Files Removed for Brevity)
├── code
...
In case you missed it, every Webiny project is organized as a [monorepo](../../core-development-concepts/project-organization/monorepo-organization), which can consist of multiple packages. This is the reason why the [`package.json`](https://github.com/webiny/webiny-js/blob/v5.13.0/packages/cli-plugin-scaffold-react-app/template/code/package.json) manifest file exists. It exists because the `code` folder represents a monorepo [workspace](https://classic.yarnpkg.com/en/docs/workspaces/).
...
Which means no initial deployment is needed. Once you've completed the scaffold's wizard and the files have been created, in order to start developing, you can simply start by running the [`webiny watch`](../../core-development-concepts/basics/watch-command) command (see below). Only when you actually want to see your application online, you can proceed as usual, by running the [`webiny deploy`](../../core-development-concepts/basics/project-deployment) command.
...
```tsx code/src/plugins/routes/myNewPage.tsx
...
But, unless the new React application was created with the [New Full-Stack Application](../../core-development-concepts/scaffolding/full-stack-application) scaffold, you will have to manually link your React application with an existing GraphQL API.
...
In case you missed it, every Webiny project is organized as a [monorepo](../../core-development-concepts/project-organization/monorepo-organization), which can consist of multiple packages. This is the reason why the [`code/package.json`](https://github.com/webiny/webiny-js/blob/v5.13.0/packages/cli-plugin-scaffold-react-app/template/code/package.json) manifest file exists. It exists because the `code` folder represents a monorepo [workspace](https://classic.yarnpkg.com/en/docs/workspaces/).
