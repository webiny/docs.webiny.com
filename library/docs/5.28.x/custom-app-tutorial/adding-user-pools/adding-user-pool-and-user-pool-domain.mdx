...
In case you missed it, check out the [dev and prod Stacks](.../../overview/features/security-overview/get-started#dev-and-prod-stacks) section.
...
```diff-ts pinterest-clone/api/pulumi/dev/cognito.ts
...
First, in order for the new cloud infrastructure code to be actually executed, we'll instantiate the imported `Cognito` class. Second, we will modify the list of returned values, which, as previously mentioned, represents the [stack output](../../custom-app-tutorial/security-overview/get-started#stack-output).
...
// https://www.webiny.com/docs/5.28.x/core-development-concepts/basics/environment-variables#debug-environment-variable
...
If you had the [`webiny watch`](../../core-development-concepts/basics/watch-command) command up and running while making the shown changes, the new cloud infrastructure resources should have been deployed automatically for you:

![Automatic Re-deployment of Cloud Infrastructure](./assets/adding-user-pool-and-user-pool-domain/cloud-infra-auto-deploy.png)

If not, you can either start a new watch session, or simply deploy the changes via the [`webiny deploy`](../../core-development-concepts/basics/project-deployment) command.

![Automatic Re-deployment of Cloud Infrastructure](./assets/adding-user-pool-and-user-pool-domain/cloud-infra-deploy.png)
