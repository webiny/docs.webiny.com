...
```typescript
function validatePairing(mdxFiles: string[], aiTxtFiles: string[]): ValidationResult {
...
function printResults(result: ValidationResult): number {
  const hasIssues = result.mdxWithoutAiTxt.length > 0 || result.aiTxtWithoutMdx.length > 0;
