...
Webiny in its Enterprise tier, offers an additional infrastructure configuration, one that additionally hardens the security posture and keeps all the communication between Webiny services within the private network without exposing any of its services to the public internet. To enable such a secure setup Webiny uses VPC gateway endpoints for DynamoDB, S3 and Lambda, and VPC interface endpoints for Opensearch, Cognito and Clourfront. In such a setup Lambda functions and their communication with other services are all kept within a private network that is not exposed to the outside internet.
In this setup, in case you need to additionally protect your Webiny so that only clients from within your AWS cloud can access it, a VPC peering connection can be established, keeping all the traffic within your network, including the client that's accessing your Webiny instance. 
