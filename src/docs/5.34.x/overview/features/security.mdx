...
To learn more about Webiny's architecture, check out [this article](../../architecture/introduction).
...
We highly recommend that you use the production deployment mode for all your projects running production traffic. [Learn more about the deployment modes.](../../architecture/deployment-modes/introduction)
...
Webiny in its Enterprise tier, offers an additional infrastructure configuration, one that further hardens the security posture and keeps the communication between specific services within the private network without exposing any of its data or communication to the public internet. 

Webiny Enterprise customers can deploy Webiny within their existing VPC that they already manage using their existing toolset. 

In addition to deploying Webiny inside a VPC, VPC endpoints can be used with services like DynamoDB (gateway endpoint), S3 (gateway endpoint), SQS (interface endpoint) and EventBridge (interface endpoint). 

As many organizations already have a securely configured VPC and VPC endpoints, Webiny can be configured to utilize those existing resources. If the preference is to deploy Webiny into an isolated AWS account, but still utilize the VPC and VPC endpoints, that configuration is also available. 
