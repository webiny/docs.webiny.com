...
At the moment, `AND` and `OR` conditionals are working only on the DynamoDB + Elasticsearch systems, as it is quite complex to implement it for the DynamoDB only systems.
...
As you can notice, having more than one filter in an `OR` block produces the match all required in that `OR` block.
...
You can notice that we could have actually written the query without the nested `OR` conditional, but to have an example we did it like this.
...
While users can nest `AND` and `OR` conditionals indefinitely, they need to be aware that it might result in performance issues.
