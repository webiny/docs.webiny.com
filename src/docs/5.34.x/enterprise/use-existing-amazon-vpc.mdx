...
The following examples were created using a brand new Webiny project, initialized with the [Amazon DynamoDB + Amazon ElasticSearch database setup](/docs/{version}/architecture/introduction#different-database-setups). That's why we'll encounter the `elasticSearch` flag (set to `true`) in Core and API applications' `webiny.application.ts` configuration files.  
...
Changing VPC settings on an existing Amazon ElasticSearch cluster may cause a complete redeployment, causing all data to be lost! Check out the [Launching your Amazon OpenSearch Service domains within a VPC](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/vpc.html) article to learn more.
...
export default createCoreApp({
    ...
    elasticSearch: true, // Only present in Amazon DynamoDB + Amazon ElasticSearch database setup.
+   vpc: {
+       useExistingVpc: {
