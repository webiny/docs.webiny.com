...
The following examples were created using a brand new Webiny project, initialized with the [Amazon DynamoDB + Amazon ElasticSearch database setup](/docs/{version}/architecture/introduction#different-database-setups). That's why we'll encounter the `elasticSearch` flag, set to `true`. 
  
  Projects that are using the Amazon DynamoDB setup don't have this parameter present in Core and API application's `webiny.application.ts` configuration file.
