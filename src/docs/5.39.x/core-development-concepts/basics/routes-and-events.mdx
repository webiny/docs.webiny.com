...
The underlying event handlers, which the main `createHandler` chooses, actually initialize Webiny system.
...
const handler = createHandler({
  plugins: [
    ...
    createDynamoDBEventHandler(async ({ request, reply, event, context, lambdaContext }) => {
      const result = await yetAnotherHeavyCalculation(event);
...
#### Multiple Event Handlers of a Single Type in a Single Handler
...
<Alert type="danger">
  You MUST call `await next();` in your event handler, otherwise the next event handler will not be executed.
</Alert>
...
const handler = createHandler({
  plugins: [
    ...
    createDynamoDBEventHandler(async ({ request, reply, event, context, lambdaContext, next }) => {
      await next();
      const result = await anotherHeavyCalculation(event);
