...
4. Deploy the API changes, run the [`webiny watch`](../../core-development-concepts/basics/watch-command) command to start a new watch session on `apps/api/headlessCMS` application code.
...
import { StorageTransformPlugin } from "@webiny/api-headless-cms/plugins/storage/StorageTransformPlugin";
