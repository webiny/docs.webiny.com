...
### Sanitize HTML in `@webiny/react-rich-text-renderer` package ([#3477](https://github.com/webiny/webiny-js/pull/3477))

We have improved the security of our [@webiny/react-rich-text-renderer](https://www.npmjs.com/package/@webiny/react-rich-text-renderer) package. 
When rendering the rich text content, we’re now sanitizing the HTML using the [sanitize-html](https://www.npmjs.com/package/sanitize-html) package, 
to prevent the possibility of an XSS attack. 

Sanitization can be completely customized, using global configuration, or it can be configured on 
