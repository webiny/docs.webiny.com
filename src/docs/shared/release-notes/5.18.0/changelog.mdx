...
Please check the [Webiny 5.18.0 migration guide](/docs/{version}/release-notes/5.18.0/upgrade-guide) for the upgrade steps.
...
For more installation-related information, you can also visit the updated [Install Webiny](/docs/{version}/tutorials/install-webiny) tutorial.
...
A complete overhaul was done on the most important development-related Webiny CLI commands - [`webiny deploy`](../../core-development-concepts/basics/project-deployment) and [`webiny watch`](../../core-development-concepts/basics/watch-command). In short, you can expect both of these to be faster and also consume significantly less system resources.
...
For more information on this change, make sure to check out the [Additional Information](/docs/{version}/release-notes/5.18.0/upgrade-guide#additional-information) section in the [Upgrade Guide](/docs/{version}/release-notes/5.18.0/upgrade-guide).

### Pulumi Command - Improved Handling of `--json` Flag

Previously, when invoking the Pulumi CLI via the [`webiny pulumi`](../../core-development-concepts/basics/webiny-cli#yarn-webiny-pulumi-folder---env-env----command) command and passing the `--json` flag to it, the final output would still include informative messages:
...
Learn more about remote runtime logs (logs forwarding) in the [Use the Watch Command](../../core-development-concepts/basics/watch-command#enabling-logs-forwarding) how-to guide.
...
To learn more more about it, please read our [Lifecycle Events](../../headless-cms/references/lifecycle-events) key topic.
...
We've expanded our existing [Create Custom Application](../../custom-app-tutorial/overview/introduction) tutorial with the new [Security](../../custom-app-tutorial/security-overview/introduction) section!
