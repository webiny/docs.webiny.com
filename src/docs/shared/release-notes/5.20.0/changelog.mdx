...
To learn more about creating content models via plugins, check out the [Define Content Models via Plugins](/docs/{version}/how-to-guides/webiny-applications/headless-cms/content-model-plugins/) guide.
...
From time to time, users may need to re-render a particular page they've previously created and published. This can now be done easily via the built-in [API Playground](/docs/{version}/how-to-guides/webiny-applications/admin-area/api-playground/), by executing a simple `rerenderPage` GraphQL mutation. For example:
...
For more details on all of the available events, please see the dedicated [Lifecycle Events](../../page-builder/references/lifecycle-events) guide.
...
In a couple of scenarios, upon running the [`webiny watch`](/docs/{version}/how-to-guides/use-watch-command/) command, adding the `--remoteRuntimeLogs` (`-r`) argument would not properly return runtime logs in user's terminal. This has now been addressed.

### Fixed React Component Scaffold's `webiny.config.ts` File ([#2123](https://github.com/webiny/webiny-js/pull/2123))

The [`webiny.config.ts`](https://github.com/webiny/webiny-js/blob/next/packages/cli-plugin-scaffold-react-component/template/webiny.config.ts) file that is generated when running the React Component scaffold has been updated. It now reflects the changes done in the recent [5.18.0](/docs/{version}/release-notes/5.18.0/changelog) release.

### Two New `webiny deploy` Hooks ([#2079](https://github.com/webiny/webiny-js/pull/2079))

You can now hook onto the pre-build and post-build events that are emitted during the [`webiny deploy`](/docs/{version}/how-to-guides/deployment/deploy-your-project/) command. More information about all related hooks can be found in the new [Build and Deploy Hooks](../../infrastructure/basics/build-and-deploy-hooks) guide.

## Documentation

### Updated All Mentions of `webiny.config.ts` Files ([#366](https://github.com/webiny/docs.webiny.com/pull/366))

All occurrences of `webiny.config.ts` configuration files have been updated to reflect the changes done in the recent [5.18.0](/docs/{version}/release-notes/5.18.0/changelog) release.
