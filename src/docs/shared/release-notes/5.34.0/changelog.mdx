...
import pe from "./assets/pes3.png";
...
### Page Elements: The Brand New Page Rendering Engine ([#2898](https://github.com/webiny/webiny-js/pull/2898))

<Image src={pe} title={"Page Elements: The Brand New Page Rendering Engine"} shadow={false}/>
...
for them. Until then, their projects will still use the now _legacy_ rendering engine, which we plan to support until the end of June 2022. After that, the legacy rendering engine will be considered deprecated and will receive no further support from the Webiny team.
