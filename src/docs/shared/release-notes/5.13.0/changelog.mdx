...
Please check the [Webiny 5.13.0 migration guide](/docs/{version}/release-notes/5.13.0/upgrade-guide) for the upgrade steps.
...
- [Full Stack Application](../../core-development-concepts/scaffolding/full-stack-application)
- [GraphQL API](../../core-development-concepts/scaffolding/graphql-api)
- [React Application](../../core-development-concepts/scaffolding/react-application)
...
The existing guides that cover extending GraphQL APIs were also updated, so make sure to [check them out](../../core-development-concepts/extending-and-customizing/extend-graphql-api) to learn more:
...
Check out the [Use AWS Profiles](/docs/{version}/how-to-guides/deployment/aws/use-aws-profiles/) how-to guide to learn how to use different AWS profiles upon deploying your Webiny project.

</Alert>

### Fix `watch` on Windows ([#1848](https://github.com/webiny/webiny-js/pull/1848))

There's currently a bug in the Pulumi path detection for file watching, so on Windows, where paths start with **C:**, etc., Pulumi thinks it's a relative path, and constructs a wrong file path to watch. To bypass this issue, and get the [watch](/docs/{version}/how-to-guides/use-watch-command/#webiny-watch-command) command to work on Windows, we've temporarily disabled path filtering on Windows.
...
With the introduction of [Webiny UI Composer](/docs/{version}/key-topics/ui-composer/introduction), in Admin Area, navigation menu items are now rendered using the new concept of [renderers](/docs/{version}/key-topics/ui-composer/creating-elements#enabling-support-for-renderers). Unfortunately, it did not include a renderer for links as a direct child of a menu group. With this release, we've improved one of the renderers to handle both 2-level and 3-level menu hierarchy.

<Alert type="info">

Users who used the [Extend Admin Area](../../admin-area/new-app-tutorial/scaffolding) scaffold were the first to experience this issue, as the main-menu items that were created during the scaffolding process simply weren't clickable.
