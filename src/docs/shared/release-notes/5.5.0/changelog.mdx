...
Please check the [Webiny 5.5.0 migration guide](/docs/{version}/release-notes/5.5.0/upgrade-guide) for the upgrade steps.
...
Let’s say you wanted to expand the default GraphQL API, that’s deployed as part of the [API](../../architecture/api/introduction) project application. At the moment, in order to do that, you’d have to do the following:

- start a watch session on relevant packages using the [`webiny workspaces run`](/docs/{version}/how-to-guides/working-with-workspaces) command
...
To learn more about the new `webiny watch` command, check out the newly added [Use the Watch Command](../../core-development-concepts/basics/watch-command) guide.
...
Note that, if upgrading to version `5.5.0`, there are a couple of [manual steps](/docs/{version}/release-notes/5.5.0/upgrade-guide#additional-manual-upgrade-steps) that need to be done in order for these fixes to be fully applied.
...
All the changes can be automatically applied in your existing Webiny project via the new `webiny upgrade` command. Please take a look at the [upgrade guide](/docs/{version}/release-notes/5.5.0/upgrade-guide) for more information.

</Alert>

#### 1. `.pulumi` Folders Moved To Project Root

By default, the Pulumi cloud infrastructure [state files](../../infrastructure/pulumi-iac/iac-with-pulumi#state-files) are stored locally - in `.pulumi` folders located in your project applications folders:
...
As noted in the [Overview](../../release-notes/upgrade-webiny#-precaution-measures) section, please test your changes in pre-production environments first.
...
Learn more about project applications in the [Project Applications](../../core-development-concepts/project-organization/project-applications) guide.
