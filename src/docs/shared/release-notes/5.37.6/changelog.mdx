...
With this release, we've enabled developers to configure federated identity providers on the Cognito setup used for Admin app login. We now support logging in via Google, Facebook, Amazon, and Apple. Custom OIDC providers are not yet supported.
