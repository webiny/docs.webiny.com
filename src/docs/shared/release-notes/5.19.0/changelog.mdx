...
`@pulumi/*` NPM packages are being utilized by Pulumi, the underlying infrastructure-as-code (IaC) solution Webiny relies on. To learn more, check out the [Infrastructure as Code With Pulumi](../../infrastructure/pulumi-iac/iac-with-pulumi) key topic.
...
In order to use the [`webiny deploy`](../../core-development-concepts/basics/project-deployment) command, AWS credentials need to be set, which can be done in [multiple ways](../../infrastructure/aws/configure-aws-credentials).
...
A new `--deploy` flag has been added to the [`webiny deploy`](../../core-development-concepts/basics/project-deployment).
