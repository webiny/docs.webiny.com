...
Note that if you've been using custom Babel plugins for Emotion, you will need to update them to the latest version as well. 
...
The second change is the way typography is accessed. Previously, you could access typography styles via the `styles.typography` object, and then by using the typography variant as a key, for example `theme.styles.typography.heading1`.
