...
Make sure to check out the [5.35.0 changelog](./changelog) to get familiar with the changes introduced in this
release.
...
As stated in the [Upgrade Webiny](../upgrade-webiny#pre-production-environments-first) section, we recommend that you
...
Environments](../../core-development-concepts/ci-cd/environments) key topic.
...
#### Custom Babel Plugins For Emotion
...
<Alert type={"info"}>
  ...
  The [upgrade command](#2-run-the-upgrade-command) will try to detect usage of custom Babel plugins for Emotion. If detected, it will print the following message in terminal: **Warning: Found custom Emotion plugins in your project. Please make sure to update them to the latest version.**

</Alert>

### Website Theme Object

The upgrade command will perform a couple of code level updates related to the central website theme object. The following sections will explain what changes are made, and how you can manually apply them if the upgrade command fails to do so.

#### Updated Theme Object Structure

With this release, we are [updating the structure of the theme object](/docs/release-notes/5.35.0/changelog#updated-theme-object-structure-3138).

<br/>
<br/>

##### **1. `styles.typography` Object**

The first change is related to the `styles.typography` object, which was previously an object where keys represented the typography variant (heading1, heading2, etc.), and values were objects representing the actual typography styles. For example:
...
From now on, the `styles.typography` object is an object where keys represent the typography type (heading, paragraph, etc.), and values are arrays of objects, where each object represents a specific typography variant. Each typography variant is an object that consists of four properties: `id`, `name`, `tag`, and `styles`.
...
The upgrade command will automatically update the `styles.typography` object in your `theme/theme.ts` file. Note that if you've added custom typography variants, the upgrade might not be able to update the `styles.typography` object correctly. In that case, you will need to manually update the `styles.typography` object in your `theme/theme.ts` file.

##### **2. Accessing Typography Styles**

The second change is the way typography is accessed. Previously, you could access typography styles via the `styles.typography` object, and then by using the typography variant as a key, for example:
...
#### Form Layot Upgraded completely

### Other

#### Warning: We couldn't map your custom key KEY_NAME to the new structure, please migrate manually.

If you're seeing **We Couldn't Map Your Custom Key {KEY_NAME} to the New Structure, Please Migrate manually.** warning,
