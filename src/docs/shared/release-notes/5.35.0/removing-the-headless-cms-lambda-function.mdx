...
With the [5.35.0 release](/docs/release-notes/5.35.0/changelog), we've removed the Headless CMS AWS Lambda Function, and it is a breaking change, although quite simple one to fix.
...
We removed the Headless CMS AWS Lambda Function from both the new project codebase and the Pulumi deployment code.
...
If you have custom code or plugins in the `apps/api/headlessCMS` folder, yes, you will need to do some changes.
...
If you have not changed anything, you can skip this article.
