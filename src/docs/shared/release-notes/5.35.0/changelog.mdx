...
In 5.35.0, we're removing the Headless CMS Lambda function from user's projects. This is a breaking change, but it is quite an easy one to migrate.

</Alert>

Starting with the `5.35.0` release, there is only one Lambda Function which handles the requests, the one called GraphQL (`apps/api/graphql`).
We removed the Headless CMS Lambda Function to simplify the development and deployment process for the users, and make it much easier to add plugins which interact with the entire system, and hook into all the lifecycle hooks Webiny has to offer.

To learn how to migrate your project, please follow the [Upgrade Guide](/docs/release-notes/5.35.0/upgrade-guide).
...
If you are using the `listTags` query directly, you will need to update your code to use the new `data, error` envelope, as well as the `tag` and `count` fields.
