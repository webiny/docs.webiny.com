...
### Listing File Tags ([#3227](https://github.com/webiny/webiny-js/pull/3227))

There was a bug when listing The File Tags in the DynamoDB + Elasticsearch deployments - not all tags were listed.

Also, we have upgraded the File Manager `listTags` GraphQL query to return the `tag` value and the `count` value, which represents the number of files that have the tag assigned.
...
<Alert type="warning">
  ...
  If you are using the `listTags` query directly, you will need to update your code to use the new `tag` and `count` fields.
