...
Please check the [Webiny 5.16.0 migration guide](/docs/{version}/release-notes/5.16.0/upgrade-guide) for the upgrade steps.
...
To deploy necessary cloud infrastructure, by default, Webiny relies on [Pulumi](../../infrastructure/pulumi-iac/iac-with-pulumi), a modern infrastructure as code framework.
...
For starters, we've introduced the `--allow-production` flag for the [`webiny watch`](../../core-development-concepts/basics/watch-command) command, which allows the user to run the command even with `prod` and `production` environments (by default, the command doesn't allow that).

Speaking of the [`webiny watch`](../../core-development-concepts/basics/watch-command) command, we've also made the logs displayed in the **Deploy** pane a bit cleaner and nicer:

![webiny watch Command - The Improved Deploy Pane](./changelog/watch-deploy-pane-improvement.png)

Finally, the [Full Stack Application](/docs/{version}/how-to-guides/scaffolding/full-stack-application/) scaffold should now work correctly on Windows operating system.
