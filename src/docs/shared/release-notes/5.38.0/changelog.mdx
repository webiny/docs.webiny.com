...
Users can create custom search queries based on model-defined fields that are categorized as one of the following data types: 
...
In addition to creating complex search queries, users can save and manage their search filters, making it easier to access and reuse them in the future.
