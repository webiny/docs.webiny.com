...
### Check basic permissions for ACO schemas ([#3296](https://github.com/webiny/webiny-js/pull/3296))
...
Apart from the new frontend user interface, this new feature also introduced multiple new GraphQL queries and mutations on the backend GraphQL API. Unfortunately, prior to this release, these operations were unintentionally left accessible and open to non-authenticated users.
...
<Alert type="danger">
  ...
  Because having the mentioned GraphQL operations open to non-authenticated users might pose a data security risk to your organization, we strongly advise all users to [upgrade to 5.35.1](/docs/release-notes/5.35.1/upgrade-guide) as soon as possible.
