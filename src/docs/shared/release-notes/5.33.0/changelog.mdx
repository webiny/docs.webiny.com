...
<Image src={cmsContext} title="Type-safety and Auto-Complete When Accessing the `cms` Context Property"/>
...
At the moment, the multi-tenancy feature is enabled as soon as a Webiny project is linked with Webiny Control Panel (WCP). Additionally, for older projects (versions 5.28.0 and lower), this can also be achieved by setting the  `WEBINY_MULTI_TENANCY` environment variable to `true`. 

But, upon performing additional tests, we've noticed that this environment wasn't being taken into consideration upon deploying the **Website** app. 

This has now been addressed. If you're relying on the `WEBINY_MULTI_TENANCY` environment variable to enable multi-tenancy, the **Website** app should now also work as expected.
