...
### Website Project Application - Introduce `previewDomains` Parameter ([#2670](https://github.com/webiny/webiny-js/pull/2670))

On a couple of occasions, we've seen users have the need to specify a custom domain not only for the public-facing website, but also for the internal React application, used for prerendering purposes.

Using the newly introduced `previewDomains` parameter, the internal React application can now also be linked with a custom domain. It's as easy as doing the following:
...
On a couple of occasions, we've seen users having the need to conditionally apply different configurations within their project's cloud infrastructure (Pulumi) code. While this was possible prior to this release, it did require a bit of code restructuring in the `webiny.application.ts` file. But that is no longer the case.  
...
At the moment, the multi-tenancy feature is enabled as soon as a Webiny project is linked with Webiny Control Panel (WCP). But, for backwards compatibility purposes, this can also be achieved by setting the  `WEBINY_MULTI_TENANCY` environment variable to `true`. 

But, upon performing additional tests, we've noticed that this environment wasn't being taken into consideration upon deploying the **Website** project application. 

This has now been addressed. If you're relying on the `WEBINY_MULTI_TENANCY` environment variable to enable multi-tenancy, the **Website** project application should now also work as expected.
...
Prior to this release, all AWS Lambda functions would receive the `AWSLambdaVPCAccessExecutionRole` IAM policy, even if the Webiny project hasn't been deployed with the VPC option enabled.

This issue has been addressed. Upon doing a new deployment, you can expect to see your `AWSLambdaVPCAccessExecutionRole` IAM policies to be replaced with the basic `AWSLambdaBasicExecutionRole` IAM policy.
...
Upon running the [`webiny deploy`](/docs/{version}/how-to-guides/deployment/deploy-your-project/) command, if a user experienced an AWS auth-related error, it was hard to debug it. Mainly because the actual underlying error wasn't printed in the terminal, even in the presence of the `--debug` flag. 
