...
Upon running the [`webiny deploy`](/docs/how-to-guides/deployment/deploy-your-project/) command, if a user experienced an AWS auth-related error, it was hard to debug it. Mainly because the actual underlying error wasn't printed in the terminal, even in the presence of the `--debug` flag.
