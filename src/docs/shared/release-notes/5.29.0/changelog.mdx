...
> If there are only 2 AWS Lambda functions in my [API project application](/docs/{version}/architecture/api/overview) now, what happened to other 12? Do they still exist?
...
Previously, the `api` folder ([the API project application](/docs/{version}/architecture/api/overview)) was located in the project root. This is no longer the case. All project applications are now located in the [`apps`](https://github.com/webiny/webiny-js/tree/v5/apps) folder.
