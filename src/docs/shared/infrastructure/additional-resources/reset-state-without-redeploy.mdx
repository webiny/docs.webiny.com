...
It's not always feasible to destroy your Webiny instance one you've started to create entries. If you need to delete entries in the Headless CMS and effectively restore your instance to a previous state, you can follow along here.
...
You need to have a deployed Webiny instance and have logged in to your AWS account. You also need you root user access key ID and secret.
...
![DDB tables in the AWS account area](./assets/reset-state-without-redeploy/1-ddb-tables.png)

If you're not sure which ddb table is associated with which Webiny instance, you can navigate to **.pulumi/apps/core/.pulumi/stacks/{env}.json** to find the table `id`.
...
## Step 2: Create ElasticSearch Snapshots

To follow the steps below you will need to get the domain endpoint of the ElasticSearch / OpenSearch. You can find this by navigating to the OpenSearch service dashboard for the region you deployed webiny to, opening up the domains menu item, clicking on the name of your ES domain, and copying the domain endpoint as shown in the screenshot below:

![How to find the ElasticSearch / OpenSearch Service domain endpoint](./assets/reset-state-without-redeploy/3a-es-domain-arn.png)

The full details of how to create Snapshots are in this AWS Docs page, but we'll outline the steps you need specifically for a Webiny instance.
...
Please follow the links above for more detail about how to achieve each step. Once you have completed these you should have a new role that looks like this:
...
Congratulations, you have successfully created a snapshot of your ElasticSearch / OpenSearch Service indexes.
...
Rename it so that it will be the same name as the table.
...
## Step 4: Restore ElasticSearch / OpenSearch indices from a Snapshot
...
If it's there, we can restore our indexes by making a `POST` request to the `_restore` endpoint by appending with the name of that snapshot to the request:
