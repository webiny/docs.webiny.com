...
In this article, we show the process of configuring your Webiny project to use one or more public identity providers (Google, Facebook, Apple, Amazon). In our example code, we'll be using Google as the identity provider.
...
<Alert type={"info"} title={"Configuring a Provider"}>

In this example, we're using Google as our external IdP. To acquire `client_id` and `client_secret` (which are specific to Google), [follow the official documentation](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-social-idp.html).
...
You can add multiple identity providers to your Cognito user pool. Simply add more identity
provider configs to the `identityProviders` array.
