...
In this article, we'll cover the Lexical Editor configuration for the heading and paragraph elements. The configuration includes toolbar actions, plugins, and Lexical nodes.
...
To add a new toolbar action, use the `<LexicalEditorConfig.Heading.ToolbarAction>` component and mount it within your Admin app.
...
// Access the Heading element for cleaner code.
...
export const App = () => {
  return (
    <Admin>
      <Cognito />
      <LexicalEditorConfig>
        <Heading.ToolbarAction name={"myCustomBoldActon"} element={<MyCustomBoldActon />} />
      </LexicalEditorConfig>
    </Admin>
  );
};
...
```jsx apps/admin/src/App.tsx
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.ToolbarAction name={"bold"} element={<MyCustomBoldActon />} />
</LexicalEditorConfig>
```

#### Remove an Action {#heading-remove-action}

To remove an action, reference the existing action by name, and pass a `remove` prop to the `<LexicalEditorConfig.Heading.ToolbarAction>` element:

```jsx apps/admin/src/App.tsx
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.ToolbarAction name={"fontSize"} remove />
</LexicalEditorConfig>
```

#### Position an Action {#heading-position-action}

To position an existing or a new custom action, use `before` or `after` props on the `<LexicalEditorConfig.Heading.ToolbarAction>` element:
...
<LexicalEditorConfig>
  ...
  <LexicalEditorConfig.Heading.ToolbarAction name={"fontSize"} after={"myCustomBoldActon"} />
</LexicalEditorConfig>
```
...
To add a new Lexical plugin, you need to use the `<LexicalEditorConfig.Heading.Plugin>` component and mount it within your Admin app.
...
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.Plugin
...
In the example above, we created a floating link popup to enable users to edit links. You can refer to [our example repository](https://github.com/webiny/webiny-examples/tree/master/lexical-editor-configuration-api) for a step-by-step guide on building this plugin.
...
```jsx apps/admin/src/App.tsx
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.Plugin name={"link"} element={<CustomFloatingLinkEditorPlugin />} />
</LexicalEditorConfig>
```

#### Remove a Plugin {#heading-remove-plugin}

To remove a plugin, reference the existing plugin by name, and pass a `remove` prop to the `<LexicalEditorConfig.Heading.Plugin>` element:

```jsx apps/admin/src/App.tsx
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.Plugin name={"link"} remove />
</LexicalEditorConfig>
```

#### Position a Plugin {#heading-position-plugin}

To position an existing or a custom plugin, use `before` or `after` props on the `<LexicalEditorConfig.Heading.Plugin>` element:

```jsx apps/admin/src/App.tsx
/* Add a new plugin after the `fontSize` plugin. */
<LexicalEditorConfig>
    <LexicalEditorConfig.Heading.Plugin name={"newPlugin"} element={<NewPlugin />} after={"fontSize"} />
</LexicalEditorConfig>

/* Position the existing plugin before some other plugin. */
<LexicalEditorConfig>
    <LexicalEditorConfig.Heading.Plugin name={"link"} before={"anotherPlugin"} />
</LexicalEditorConfig>
```
...
<Alert type={"warning"}>
  Built-in nodes cannot be removed or replaced to avoid existing data corruption. You can only
  remove or replace your custom nodes, or nodes added by other 3rd party plugins.
</Alert>

#### Add a Node {#heading-add-node}

To add a new Lexical node, use the `<LexicalEditorConfig.Heading.Node>` component and mount it within your Admin app.
...
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.Node name={"emojiNode"} node={EmojiNode} />
</LexicalEditorConfig>;
```
...
```jsx apps/admin/src/App.tsx
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.Node name={"emojiNode"} node={MyExtendedEmojiNode} />
</LexicalEditorConfig>
```

#### Remove a Node {#heading-remove-node}

To remove a node, reference the existing node by name, and pass a `remove` prop to the `<LexicalEditorConfig.Heading.Node>` element:

```jsx apps/admin/src/App.tsx
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.Node name={"emojiNode"} remove />
</LexicalEditorConfig>
```

#### Position a Node {#heading-position-node}

To position your nodes, you can use `before` and `after` props on the `<LexicalEditorConfig.Heading.Node>` element:

```jsx apps/admin/src/App.tsx
<LexicalEditorConfig>
  <LexicalEditorConfig.Heading.Node name={"emojiNode"} node={EmojiNode} after={"newNode"} />
  <LexicalEditorConfig.Heading.Node
    name={"newNode"}
    node={NewNode}
    before={"myExtendedEmojiNode"}
  />
</LexicalEditorConfig>
```
...
| Class                 | Name                 |
| --------------------- | -------------------- |
| FontColorNode         | `font-color-node`    |
| ImageNode             | `image`              |
| ListNode              | `webiny-list`        |
| ListItemNode          | `webiny-listitem`    |
| CodeNode              | `code`               |
| HashtagNode           | `hashtag`            |
| CodeHighlightNode     | `code-highlight`     |
| TypographyElementNode | `typography-el-node` |
| ParagraphNode         | `paragraph-element`  |
| HeaderNode            | `header-element`     |
| QuoteNode             | `webiny-quote`       |
