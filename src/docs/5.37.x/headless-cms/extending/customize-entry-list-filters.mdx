...
To add a new filter, you must use the `ContentEntryListConfig` component and mount it within your `Admin` app. Your filter is just a regular React element, so it can render anything.
...
The following example shows how to implement a filter that shows "Deluxe properties only". For this example, we treat a property as "Deluxe" when its `price` is higher than 1 million. Note that we are only showing the actual filter implementation. To see the filter registration
