...
To add a new filter, you need a regular React element to pass to the `Browser.Filter` component through the `element` prop. Additionally, you must include the `name` prop, which is used as a reference.
...
The following example shows how to implement a filter that shows "Deluxe properties only": we treat a property as "Deluxe" when its `price` is higher than 1 million. Note that we are only showing the actual filter implementation. To see the filter registration
