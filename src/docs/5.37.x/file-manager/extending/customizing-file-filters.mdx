---
title: Customizing File Filters
...
By default, Webiny File Manager offers a single built-in filter, to help you quickly filter files based on their type (images, documents, videos). In this article, you will learn how to add more filters, but also remove or replace existing filters in the filters bar.

<Image src={fmFilters} title="Built-in filter by type" />

## Configuration

All modifications that are currently available for the File Manager, are exposed via a dedicated configuration component: the `FileManagerViewConfig` component. This component has other components attached to it, so by importing this one component, you get everything you need to customize the File Manager. This component contains 2 child components: `Browser` and `FileDetails`.
...
<Image src={fmGridView} title="File Manager Browser" />

Components to customize this part of the File Manager are located in the `FileManagerViewConfig.Browser` namespace.

### File Details

File details is the drawer that shows the details of each file, and allows users to update file attributes, crop the file, add file tags, etc.

<Image src={fmFileDetails} title="File Details Drawer" />

Components to customize this part of the File Manager are located in the `FileManagerViewConfig.FileDetails` namespace.
...
### Example 1: Set and Unset a Filter
...
### Example 2: Using Filters Converter for Complex Filters
