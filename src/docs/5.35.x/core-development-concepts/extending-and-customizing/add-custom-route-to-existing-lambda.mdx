...
In version 5.35 or later we have removed the headlessCMS Lambda and its handler, so if you have any existing routes there, they should be moved as part of the upgrade to v5.35.0 Please refer to the relevant upgrade documentation. Everything you might need from the system is available in the GraphQL Lambda handler.
