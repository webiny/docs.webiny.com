...
Plugins are a vital part of Webiny, and the majority of Webiny's functionality lives in plugins. You can learn more about plugins [here](../../core-development-concepts/basics/plugins).
...
Run the [`webiny watch`](../../core-development-concepts/basics/watch-command) command to start a new watch session on `apps/admin` application code.
...
3. As the [`webiny watch`](../../core-development-concepts/basics/watch-command) command is already running on `apps/admin`, we should see these changes immediately.
...
4. Deploy the API changes, run the [`webiny watch`](../../core-development-concepts/basics/watch-command) command to start a new watch session on `apps/api/graphql` application code.
...
Notice how we had to run the [`yarn workspace`](https://classic.yarnpkg.com/en/docs/cli/workspace/) command and specify the workspace name (`api-headless-cms`) in order to add the [`cryptr`](https://www.npmjs.com/package/cryptr) NPM package. This is because every Webiny project is organized as a monorepo and can consist of multiple workspaces. To learn more, check out the [Monorepo Organization](../../core-development-concepts/project-organization/monorepo-organization) key topic.
