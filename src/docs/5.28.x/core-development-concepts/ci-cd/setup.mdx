...
In order to set up your own CI/CD workflow faster (with all of the concepts, steps, and strategies explained in the [CI/CD](../../core-development-concepts/ci-cd/introduction) key topics), we recommend you use the built-in CI/CD scaffold.
...
![GitHub Actions](./assets/ci-cd/gh_actions_logo.png)
...
The created workflow files follow workflows described in the [CI/CD](../../core-development-concepts/ci-cd/introduction) key topics section. Before running them, you're free to inspect and adjust them to your needs.

</Alert>

<Alert type="info">

Workflows rely on [GitHub Environments](https://docs.github.com/en/actions/reference/environments) in order to establish environment protection rules and store all of the necessary secrets, for example AWS credentials or [Pulumi Backend](../../core-development-concepts/ci-cd/cloud-infrastructure-state-files#using-different-backends).
...
![GitHub Environments](./assets/ci-cd/gh_environments.png)
...
1. provide deployment secrets - AWS credentials and [Pulumi Backend](../../core-development-concepts/ci-cd/cloud-infrastructure-state-files#using-different-backends).
...
`AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` represent credentials of an AWS account that will be used to deploy necessary cloud infrastructure resources, into the specific environment. Note that, as stated in the [CI/CD](../../core-development-concepts/ci-cd/environments#cloud-aws-accounts) key topics, we recommend every environment (`dev`, `staging`, `prod`) uses its own AWS account. So, these should be different for each environment.

<Alert type="success">

Learn how to create AWS credentials in the [Configure AWS Credentials](../../infrastructure/aws/configure-aws-credentials) guide.
...
Upon deploying cloud infrastructure resources with [Pulumi](../../infrastructure/pulumi-iac/iac-with-pulumi), we need to specify the secrets provider. As the name itself suggest, this feature enables us to store deployment-related secrets, for example API keys, connection credentials, and similar.
...
The last piece of the puzzle is the `WEBINY_PULUMI_BACKEND` secret, which determines the [Pulumi backend](../../core-development-concepts/ci-cd/cloud-infrastructure-state-files#using-different-backends) that we want to use in order to store our cloud infrastructure state files. At the moment, here we recommend you use one of the following two options.

Either you simply manually deploy an Amazon S3 bucket within the AWS account that's about to be used to deploy your project into a specific environment, or alternatively, use the [Pulumi Service](../../core-development-concepts/ci-cd/cloud-infrastructure-state-files#pulumi-service). In case of the former, just set the S3 URI, for example `s3://my-project-pulumi-state-files-dev`. In case of the latter, simply paste the Pulumi Service [Access Token](https://app.pulumi.com/account/tokens).

<Alert type="success">

If you choose to use Amazon S3 as the backend and you are using multiple AWS Accounts [as recommended](../../core-development-concepts/ci-cd/environments#cloud-aws-accounts), note that you will end up with creating multiple buckets, one within each AWS account. Each Amazon S3 bucket will only store cloud infrastructure state files for a specific [shared environment](../../core-development-concepts/ci-cd/environments#shared-environments).

</Alert>

<Alert type="info">

For more information on backends, we recommend you read the [Cloud Infrastructure State Files](../../core-development-concepts/ci-cd/cloud-infrastructure-state-files#using-different-backends) key topic.

</Alert>

##### 4. Start Developing

To start developing, based on the [GitHub Flow](../../core-development-concepts/ci-cd/version-control#github-flow), the first step of every developer would be to simply create a new branch from the default `dev` branch. Once ready, we submit a pull request, again against the `dev` branch.

<Alert type="success">

Read more about the GitHub Flow in the [Version Control](../../core-development-concepts/ci-cd/version-control#github-flow) key topic.
