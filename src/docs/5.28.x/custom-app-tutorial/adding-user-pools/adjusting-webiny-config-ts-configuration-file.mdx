...
Changes made to the [`webiny.config.ts`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/05-security/03-cloud-infrastructure/03-adjusting-webiny-config-ts/pinterest-clone/app/code/webiny.config.ts) file aren't automatically picked up by already active watch sessions, initialized with the [`webiny watch`](../../core-development-concepts/basics/watch-command) command. Before you start making any changes, make sure to terminate any active watch sessions you might have and run them again once the [`webiny.config.ts`](https://github.com/webiny/webiny-examples/blob/master/create-custom-application/05-security/03-cloud-infrastructure/03-adjusting-webiny-config-ts/pinterest-clone/app/code/webiny.config.ts) file is in the right place.
...
Under the hood, essentially, the [`webiny deploy`](../../core-development-concepts/basics/project-deployment) command performs two steps:
...
Once the first deployment has been performed, the following deployments of our React application can be done as usual, with the single [`webiny deploy`](../../core-development-concepts/basics/project-deployment) execution:
...
To determine whether the React application has been deployed or not, we can run the following [`webiny output`](../../core-development-concepts/basics/webiny-cli#yarn-webiny-output-folder---env-env) command:
