...
By completing the [previous set of tutorials](./introduction), in our Admin Area application, we should have the following items in the main menu:
...
As you may have noticed, we're using the exact same logic and code, that we previously used while trying to secure the [`getCarManufacturer`](./securing-graphql-api#securing-graphql-api-resolvers) GraphQL resolver function. The only aspect that's different is the way we fetch the currently logged-in user's permission. Here, we are using the `useSecurity` React hook in order to first get the logged-in user, and then, via the `getPermission` method, the `car-manufacturers` permission.
