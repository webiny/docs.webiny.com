...
The best way to start extending the Admin Area application would be to use the [Extend Admin Area](./scaffolding) scaffold, which will automatically generate all of the fundamental application pieces for us. On the application code level, this includes the necessary plugins which create the following:

- in the Admin Area application: a new route, menu item, and a [CRUD view](./scaffolding#new-webiny-admin-area-module)
...
1. the path to our GraphQL API's plugins folder (`apps/api/graphql/src/plugins` by default)
2. the path to our Admin Area application's plugins folder (`apps/admin/src/plugins` by default)
...
In order to actually see the changes that the **Extend Admin Area** scaffold performed and continue developing, we will start two separate watch sessions. This can be achieved via the [`webiny watch`](../../core-development-concepts/basics/watch-command) command.
...
Once the two watch sessions have been started, in our Admin Area application, we should be able to see the new **Car Manufacturers** module and, via the generated [CRUD view](./scaffolding#new-webiny-admin-area-module), perform essential CRUD operations.
