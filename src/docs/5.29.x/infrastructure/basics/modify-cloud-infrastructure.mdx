...
When Webiny is deployed, apart from the application code (for example your GraphQL API or Admin React application), it also deploys required of cloud infrastructure resources. And although the deployed resources are already configured in the best possible manner, there are still occasions where modifications might be needed, or the deployed cloud infrastructure needs to be expanded with additional resources. 
