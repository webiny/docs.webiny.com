...
Note that the code assumes the AWS Lambda function's code is located in the `apps/api/myCronJob` folder. The full code can be found in our [Examples GitHub  repository](https://github.com/webiny/webiny-examples/tree/master/extend-admin-area-security).
