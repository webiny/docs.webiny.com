...
And although the deployed resources are already configured in the best possible manner, there are still occasions where modifications might be needed, or the deployed cloud infrastructure needs to be expanded with additional resources. 
...
export default createCoreApp({
    pulumi: app => {
        tagResources({
            // We are assigning Owner and Contact tags, whose values are read from environment variables. 
