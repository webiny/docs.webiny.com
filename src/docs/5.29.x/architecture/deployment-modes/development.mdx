...
an AZ failure. The same cannot be said for the Amazon ElasticSearch Service <diagram-letter>
  h
</diagram-letter>, which is only deployed in a single AZ <diagram-letter>C</diagram-letter>.

<Alert type="info">

The Amazon ElasticSearch Service is only deployed with the **Amazon DynamoDB + Amazon ElasticSearch Service** setup.

For more information, visit the [Different Setups](/docs/{version}/architecture/introduction#different-setups) sections.
...
In the development mode, it is deployed into a single AZ, just for cost reasons. But yes, production workloads should use two or three AZs, hence the production mode. Check out the [Amazon ElasticSearch Service's FAQ page](https://aws.amazon.com/elasticsearch-service/faqs/) for more information.
