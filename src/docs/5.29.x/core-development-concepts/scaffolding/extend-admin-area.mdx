...
The **Extend Admin Area** scaffold (previously **Admin Area Package**) received a major overhaul in [Webiny 5.9.0](../../release-notes/5.9.0/changelog). Note that this guide is not relevant for projects that are using Webiny 5.8.0 and lower.
...
![New Webiny Admin Area Module](./assets/extend-admin-area/scaffolds-admin-area.png)
...
![Locales Selector](./assets/extend-admin-area/locales-selector.png)
...
The scaffold does not add any additional cloud infrastructure resources. The generated application code is simply added to the existing Admin Area React application code ([`apps/admin`](https://github.com/webiny/webiny-js/tree/v5.10.0/packages/cwp-template-aws/template/apps/admin/code)).
...
Luckily, with a couple of built-in utilities, this can be relatively easily added. Please check out the [existing tutorials](../../core-development-concepts/security-framework/introduction) to learn how to implement these on your own.
