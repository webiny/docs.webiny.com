...
Depending on your preferences, you can import which ever you like. We import methods from the root of the `@webiny/handler-aws`. 

#### What Is the `createEventHandler` Method?
This method creates the handler of received event. At this point while Webiny system is ready - all passed plugins are applied, security checks are done, etc...
...
We hope that name would indicate the amount of `event handlers` you can define, so:
- route - multiple
- event - single
...
// ... other imports
import { createApiGatewayHandler as createHandler, createApiGatewayRoute } from "@webiny/handler-aws";
...
<Alert type="warning">
  Security checks will be done as on the <strong>/graphql</strong> endpoint. If you need customizations, feel free to add them.
</Alert>
