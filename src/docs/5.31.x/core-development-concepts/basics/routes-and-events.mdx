...
This enables us to give our users the possibility to add different routes and events handling.

<Alert type="warning">
  Note that while you can define multiple routes per handler, you can define only a single event per handler.
</Alert>
...
export default createApiApp({
    pulumi: app => {
        ...
        app.resources.apiGateway.addRoute("/webiny", {
            path: "/webiny",
            method: "POST",
            function: graphQLModule.functions.graphql.output.arn
        });
    }
});
...
And yes, you can catch API Gateway event like this, but there is no point since we have a handler specifically designed for that.
Of course, feel free to use what ever is available and can help them to achieve your goal.
