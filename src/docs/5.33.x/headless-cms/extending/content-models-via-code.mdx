...
Let's create a [`models.ts`](https://github.com/webiny/webiny-examples/blob/master/headless-cms/content-model-plugins/basic/api/code/headlessCMS/src/plugins/models.ts) file in the `apps/api/headlessCMS/src/plugins` directory.
...
Let's make the following changes in `apps/api/headlessCMS/src/index.ts` file:
...
```diff-js apps/api/headlessCMS/src/plugins/models.ts
export default [
  (...)
  fields: [
     (...)
