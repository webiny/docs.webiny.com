...
Old ones are still available, but marked as deprecated, and they will be removed in 5.35.0.
</Alert>
...
Those new objects have `StorageTransformPlugin` run on them to prepare them to be stored. To find out why we use `StorageTransformPlugin`, read about it in this [article](./storage-transform-plugin).
