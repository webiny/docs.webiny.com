...
There are two ways of setting the `index.refresh_interval`. First one is via the REST call to your Elasticsearch cluster, which is for the existing indexes:
```
...
The second one is for the indexes that are not yet created, and it is done via the `CmsEntryElasticsearchIndexPlugin`:
```ts apps/api/headlessCMS/src/plugins/customConfiguration.ts
...
export const customConfiguration = new CmsEntryElasticsearchIndexPlugin({
        ...
        return {
            ...
            settings: {
                ...
                index: {
                    ...
                    /**
                     * If you are wondering why it's string:
...
And then you need to register it in the `apps/api/headlessCMS/src/index.ts`.
```ts apps/api/headlessCMS/src/index.ts
export const handler = createHandler({
    plugins: [
        /**
         * Find the createHeadlessCmsContext and add the customConfiguration plugin into the plugins array in createHeadlessCmsStorageOperations.
         */
        createHeadlessCmsContext({
            storageOperations: createHeadlessCmsStorageOperations({
                documentClient,
                elasticsearch,
                plugins: [createGzipCompression(), customConfiguration]// <--- here
            })
        }),
    ]
})
