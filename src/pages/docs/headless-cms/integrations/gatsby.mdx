...
Use the [official starter](https://github.com/webiny/gatsby-starter-webiny) for 1-click install on Vercel.
...
For more information about the [Headless CMS GraphQL API, see this page of our docs](/docs/headless-cms/basics/graphql-api).
...
Next, open the file `gatsby-config.js` and add the following to the top of the file:
...
This will ensure we can access the environment variables we created. In side the `module.exports` object we're going to define our new plugin and the configuration we need to fetch data:
...
To do that, first import the dependencies we will need:
...
Next we need to export a `createPages` function which will loop over our data and create pages using a template we specify.
...
```javascript /src/templates/blog-post.js
...
Now you should be able to render data in the page component:
...
If this is succesful you should be able to start your development server and see pages rendered with URL paths that correspond to your posts.
...
You may have noticed that we chose a `rich-text` field for the **body** of the post. You can copy the code from `https://github.com/webiny/webiny-js/blob/next/packages/react-rich-text-renderer/src/index.tsx` into a new component and use that in your template.
...
[For more information about the rich text renderer, see this documentation](/docs/headless-cms/extending/render-rich-text-content)
