...
The shown diagram gives an overview of which cloud infrastructure resources are deployed when the **Custom VPC** option was chosen during the creation of a new Webiny project. When compared to the [Default VPC](/docs/architecture/api/overview-vpc-default) option, essentially, resources still work and communicate with each other in the same way, except this time, there are a couple of additional network-level resources, and rules in place. This helps in improving your project's overall security posture.
