...
For brevity, the diagram doesn't include network-level cloud infrastructure resources, like region, VPC, availability zones, and so on. Check out the [Default VPC](/docs/architecture/api/overview-vpc-default) and [Custom VPC](/docs/architecture/api/overview-vpc-custom) topics if you're interested in that aspect of the deployed cloud infrastructure.
