...
1) Running a multi-tenant system, while only having the `root` tenant, will now properly route all requests _even without having a custom domain_ configured. A custom domain becomes mandatory only when you have more than 1 tenant in your system.
