...
On the other hand, a much faster way of extending the GraphQL API would be to use the [Extend GraphQL API scaffold](../../core-development-concepts/scaffolding/extend-graphql-api), which not only creates all of the boilerplate code, but also a set of essential GraphQL query and mutations, sets up the database connection, creates sample tests, and more.
...
In order to actually see the new operations in your GraphQL API, your application code needs to be redeployed. This can be done as usual via the [`webiny deploy`](../../core-development-concepts/basics/project-deployment), or, even easier, via the [`webiny watch`](../../core-development-concepts/basics/watch-command) command. The latter will not only deploy the changes, but also start a new watch session, which will automatically redeploy further application code changes, as you perform them.

For more information on how to use it, revisit the [Using the Watch Command](../overview/get-started#using-the-watch-command) topic in the previous tutorial section.
...
Before we continue, note that in order to do a quick manual test of our GraphQL API, we need to have the URL over which it can be accessed. This is where the following [`webiny output`](../../core-development-concepts/basics/webiny-cli#common-commands) command can help us:
...
By default, query and mutation operations defined by the [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) are publicly accessible. For now, this is not a concern for us. As mentioned, we will tackle the security aspect of our application in the upcoming sections of this tutorial.
...
The application code generated by the [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) scaffold relies on [DynamoDB Toolbox](https://github.com/jeremydaly/dynamodb-toolbox), which is a library that makes it a bit easier to interact with Amazon DynamoDB. The `Entity` class we've seen in the above code sample is part of it.

For more information, check out the [Storing and Retrieving Data](../../core-development-concepts/scaffolding/extend-graphql-api#storing-and-retrieving-data) section in the [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) scaffold guide.
...
With the [`webiny watch`](../../core-development-concepts/basics/watch-command/) command up and running, the performed application code changes should be automatically rebuilt and redeployed into the cloud.
