...
The **GraphQL API** scaffold creates a new standalone GraphQL HTTP API, which can be utilized by your frontend applications (for example, applications created with the [React Application](../../core-development-concepts/scaffolding/react-application) scaffold). Once created, you can immediately deploy it into your cloud (AWS) account and continue further development with just a couple of [Webiny CLI](../../core-development-concepts/basics/webiny-cli) commands.
...
Furthermore, note that when [deploying](../../core-development-concepts/basics/project-deployment) into the `prod` (production) environment, the GraphQL Handler AWS Lambda <diagram-letter>C</diagram-letter> function will be deployed into a new [Amazon Virtual Private Cloud (VPC)](https://aws.amazon.com/vpc/). Down the line, doing this for cloud infrastructure resources can improve your GraphQL API's overall security posture.

More information on different databases and this dual cloud infrastructure setup can be found in the [Development](../../core-development-concepts/scaffolding/full-stack-application#development) section below.

<Alert type="info">

During the scaffold's setup wizard, you will be asked if you want to immediately deploy the needed cloud infrastructure resources. If you decide not to do it, you can deploy it later, as usual, with the [`webiny deploy`](../../core-development-concepts/basics/project-deployment) command.

</Alert>

### Compatible With the Existing Extend GraphQL API Scaffold

In order to speed up development, you can use the existing [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) scaffold to quickly extend the created GraphQL API with new sets of essential CRUD GraphQL query and mutation operations.

### Testing

Except for the application and cloud infrastructure code, the scaffold also creates the necessary [Jest](../../core-development-concepts/basics/tools-and-libraries#testing) configuration and foundation for testing the GraphQL API functionality you'll be building. The already mentioned [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) scaffolds takes advantage of this and immediately creates a couple of example tests you can use for reference.
...
This is where you'll be creating plugins, either manually or via other scaffolding utilities, for example via the [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) scaffold.
...
The config for the [Jest](../../core-development-concepts/basics/tools-and-libraries#testing) testing framework. Feel free to adjust it to your needs.
...
The config for the [Babel](../../core-development-concepts/basics/tools-and-libraries#building-code) JavaScript compiler. Feel free to adjust it to your needs.
...
In case you missed it, every Webiny project is organized as a [monorepo](../../core-development-concepts/project-organization/monorepo-organization), which can consist of multiple packages. And this is the reason why the [`code/graphql/package.json`](https://github.com/webiny/webiny-js/blob/v5.13.0/packages/cli-plugin-scaffold-graphql-api/template/code/graphql/package.json) manifest file exists. It exists because the `code/graphql` folder represents a monorepo [workspace](https://classic.yarnpkg.com/en/docs/workspaces/).

In most cases, this file will only be modified when new NPM packages are added.

<Alert type="info">

Learn more about the monorepo organization in the [Monorepo Organization](../../core-development-concepts/project-organization/monorepo-organization) key topic.
...
Learn more about Pulumi, the default infrastructure-as-code (IaC) framework Webiny relies on, in the [Infrastructure as code with Pulumi](../../infrastructure/pulumi-iac/iac-with-pulumi) key topic.

</Alert>

### Deployment

Once you've completed the scaffold's wizard and the files have been created, in order to actually access the GraphQL API, you need to deploy it. This can be done as usual via the [`webiny deploy`](../../core-development-concepts/basics/project-deployment) command, or, even easier, if you're about to jump straight into coding, by running the [`webiny watch`](../../core-development-concepts/basics/watch-command) command. This command will not only deploy the changes, but also start a new watch session, which will automatically redeploy further application code changes, as you perform them (more on this below).
...
The most straightforward way to continue developing on top of the created code would be via the [`webiny watch`](../../core-development-concepts/basics/watch-command) command.
...
To learn more about how to write different types of tests, we recommend you check out the [Writing and Running Tests](../../core-development-concepts/scaffolding/extend-graphql-api#writing-and-running-tests) section in the [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) guide.
...
By default, the [Extend GraphQL API](../../core-development-concepts/scaffolding/extend-graphql-api) scaffold relies on [DynamoDB Toolbox](https://github.com/jeremydaly/dynamodb-toolbox) for all Amazon DynamoDB interactions.
...
Luckily, with a couple of built-in utilities, this can be relatively easily added. Please check out the [existing tutorials](../../admin-area/new-app-tutorial/security) to learn how to implement these on your own.
...
In case you missed it, every Webiny project is organized as a [monorepo](../../core-development-concepts/project-organization/monorepo-organization), which can consist of multiple packages. This is the reason why the [`code/graphql/package.json`](https://github.com/webiny/webiny-js/blob/v5.13.0/packages/cli-plugin-scaffold-graphql-api/template/code/graphql/package.json) manifest file exists. It exists because the `code/graphql` folder represents a monorepo [workspace](https://classic.yarnpkg.com/en/docs/workspaces/).
