...
description: Learn how to change Elasticsearch Name To Contain Locale Code.
---
...
- how to how to change Elasticsearch Name To Contain Locale Code.
...
This will cover cloning the existing Elasticsearch index to another one (name which contains the locale code).

This will NOT change the Elasticsearch index mappings or settings (apply templates) for the new index.
...
<Alert type="danger">
  Skip this step if your CI automatically deploys code when environment variable changes.
</Alert>
...
The article [How To Customize Elasticsearch Index Template](/docs/core-development-concepts/extending-and-customizing/how-to-customize-elasticsearch-index-template) contains the information on how we create the index names.
...
Let's say if the system is on `en-GB` locale, then the new Elasticsearch index name would be `root-en-gb-page-builder`. Note that `GB` is in lower case - that is because Elasticsearch does not support index names with upper case. 

```title="The Action To Be Executed"
...
You __MUST__ repeat previous steps for of the indexes you have (except Headless CMS ones, of course) because the environment variable you add will affect all of the applications.

#### 8. Deploy The Code For Environment Variable To Have Effect
