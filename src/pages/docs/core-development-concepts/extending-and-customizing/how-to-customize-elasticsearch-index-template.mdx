...
- how to customize Elasticsearch index template for each application
...
### When Do We Use The Elasticsearch Index Template?

We use the index templates for our File Manager, Form Builder, Headless CMS and Page Builder applications.

In File Manager, Form Builder and Page Builder applications we use one index per tenant and locale, if locale specific indexes are set.
...
* File Manager Files [template](https://github.com/webiny/webiny-js/blob/2f55b1d6aea2085f8c46fc73c48157a49cd46ae0/packages/api-file-manager-ddb-es/src/elasticsearch/templates/base.ts)
* Form Builder Forms and Submissions [template](https://github.com/webiny/webiny-js/blob/2f55b1d6aea2085f8c46fc73c48157a49cd46ae0/packages/api-form-builder-so-ddb-es/src/elasticsearch/templates/base.ts)
* Headless CMS Entries [template](https://github.com/webiny/webiny-js/blob/2f55b1d6aea2085f8c46fc73c48157a49cd46ae0/packages/api-headless-cms-ddb-es/src/elasticsearch/templates/base.ts)
* Page Builder Pages [template](https://github.com/webiny/webiny-js/blob/2f55b1d6aea2085f8c46fc73c48157a49cd46ae0/packages/api-page-builder-so-ddb-es/src/elasticsearch/templates/base.ts)
...
### When Would You Use The Possibility To Change The Elasticsearch Index Template?

Good example on when to change the default template, or add a new one, is when you are using a locale which is not supported by our default template, eg. Japanese.

The Japanese Elasticsearch index template will look a lot different from our default one because of different text analyzers and mapping properties.
...
1. The `name` property must be __UNIQUE__, or it will override the existing template configuration.
Let's say you name your custom Headless CMS index template `headless-cms-entries-index-default` - it will overwrite our default template configuration, so be careful if you do not want to override existing templates.

2. If you want to apply your template after some other template, use the `order` value greater than the template you want to override.
...
<Alert type="warning">
  Note that templates stack on each other, so if you do not override the template configuration from a first template applied, it will be set into the created index.
</Alert>
...
4. We gave our template plugins different minimum and maximum `order` values:
...
Index name can contain dashes (-), underscores (_), numbers (0-9) and english letters (a-z).
...
Use case for this would be for when you have multiple environments on a single Elasticsearch instance. You can prefix each environment indexes with, for example, `dev`, `prod`, `staging`, `test`, etc...
...
Headless CMS index template plugin `CmsEntryElasticsearchIndexTemplatePlugin`, which disables mappings on a `largeText` field in `de` locale:
```typescript
...
Page Builder index template plugin `PageElasticsearchIndexTemplatePlugin`, which disables indexing on a `largeText` field:
```typescript
...
* [Elasticsearch - Create or update index template API](https://www.elastic.co/guide/en/elasticsearch/reference/7.17/indices-templates-v1.html#put-index-template-v1-api-request-body)
