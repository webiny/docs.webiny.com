...
import fmBulkActions from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-bulk-actions.png";
import fmBulkActionsSimple from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-bulk-actions-simple.gif";
import fmBulkActionsAsync from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-bulk-actions-async.gif";
import fmBulkActionsDiscover from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-bulk-actions-discover.png";
import fmBulkActionsPosition from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-bulk-actions-position.png";
...
<ul>
<li parentName="ul">{`how to add a custom bulk action to the File Manager`}</li>
...
<p>{`In File Manager, pre-built actions empower users to delete, or move multiple files effortlessly. These actions become visible when you select one or more files from the list.`}</p>
<p>{`To work with bulk actions, you need to use the `}<inlineCode parentName="p">{`FileManagerViewConfig`}</inlineCode>{` component. For this article, we will use the `}<inlineCode parentName="p">{`BulkAction`}</inlineCode>{` in the `}<inlineCode parentName="p">{`Browser`}</inlineCode>{` namespace.`}</p>
...
<p><inlineCode parentName="p">{`Browser`}</inlineCode>{` is the crucial component of the Admin app’s File Manager. This is where users can easily browse through files, apply filters, perform searches, upload new files and organize them into folders.`}</p>
<p>{`If users select one or more files from the list, the bulk action bar will appear.`}</p>
<Image src={fmBulkActions} title="Browser and the Built-in Bulk Actions" mdxType="Image" />
<_Heading level={2} id={"add-a-bulk-action"} nextElement={{
      "type": "paragraph"
    }}>Add a Bulk Action</_Heading>
<p>{`To add a new bulk action, use the `}<inlineCode parentName="p">{`FileManagerViewConfig`}</inlineCode>{` component and mount it within your Admin app. This component will serve as the foundation for your bulk actions.`}</p>
...
<p>{`Here is an example of creating a bulk action that copies data from the selected files to the clipboard in JSON format.`}</p>
<Editor title="apps/admin/src/App.tsx" lang="diff-tsx" mdxType="Editor">
{`import React from "react";
import { Admin } from "@webiny/app-serverless-cms";
import { Cognito } from "@webiny/app-admin-users-cognito";
...
<p>{`The `}<inlineCode parentName="p">{`items`}</inlineCode>{` property obtained from `}<inlineCode parentName="p">{`useWorker()`}</inlineCode>{` is a crucial part of managing bulk actions: these are the currently selected items within the context of the File Manager.`}</p>
<p>{`This is the whole process of registering a new bulk action element.`}</p>
<Image src={fmBulkActionsSimple} title="Custom Simple Bulk Action" mdxType="Image" />
<_Heading level={3} id={"asynchronous-bulk-action"} nextElement={{
      "type": "paragraph"
    }}>Asynchronous Bulk Action</_Heading>
<p>{`Sometimes, you may need to execute a bulk action that involves multiple asynchronous tasks, such as sending selected files to an external service. In such cases, we can use the `}<inlineCode parentName="p">{`useWorker`}</inlineCode>{` and `}<inlineCode parentName="p">{`useDialogs`}</inlineCode>{` hooks to create a seamless user experience:`}</p>
<ul>
...
<p>{`Here’s an example of creating a bulk action that sends the selected files to an external service using `}<inlineCode parentName="p">{`fetch`}</inlineCode>{` and shows the result of each iteration.`}</p>
<Editor title="apps/admin/src/App.tsx" lang="diff-tsx" mdxType="Editor">
{`import React from "react";
import { Admin } from "@webiny/app-serverless-cms";
import { Cognito } from "@webiny/app-admin-users-cognito";
...
<Image src={fmBulkActionsAsync} title="Asyncronous Bulk Action" mdxType="Image" />
...
<Image src={fmBulkActionsPosition} title="Position Your Action Before Another Action" mdxType="Image" />
