...
import fmFoldersFieldsSchema from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-folder-custom-fields-schema.png";
import fmAdvancedDetails from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-folder-custom-fields.png";
import fmFieldRendererDecorator from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/fm-field-renderer-decorator.png";
...
<ul>
<li parentName="ul">{`how to add custom folder fields to the File Manager`}</li>
...
<Gallery mdxType="Gallery">
  <Image src={fmFoldersFieldsSchema} title={"Custom fields in the GraphQL schema"} mdxType="Image" />
...
<p>{`To add a new field, use the `}<inlineCode parentName="p">{`createFmFileFolderModelModifier`}</inlineCode>{` plugin factory, provided by the `}<inlineCode parentName="p">{`@webiny/api-file-manager-aco`}</inlineCode>{` package.`}</p>
...
<p>{`This plugin is specifically designed to add fields to folders related to File Manager.`}</p>
<p>{`In GraphQL, this plugin prefixes field IDs with `}<inlineCode parentName="p">{`fm_file_`}</inlineCode>{` by default.`}</p>
...
<li parentName="ul"><inlineCode parentName="li">{`FileManagerViewConfig`}</inlineCode>{`: This component acts as a wrapper, enabling access to configuration utilities for the File Manager List view. It must be mounted as a parent of other config components to ensure the configurations are applied correctly.`}</li>
</ul>
...
<p>{`Once you run your Admin app and open the File Manager, edit an existing folder to see the custom renderer in action. While this example hides the field, you can extend it to include more advanced field customizations.`}</p>
<Image src={fmFieldRendererDecorator} title="Custom renderer is applied to 'carMake' field" mdxType="Image" />
{/* ================== AUTO-GENERATED ================== */}
