...
import customFmRenderer from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/custom-fm-renderer.mp4";
import accountFormState from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/account-form-state.png";
import accountFormStateWithMeta from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/file-manager/extending/assets/account-form-state-with-meta.png";
...
const tableOfContents = [{
  ...
  "title": "FileManagerRendererProps",
  "slug": "file-manager-renderer-props",
  "children": []
}, {
  "title": "Replacing the Default Renderer",
  "slug": "replacing-the-default-renderer",
  "children": []
}, {
  "title": "Conclusion",
  "slug": "conclusion",
...
<p>{`File Manager component can be customized using our Composition API, which allows you to replace the default renderer entirely, or apply conditional logic based on the props being passed to it. In this article, we’ll show how you can implement a custom renderer.`}</p>
<p>{`Replacement of the default renderer is useful if you want to integrate your own asset manager, which communicates with a different API, or a third party library/service (like Cloudinary, Dropbox, etc.), and have Webiny use your new component everywhere in the system.`}</p>
<Alert type="info" title="Important" mdxType="Alert">
<p>{`This article demonstrates renderer customization to completely bypass the default File Manager GraphQL API. If you want to extend the existing renderer, or the File Manager GraphQL schema, please get in touch on our `}<a parentName="p" {...{
          "href": "https://www.webiny.com/slack"
        }}>{`Slack Community`}</a>{`.`}</p>
</Alert>
<_Heading level={2} id={"file-manager-renderer-props"} nextElement={{
      "type": "paragraph"
    }}>FileManagerRendererProps</_Heading>
<p>{`To understand the FileManager React API, let’s look at the following props type. These are the props that are passed to the renderer, and these are the props that you should be able to handle in your custom implementation, as they’re used in various places in Webiny.`}</p>
...
<_Heading level={2} id={"replacing-the-default-renderer"} nextElement={{
      "type": "paragraph"
    }}>Replacing the Default Renderer</_Heading>
<p>{`The following source code demonstrates everything you need to create and register a component plugin for the File Manager renderer.`}</p>
<p>{`The article assumes that you’re using the default Webiny setup, thus the usage of Cognito in the code example.`}</p>
...
<p>{`And that’s it! With this, we’ve created the most basic custom File Manager renderer which will now be rendered everywhere in the system. Here’s a little preview of what it looks like in the Webiny Admin app.`}</p>
<Video src={customFmRenderer} mdxType="Video" />
<p>{`And if we inspect the form state, we’ll find our image data exactly as we specified in our custom renderer:`}</p>
<Image src={accountFormState} title="Image from the custom renderer" mdxType="Image" />
<p>{`Notice how we don’t have our `}<inlineCode parentName="p">{`meta`}</inlineCode>{` array present in the state. That’s because the view in the video uses a `}<inlineCode parentName="p">{`<SingleImageUpload/>`}</inlineCode>{` component which, by default, ignores the `}<inlineCode parentName="p">{`meta`}</inlineCode>{` returned from the File Manager.
If this was a simple `}<inlineCode parentName="p">{`FileManager`}</inlineCode>{` component, form state would look like this:`}</p>
<Image src={accountFormStateWithMeta} title="Image from the custom renderer with meta" mdxType="Image" />
<_Heading level={2} id={"conclusion"} nextElement={{
      "type": "paragraph"
    }}>Conclusion</_Heading>
<p>{`We’ve covered how to override the default File Manager renderer and implement your custom logic of handling files. From here, you can continue upgrading this basic example and connect your React component to an external API or an asset management service. The only thing you need to follow is the component interface defined by the `}<a parentName="p" {...{
        "href": "#file-manager-renderer-props"
      }}>{`FileManagerRendererProps`}</a>{` type.`}</p>
{/* ================== AUTO-GENERATED ================== */}
