...
import teamCreationVideo from "/Users/svenalhamad/Dev/docs.webiny.com/docs/user-guides/5.x/security/essentials/assets/team-creation/create-team.mp4";
...
<p>{`To follow this tutorial, you need to have the `}<strong parentName="p">{`E-commerce`}</strong>{` user role in your Webiny instance. If you don’t have it, please follow the `}<_Link href="/docs/5.x/user-guides/security/essentials/role-creation">{`Creation of Role`}</_Link>{` tutorial to create it.`}</p>
...
<li parentName="ol">
<p parentName="li">{`Click `}<strong parentName="p">{`+ NEW TEAM`}</strong>{`.`}</p>
...
<li parentName="ol">
<p parentName="li">{`In the `}<strong parentName="p">{`Roles`}</strong>{` dropdown, select `}<strong parentName="p">{`E-Commerce`}</strong>{`.`}</p>
...
<li parentName="ol">
<p parentName="li">{`Click `}<strong parentName="p">{`SAVE TEAM`}</strong></p>
