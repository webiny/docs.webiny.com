...
import pbFoldersFieldsSchema from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/page-builder/extending/assets/pb-folder-custom-fields-schema.png";
import pbAdvancedDetails from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/page-builder/extending/assets/pb-folder-custom-fields.png";
import pbFieldRendererDecorator from "/Users/svenalhamad/Dev/docs.webiny.com/docs/developer-docs/5.x/page-builder/extending/assets/pb-field-renderer-decorator.png";
...
<ul>
<li parentName="ul">{`how to add custom folder fields to the Page Builder`}</li>
...
<Gallery mdxType="Gallery">
  <Image src={pbFoldersFieldsSchema} title={"Custom fields in the GraphQL schema"} mdxType="Image" />
  <Image src={pbAdvancedDetails} title={"Custom fields in the Admin app"} mdxType="Image" />
</Gallery>
<_Heading level={2} id={"add-a-field"} nextElement={{
      "type": "paragraph"
    }}>Add a Field</_Heading>
<p>{`To add a new field, use the `}<inlineCode parentName="p">{`createPbPageFolderModelModifier`}</inlineCode>{` plugin factory, provided by the `}<inlineCode parentName="p">{`@webiny/api-page-builder-aco`}</inlineCode>{` package.`}</p>
...
<p>{`This plugin is specifically designed to add fields to folders related to Page Builder Pages.`}</p>
<p>{`In GraphQL, this plugin prefixes field IDs with `}<inlineCode parentName="p">{`pbPage_`}</inlineCode>{` by default.`}</p>
...
<li parentName="ul"><inlineCode parentName="li">{`PageListConfig`}</inlineCode>{`: This component acts as a wrapper, enabling access to configuration utilities for the Page Builder List view. It must be mounted as a parent of other config components to ensure the configurations are applied correctly.`}</li>
</ul>
...
<p>{`Once you run your Admin app and navigate to the Page Builder, edit an existing folder to see the custom renderer in action. While this example hides the field, you can extend it to include more advanced field customizations.`}</p>
<Image src={pbFieldRendererDecorator} title="Custom renderer is applied to 'carMake' field" mdxType="Image" />
{/* ================== AUTO-GENERATED ================== */}
