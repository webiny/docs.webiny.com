...
import assignRoleVideo from "/Users/svenalhamad/Dev/docs.webiny.com/docs/user-guides/6.0.x/security/essentials/assets/assign-role/assign-role-to-user.mp4";
...
<p>{`To follow this tutorial, you need to have the `}<strong parentName="p">{`E-commerce`}</strong>{` role and the `}<strong parentName="p">{`John Doe`}</strong>{` user in your Webiny instance. If you don’t have them, please follow the `}<_Link href="/docs/user-guides/security/essentials/create-user-role">{`Create Role`}</_Link>{` and `}<_Link href="/docs/user-guides/security/essentials/create-user">{`Create User`}</_Link>{` tutorials to create it.`}</p>
...
<li parentName="ol">
<p parentName="li">{`Click `}<strong parentName="p">{`Roles`}</strong>{`.`}</p>
<p parentName="li">{`✔️ The `}<strong parentName="p">{`Roles`}</strong>{` accordion opens.`}</p>
</li>
<li parentName="ol">
