...
import roleCreationVideo from "/Users/svenalhamad/Dev/docs.webiny.com/docs/user-guides/6.0.x/security/essentials/assets/create-user-role/create-user-role.mp4";
...
<p>{`To follow this tutorial, you need to have a content model group named `}<strong parentName="p">{`E-commerce`}</strong>{` in your Webiny instance. If you don’t have it, please follow the `}<_Link href="/docs/user-guides/headless-cms/essentials/create-content-model-group">{`Create Content Model Group`}</_Link>{` tutorial to create it.`}</p>
...
<p parentName="li">{`✔️  The `}<strong parentName="p">{`Roles`}</strong>{` screen opens.`}</p>
</li>
<li parentName="ol">
...
<p parentName="li">{`b. In the `}<strong parentName="p">{`Primary Actions`}</strong>{` dropdown, click `}<strong parentName="p">{`Read, write`}</strong>{`.`}</p>
<p parentName="li">{`c. Under `}<strong parentName="p">{`Publishing actions`}</strong>{`, select `}<strong parentName="p">{`Publish`}</strong>{` and `}<strong parentName="p">{`Unpublish`}</strong>{` checkboxes.`}</p>
</li>
<li parentName="ol">
